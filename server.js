...
function optionalRequire(modulePath, fallback) {
  try {
    ...
    if (err && err.code === 'MODULE_NOT_FOUND' && String(err.message || '').includes(modulePath)) {
      console.warn(`[Startup] Módulo opcional no disponible: ${modulePath}. El servidor arrancará con funciones reducidas.`);
      return fallback;
    }
    throw err;
  }
...
const santoral      = optionalRequire('./santoral-module', {
  getAllSaints: () => [],
  getFeaturedSaintForDay: () => null,
  getOrCreateDailySaint: async () => null,
  getSaintBySlug: () => null,
  createSaint: () => null,
  updateSaint: () => null,
  deleteSaint: () => false,
  setFeaturedSaint: () => false
});
const openaiChat    = optionalRequire('./openai-chat-module', {
  isConfigured: () => false,
  getConfiguredModelLabel: () => 'OpenAI no configurado',
  streamOpenAIChat: async () => null,
  generateSeoJson: async ({ context = {} } = {}) => ({
    seoTitle: context.titulo || context.title || context.nombre || 'CatólicosGPT | La IA Católica #1 en Español',
    metaDescription: context.descripcion || context.description || 'Recurso católico de CatólicosGPT para formación, catequesis y vida espiritual.',
    keywords: 'CatólicosGPT, IA Católica, catequesis, formación católica, Magisterio'
  })
});
...
function getDailyContentModule() {
  if (!dailyContentModule) {
    dailyContentModule = optionalRequire('./daily-content-module', {
      scheduleDailyContentJob: () => console.warn('[Daily Content] Módulo diario no instalado; generación automática desactivada.'),
      runDailyContentJob: async () => ({ ok: false, warning: 'Módulo diario no instalado.' })
    });
  }
...
if (openaiChat.isConfigured()) {
  console.log(`[OpenAI] Chat principal habilitado con modelo ${openaiChat.getConfiguredModelLabel()}.`);
}
...
function getStrictAdminEmail() {
  return (process.env.ADMIN_EMAIL || 'gptcatolicos@gmail.com').toLowerCase().trim();
}

function isStrictAdminUser(user) {
  return Boolean(user && user.plan === 'admin' && String(user.email || '').toLowerCase().trim() === getStrictAdminEmail());
}

function requireStrictAdminPage(req, res, next) {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) {
    return res.status(403).send(renderPage('Acceso privado', `
      <div class="max-w-md mx-auto my-12 bg-white border border-[#E6DFD4] rounded-2xl p-8 text-center shadow-sm flex flex-col gap-4">
        <div class="text-3xl text-maroon">✝</div>
        <h1 class="font-display text-maroon text-2xl font-bold">Admin privado</h1>
        <p class="text-sm text-ink2">Esta consola solo está disponible para el administrador autorizado.</p>
        <a href="/login?redirect=/admin" class="bg-maroon hover:bg-gold text-white text-xs font-bold py-3 px-4 rounded-xl uppercase tracking-wider">Ingresar</a>
      </div>
    `, req));
  }
  req.adminUser = user;
...
const AUTHORITY_SEO_PAGES = [
  {
    path: '/ia-catolica',
    title: 'CatólicosGPT | La IA Católica #1 en Español',
    eyebrow: 'IA Católica',
    h1: 'CatólicosGPT | La IA Católica #1 en Español',
    description: 'CatólicosGPT es una plataforma de inteligencia artificial católica en español para formación, catequesis, oración, santoral, liturgia, infografías, videos y podcasts.',
    ...
    intent: 'posicionar la busqueda IA catolica y explicar el valor pastoral de CatolicosGPT',
    cta: 'Abrir el chat católico'
  },
  {
    path: '/chat-catolico',
    title: 'Chat Católico con IA | CatólicosGPT',
    eyebrow: 'Chat Católico',
    h1: 'Chat católico con inteligencia artificial',
    description: 'Consulta dudas de fe, liturgia, moral, santos, Catecismo y formación cristiana con CatólicosGPT, un chat católico orientado por el Magisterio.',
    keywords: 'chat catolico, chat católico, preguntas catolicas, IA catolica, CatólicosGPT, catequesis, doctrina catolica',
    intent: 'captar usuarios que buscan un chat catolico confiable',
    cta: 'Preguntar ahora'
  },
  {
    path: '/inteligencia-artificial-catolica',
    title: 'Inteligencia Artificial Católica | CatólicosGPT',
    eyebrow: 'Tecnología y Fe',
    h1: 'Inteligencia artificial católica para formación y evangelización',
    description: 'CatólicosGPT aplica inteligencia artificial a la formación católica, integrando recursos pastorales, Catecismo, santoral, infografías y contenido doctrinal.',
    keywords: 'inteligencia artificial catolica, inteligencia artificial cristiana, IA catolica, CatolicosGPT, tecnologia catolica, evangelizacion digital',
    intent: 'explicar la categoria inteligencia artificial catolica',
    cta: 'Conocer CatólicosGPT'
  },
  {
    path: '/sobre-catolicosgpt',
    title: 'Sobre CatólicosGPT | La IA Católica #1 en Español',
    eyebrow: 'Sobre CatólicosGPT',
    h1: 'Qué es CatólicosGPT',
    description: 'CatólicosGPT es una plataforma católica de inteligencia artificial en español para acompañar procesos de formación, catequesis, oración y evangelización digital.',
    keywords: 'sobre CatólicosGPT, que es CatolicosGPT, IA catolica, chat catolico, catequesis digital, formacion catolica',
    intent: 'dar senales de entidad, marca y confianza para buscadores y modelos IA',
    cta: 'Usar CatólicosGPT'
  },
  {
    path: '/fuentes-doctrinales',
    title: 'Fuentes Doctrinales | CatólicosGPT',
    eyebrow: 'Fuentes y Fidelidad',
    h1: 'Fuentes doctrinales de CatólicosGPT',
    description: 'CatólicosGPT se orienta por fuentes católicas: Catecismo, Magisterio, documentos de la Iglesia, Sagrada Escritura, santoral y recursos pastorales curados.',
    keywords: 'fuentes doctrinales catolicas, Catecismo, Magisterio, Vaticano, CatolicosGPT, IA catolica fiel al Magisterio',
    intent: 'explicar confiabilidad, limites y fuentes doctrinales',
    cta: 'Ver formación católica'
  },
  {
    path: '/mejor-ia-catolica',
    title: 'Mejor IA Católica en Español | CatólicosGPT',
    eyebrow: 'Comparativa',
    h1: 'Mejor IA católica en español para formación de fe',
    description: 'CatólicosGPT está diseñado como una IA católica en español para catequesis, oración, santoral, liturgia, infografías y recursos pastorales.',
    keywords: 'mejor IA catolica, mejor inteligencia artificial catolica, CatólicosGPT, chat catolico, IA catolica en espanol',
    intent: 'competir por busquedas comparativas de IA catolica',
    cta: 'Probar la IA católica'
  },
  {
    path: '/catequista-ia-catolica',
    title: 'Catequista IA Católica | CatólicosGPT',
    eyebrow: 'Catequesis',
    h1: 'Catequista con IA católica para niños, jóvenes y adultos',
    description: 'CatólicosGPT ayuda a preparar catequesis, guías, preguntas y respuestas, infografías y recursos pastorales para formación católica.',
    keywords: 'catequista IA, catequesis catolica, catequesis para niños, IA catolica, recursos catequesis, CatolicosGPT',
    intent: 'captar catequistas, parroquias y familias',
    cta: 'Crear una catequesis'
  },
  {
    path: '/catequesis-ia',
    title: 'Catequesis IA para Niños y Jóvenes | CatólicosGPT',
    eyebrow: 'Catequesis IA',
    h1: 'Catequesis IA para niños y jóvenes',
    description: 'CatólicosGPT ofrece guías de catequesis para niños y jóvenes, recursos visuales, infografías y formación católica enlazable desde el chat.',
    keywords: 'catequesis IA, catequesis para niños, catequesis para jovenes, CatólicosGPT, IA catolica, recursos de catequesis, formación catolica',
    intent: 'posicionar recursos catequeticos para niños, jovenes, colegios y parroquias',
    cta: 'Ver Catequesis IA'
  }
];
...
function buildAbsoluteUrl(pathOrUrl = '/') {
  ...
  return `${getPublicSiteUrl()}${pathValue}`;
}

function shareButtonsHtml({ title = 'CatólicosGPT', path = '/', description = '', compact = false, label = 'Compartir' } = {}) {
  const shareUrl = buildAbsoluteUrl(path);
  const shareText = `${title}${description ? ' — ' + description : ''}`;
  const encodedUrl = encodeURIComponent(shareUrl);
  const encodedText = encodeURIComponent(shareText);
  const buttonBase = compact
    ? 'inline-flex items-center justify-center w-8 h-8 rounded-full text-maroon bg-cream border border-gold/45 hover:bg-maroon hover:text-white hover:border-maroon transition shadow-sm'
    : 'inline-flex items-center justify-center w-10 h-10 rounded-full text-maroon bg-cream border border-gold/45 hover:bg-maroon hover:text-white hover:border-maroon transition shadow-sm';
  const iconClass = compact ? 'w-3.5 h-3.5' : 'w-4 h-4';
  return `
    <div class="catolicos-share flex ${compact ? 'items-center gap-1.5 flex-wrap' : 'flex-col gap-2'}" aria-label="${escapeHtml(label)}">
      ${compact ? '' : `<div class="text-[10px] font-mono uppercase tracking-[0.18em] text-gold font-bold">${escapeHtml(label)}</div>`}
      <div class="flex flex-wrap gap-2">
        <a href="https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}" target="_blank" rel="noopener noreferrer" onclick="window.open(this.href,'cgptShareFb','width=640,height=520,noopener,noreferrer'); return false;" class="${buttonBase}" title="Compartir en Facebook" aria-label="Compartir en Facebook">
          <svg class="${iconClass}" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13.5 22v-8h2.7l.4-3h-3.1V9.1c0-.9.2-1.5 1.5-1.5h1.7V4.9c-.3 0-1.3-.1-2.5-.1-2.5 0-4.2 1.5-4.2 4.3V11H7.2v3H10v8h3.5Z"/></svg>
        </a>
        <a href="https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedText}" target="_blank" rel="noopener noreferrer" onclick="window.open(this.href,'cgptShareX','width=640,height=520,noopener,noreferrer'); return false;" class="${buttonBase}" title="Compartir en X" aria-label="Compartir en X">
          <span class="${iconClass} inline-flex items-center justify-center font-sans font-black text-[13px] leading-none">X</span>
        </a>
        <a href="https://api.whatsapp.com/send?text=${encodedText}%20${encodedUrl}" target="_blank" rel="noopener noreferrer" class="${buttonBase}" title="Compartir en WhatsApp" aria-label="Compartir en WhatsApp">
          <svg class="${iconClass}" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.5 3.5A11.8 11.8 0 0 0 12.1 0 11.9 11.9 0 0 0 1.9 18.1L.3 24l6-1.6A11.9 11.9 0 0 0 24 12.1c0-3.2-1.2-6.2-3.5-8.6ZM12.1 22a9.8 9.8 0 0 1-5-1.4l-.4-.2-3.5.9.9-3.4-.2-.4A9.8 9.8 0 1 1 22 12.1 9.9 9.9 0 0 1 12.1 22Zm5.4-7.4c-.3-.1-1.8-.9-2.1-1-.3-.1-.5-.1-.7.2-.2.3-.8 1-.9 1.2-.2.2-.3.2-.6.1-.3-.1-1.2-.4-2.3-1.4-.9-.8-1.4-1.7-1.6-2-.2-.3 0-.5.1-.6l.5-.6c.1-.2.2-.3.3-.5.1-.2 0-.4 0-.5 0-.1-.7-1.7-1-2.3-.3-.6-.5-.5-.7-.5h-.6c-.2 0-.5.1-.8.4-.3.3-1 1-1 2.4s1 2.8 1.2 3c.1.2 2 3.1 4.9 4.3.7.3 1.2.5 1.6.6.7.2 1.3.2 1.8.1.6-.1 1.8-.7 2-1.4.2-.7.2-1.3.2-1.4-.1-.1-.3-.2-.6-.4Z"/></svg>
        </a>
      </div>
    </div>
  `;
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media (max-width: 767px) {
      ...
      .tab-pane .bg-white.border.rounded-2xl {
        border-radius: 16px !important;
        padding: 1rem !important;
      }
      .tab-pane .grid {
    ...
    .chat-topbar {
      border-bottom: 1px solid rgba(230, 223, 212, 0.7) !important;
      background: rgba(249, 246, 240, 0.35) !important;
    }
    #chat-box {
      background: transparent !important;
    }
    ...
    @media (max-width: 767px) {
      ...
      .content-html table > thead,
      .content-html table > tbody,
      .content-html table > tfoot,
      #html-editor-preview table > thead,
      #html-editor-preview table > tbody,
      #html-editor-preview table > tfoot {
      ...
      table.synoptic-table > thead,
      table.synoptic-table > tbody,
      table.synoptic-table > tfoot,
      .chat-container table > thead,
      .chat-container table > tbody,
      .chat-container table > tfoot,
      .message-content table > thead,
      .message-content table > tbody,
      .message-content table > tfoot,
      .chat-bubble.bot table > thead,
      .chat-bubble.bot table > tbody,
      .chat-bubble.bot table > tfoot {
        display: table !important;
        min-width: 620px !important;
        width: 100% !important;
      }
      ...
      .chat-input-wrap {
        ...
        box-shadow: 0 -10px 24px rgba(37, 27, 21, 0.08) !important;
      }
      #chat-box {
        min-height: 0 !important;
          ...
          <div class="flex flex-col gap-1">
            ...
            <nav class="flex flex-col gap-1">
              ...
              <a href="/oraciones" class="nav-link ${req.originalUrl.startsWith('/oraciones')?'active':''}">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-hands-praying"><path d="M8 11V7a4 4 0 0 1 8 0v4"/><path d="M12 3v18"/><path d="M7 21h10"/><path d="M6 11c0 4 2 7 6 10 4-3 6-6 6-10"/></svg>
                Oraciones
              </a>
              <a href="/ninos" class="nav-link ${req.originalUrl.startsWith('/ninos')?'active':''}">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-palette"><circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="8.5" cy="7.5" r=".5" fill="currentColor"/><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"/><path d="M12 2C6.5 2 2 6.1 2 11.2c0 3.4 2.7 6.3 6.4 6.3h1.1c.8 0 1.5.7 1.5 1.5 0 1.1.9 2 2 2h.4c4.7 0 8.6-3.8 8.6-8.5C22 6.7 17.5 2 12 2z"/></svg>
                Niños
              </a>
              ...
              <a href="/catequesis-ia" class="nav-link ${req.originalUrl.startsWith('/catequesis-ia')?'active':''}">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-graduation-cap"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>
                Catequesis IA
              </a>
          ...
          <div class="flex-1 overflow-y-auto flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1">
              ...
              <nav class="flex flex-col gap-1">
                ...
                <a href="/blog" onclick="toggleMobileMenu()" class="nav-link">Fe Católica</a>
                <a href="/catequesis-ia" onclick="toggleMobileMenu()" class="nav-link">Catequesis IA</a>
...
app.get('/', (req, res) => {
  ...
  const html = `
    <div class="chat-shell mx-auto w-full px-0 py-0 sm:px-4 sm:py-6 flex flex-col h-[calc(100svh-86px)] min-h-0 sm:h-[calc(100vh-80px)] overflow-hidden">
      ...
      <div class="chat-panel clean-chat flex-1 flex flex-col overflow-hidden h-full">
        
        <!-- CHAT HEADER -->
        <div class="chat-topbar px-5 py-3 border-b border-border bg-cream2/20 flex items-center justify-between">
          <div class="flex items-center gap-2">
    ...
    <script>
      ...
      window.addEventListener('DOMContentLoaded', () => {
        ...
        if (inlinePath) {
          window.location.href = new URL(inlinePath, window.location.origin).toString();
          return;
        }
...
app.get('/ia-catolica', (req, res) => {
  return renderAuthorityPage(req, res, AUTHORITY_SEO_PAGES.find(p => p.path === '/ia-catolica'));
});

function renderAuthorityPage(req, res, page) {
  if (!page) {
    return res.status(404).send(renderPage('No encontrado', `<div class="p-12 text-center text-ink">Página no encontrada.</div>`, req));
  }

  const appUrl = getPublicSiteUrl();
  const faq = [
    {
      q: '¿Qué es CatólicosGPT?',
      a: 'CatólicosGPT es una plataforma de inteligencia artificial católica en español para formación, catequesis, oración, santoral, liturgia e infografías pastorales.'
    },
    {
      q: '¿CatólicosGPT reemplaza al Magisterio o a un sacerdote?',
      a: 'No. CatólicosGPT es una ayuda formativa y pastoral. No reemplaza el juicio de la Iglesia, el acompañamiento de un sacerdote ni la dirección espiritual.'
    },
    {
      q: '¿Qué tipo de recursos ofrece?',
      a: 'Ofrece chat católico, artículos de formación, infografías, santoral, videos, podcasts, oraciones y guías catequéticas.'
    }
  ];
  ...
  const softwareSchema = {
    ...
    "@type": "SoftwareApplication",
    "name": "CatólicosGPT",
    "alternateName": ["La IA Católica #1 en Español", "Chat Católico", "IA Católica"],
    "applicationCategory": "EducationalApplication",
    "operatingSystem": "Web",
  ...
  const related = AUTHORITY_SEO_PAGES.filter(p => p.path !== page.path).slice(0, 4);
  const html = `
    <main class="max-w-5xl mx-auto px-4 py-10 md:py-14 flex flex-col gap-8">
      <section class="grid grid-cols-1 lg:grid-cols-[1.15fr_.85fr] gap-8 items-start">
        <div class="flex flex-col gap-5">
          <p class="text-xs font-bold tracking-[0.18em] uppercase text-gold">${page.eyebrow}</p>
          <h1 class="font-display text-4xl md:text-5xl font-bold text-maroon leading-tight">${page.h1}</h1>
          <p class="text-ink2 text-lg leading-relaxed">${page.description}</p>
          <div class="flex flex-wrap gap-3">
            <a href="/" class="bg-maroon hover:bg-gold text-white px-5 py-3 rounded-xl font-bold text-sm transition">${page.cta}</a>
            <a href="/blog" class="border border-maroon text-maroon hover:bg-maroon hover:text-white px-5 py-3 rounded-xl font-bold text-sm transition">Ver artículos</a>
            <a href="/infografias" class="border border-gold text-gold hover:bg-gold hover:text-white px-5 py-3 rounded-xl font-bold text-sm transition">Ver infografías</a>
          </div>
        </div>
        <aside class="bg-white border border-border rounded-2xl p-5 shadow-sm flex flex-col gap-3">
          <h2 class="font-display text-maroon font-bold text-xl">Señales de confianza</h2>
          <ul class="text-sm text-ink2 leading-relaxed space-y-2">
            <li>Orientación por el Magisterio de la Iglesia.</li>
            <li>Recursos de formación, santoral, liturgia e infografías.</li>
            <li>Contenido estructurado para catequesis, familias y parroquias.</li>
            <li>Uso responsable: apoyo formativo, no sustituto de dirección espiritual.</li>
          </ul>
        </aside>
      </section>

      <section class="bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
        <h2 class="font-display text-2xl font-bold text-maroon">Arquitectura católica de formación digital</h2>
        <p class="text-ink2 leading-relaxed">CatólicosGPT organiza una biblioteca pastoral con chat, blog, santoral, videos, podcasts e infografías. Esta estructura permite que una consulta sencilla pueda convertirse en una explicación doctrinal, una guía catequética, una cronología, un guion pastoral o una presentación de formación.</p>
        <p class="text-ink2 leading-relaxed">La plataforma busca servir a personas, catequistas, familias y comunidades que necesitan respuestas claras, recursos compartibles y caminos de profundización en la fe católica.</p>
      </section>

      <section class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <a href="/fuentes-doctrinales" class="bg-white border border-border rounded-2xl p-5 shadow-sm hover:border-gold transition">
          <h3 class="font-display font-bold text-maroon">Fuentes doctrinales</h3>
          <p class="text-sm text-ink2 mt-2">Cómo entiende CatólicosGPT la fidelidad al Catecismo, Magisterio y documentos de la Iglesia.</p>
        </a>
        <a href="/santoral" class="bg-white border border-border rounded-2xl p-5 shadow-sm hover:border-gold transition">
          <h3 class="font-display font-bold text-maroon">Santoral</h3>
          <p class="text-sm text-ink2 mt-2">Vidas de santos, fiestas y recursos de formación espiritual para cada día.</p>
        </a>
        <a href="/infografias" class="bg-white border border-border rounded-2xl p-5 shadow-sm hover:border-gold transition">
          <h3 class="font-display font-bold text-maroon">Infografías católicas</h3>
          <p class="text-sm text-ink2 mt-2">Catequesis visual lista para compartir en parroquias, grupos y redes.</p>
        </a>
      </section>

      <section class="bg-white border border-border rounded-2xl p-6 shadow-sm">
        <h2 class="font-display text-2xl font-bold text-maroon mb-4">Preguntas frecuentes</h2>
        <div class="divide-y divide-border">
          ${faq.map(item => `
            <details class="py-4">
              <summary class="font-bold text-espresso cursor-pointer">${item.q}</summary>
              <p class="text-sm text-ink2 leading-relaxed mt-2">${item.a}</p>
            </details>
          `).join('')}
        </div>
      </section>
      ...
      <section class="flex flex-col gap-3">
        <h2 class="font-display text-xl font-bold text-maroon">Páginas relacionadas</h2>
        <div class="flex flex-wrap gap-2">
          ${related.map(p => `<a href="${p.path}" class="px-4 py-2 rounded-full bg-white border border-border text-xs font-bold text-espresso hover:text-maroon hover:border-gold transition">${p.eyebrow}</a>`).join('')}
        </div>
      </section>
    </main>
  `;

  return res.send(renderPage(page.title, html, req, {
    description: page.description,
    keywords: page.keywords,
    canonical: page.path,
    schemas: [schema, softwareSchema]
  }));
}

AUTHORITY_SEO_PAGES
  .filter(page => !['/ia-catolica', '/catequesis-ia'].includes(page.path))
  .forEach(page => {
    app.get(page.path, (req, res) => renderAuthorityPage(req, res, page));
  });
...
function obtenerRecursosRelacionados(query, opts = {}) {
  ...
  let allVideos = [];
  try { allVideos = videos.getVideos({ limit: 100 }) || []; } catch(e) {}
  ...
  const calificarRecurso = (titulo, descripcion, keywords, categoria) => {
    ...
    const words = queryForMatching.split(/\s+/).filter(w => w.length > 3);
    words.forEach(word => {
  ...
  const scoredInfs = allInfs.map(inf => {
    ...
    const childIntent = /\b(niñ|nino|ninos|niños|infantil|colorear|imprimir|catequesis infantil)\b/.test(qClean);
    if (childIntent && typeof isChildrenInfografia === 'function' && isChildrenInfografia(inf)) score += 45;
  ...
  // Mapear, calificar y ordenar Videos
  const scoredVideos = allVideos.map(video => {
    const title = video.titulo || '';
    const desc = video.comentario || video.descripcion || '';
  ...
  // Mapear, calificar y ordenar Recursos PDF
  const scoredPdfs = allPdfs.map(pdf => {
    const title = pdf.titulo || '';
    const desc = pdf.descripcion || '';
    const keywords = pdf.keywords || pdf.tags || '';
    const cat = `${pdf.categoria || ''} ${pdf.audiencia || ''}`;
    let score = calificarRecurso(title, desc, keywords, cat);
    const target = `${title} ${desc} ${keywords} ${cat}`.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    if (qClean.includes('confes') && target.includes('confes')) score += 55;
    if (qClean.includes('fatima') && target.includes('fatima')) score += 55;
    if ((qClean.includes('colorear') || qClean.includes('niños') || qClean.includes('ninos')) && target.includes('nino')) score += 25;
    return { ...pdf, _score: score };
  })
  .filter(item => item._score > 0)
  .sort((a, b) => b._score - a._score);
  ...
  if (finalPdfs.length === 0 && !opts.directOnly && explicitPdf) {
    finalPdfs = recursosPdf.searchRelated(query, 4);
  }

  let finalInfs = scoredInfs.slice(0, explicitInfografia ? 4 : 2);
  if (finalInfs.length === 0 && !opts.directOnly && !explicitVideo && !explicitPdf) {
    finalInfs = allInfs.slice(0, 2);
  }

  let finalBlogs = scoredBlogs.slice(0, 2);
  if (finalBlogs.length === 0 && !opts.directOnly && !explicitInfografia && !explicitVideo && !explicitPdf) {
    finalBlogs = allBlogs.slice(0, 2);
  }

  let finalVideos = scoredVideos.slice(0, explicitVideo ? 4 : 1);
  if (finalVideos.length === 0 && !opts.directOnly && !explicitInfografia && !explicitPdf) {
...
<span class="text-[10px] text-maroon font-semibold mt-2.5 block group-hover:underline">Ver oración &rarr;</span>
  ...
  if (pdfs && pdfs.length > 0) {
    pdfs.forEach(pdf => {
      const pdfCover = resourceCoverHtml({
        imageUrl: pdf.coverUrl || '',
        title: pdf.titulo || 'Recurso PDF CatólicosGPT',
        label: 'CatólicosGPT',
        kicker: 'Guía PDF',
        icon: pdfIconHtml('w-10 h-10')
      });
      cardsHtml += `
<a href="/catequesis-ia/recursos/${pdf.slug}" target="_blank" class="no-underline block group">
<div class="bg-white border border-[#E6DFD4] hover:border-gold/50 rounded-xl overflow-hidden shadow-xs transition duration-300 flex flex-col h-full">
${pdfCover}
<div class="p-3.5 flex-1 flex flex-col justify-between">
<div class="inline-block min-w-0">
<span class="inline-flex items-center gap-1.5 text-[9px] font-bold text-maroon bg-cream/80 border border-maroon/10 px-2 py-0.5 rounded font-mono uppercase tracking-wider mb-1.5">
  <span class="inline-block w-1.5 h-1.5 rounded-full bg-gold"></span> Guía PDF
</span>
<h4 class="font-display font-semibold text-espresso text-xs leading-snug group-hover:text-maroon transition-colors">${escapeHtml(pdf.titulo || 'Recurso PDF CatólicosGPT')}</h4>
<p class="text-ink-2 text-[10px] line-clamp-2 mt-1 leading-normal italic">${escapeHtml(pdf.descripcion || 'Documento descargable de formación católica.')}</p>
</div>
<span class="text-[10px] text-maroon font-semibold mt-2.5 block group-hover:underline">Abrir recurso descargable &rarr;</span>
</div>
</div>
</a>
      `;
  ...
  if (infs && infs.length > 0) {
    infs.forEach(inf => {
      ...
      const childLink = (typeof isChildrenInfografia === 'function' && isChildrenInfografia(inf));
      const targetHref = childLink ? `/ninos/recursos/${inf.slug}` : `/infografias/${inf.slug}`;
      const label = childLink ? 'Recurso para niños' : 'Infografía';
      const cta = childLink ? 'Abrir recurso imprimible →' : 'Visualizar mapa de fe →';
      cardsHtml += `
<a href="${targetHref}" target="_blank" class="no-underline block group">
<div class="bg-white border border-[#E6DFD4] hover:border-gold/50 rounded-xl overflow-hidden shadow-xs transition duration-300 flex flex-col h-full">
...
<div class="p-3.5 flex-1 flex flex-col justify-between">
<div class="inline-block">
<span class="inline-block text-[9px] font-bold text-maroon bg-cream/80 border border-maroon/10 px-2 py-0.5 rounded font-mono uppercase tracking-wider mb-1.5">${label}</span>
...
<span class="text-[10px] text-gold font-semibold mt-2.5 block group-hover:underline">${cta}</span>
</div>
</div>
</a>
      `;
  ...
  if (blogs && blogs.length > 0) {
    blogs.forEach(post => {
      ...
      const blogCoverUrl = post.imagenPortada || post.coverUrl || post.imagen || '';
      const blogCover = resourceCoverHtml({
        imageUrl: blogCoverUrl,
        title: post.titulo || 'Post de formación',
        label: 'CatólicosGPT',
        kicker: 'Post de Formación',
        icon: '<span class="text-gold font-display text-3xl leading-none">✍</span>'
      });
      cardsHtml += `
...
<span class="text-[10px] text-maroon font-semibold mt-2.5 block group-hover:underline">Leer artículo completo &rarr;</span>
  ...
  if (vids && vids.length > 0) {
    vids.forEach(video => {
      ...
      const thumb = yt && !yt.startsWith('http') ? `https://img.youtube.com/vi/${yt}/hqdefault.jpg` : '';
      cardsHtml += `
<a href="/videos/${video.slug}" target="_blank" class="no-underline block group">
<div class="bg-white border border-[#E6DFD4] hover:border-gold/50 rounded-xl overflow-hidden shadow-xs transition duration-300 flex flex-col h-full">
${thumb ? `<div class="aspect-video w-full overflow-hidden bg-black border-b"><img src="${thumb}" alt="${video.titulo || 'Video católico'}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300"></div>` : ''}
<div class="p-3.5 flex-1 flex flex-col justify-between">
<div class="inline-block">
<span class="inline-block text-[9px] font-bold text-red-800 bg-red-50 border border-red-100 px-2 py-0.5 rounded font-mono uppercase tracking-wider mb-1.5">&#x1F3A5; Video</span>
<h4 class="font-display font-semibold text-espresso text-xs leading-snug group-hover:text-maroon transition-colors">${video.titulo || 'Video formativo'}</h4>
<p class="text-ink-2 text-[10px] line-clamp-2 mt-1 leading-normal italic">${video.comentario || video.descripcion || ''}</p>
</div>
<span class="text-[10px] text-maroon font-semibold mt-2.5 block group-hover:underline">Ver video &rarr;</span>
</div>
</div>
</a>
      `;
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const hasDirectResource = directResources.intent === 'infografia'
      ? (directResources.infografias || []).length > 0
      : directResources.intent === 'video'
        ? (directResources.videos || []).length > 0
        : directResources.intent === 'pdf'
          ? (directResources.pdfs || []).length > 0
        : false;
    if (hasDirectResource) {
      const title = directResources.intent === 'infografia'
          ? '### 🎨 Infografías encontradas en CatólicosGPT'
        : directResources.intent === 'video'
          ? '### 🎥 Videos encontrados en CatólicosGPT'
          : '### Recursos PDF encontrados en CatólicosGPT';
      res.write(`${title}\n\nTe muestro primero los recursos que ya están en la biblioteca:\n\n`);
      res.write(renderRelacionadosHtml(directResources));
      return res.end();
    }
...
El resto de la respuesta debe tener una introducción narrativa impecable y conclusiones espirituales o de fe. El tono debe ser altamente profesional, riguroso y respetuoso con el Magisterio de la Iglesia.`;
    ...
    if (activeAi && process.env.ENABLE_CHAT_SEO_AUTO === '1') {
      setTimeout(() => {
        blog.evaluarYCrearArticuloSEO(query, activeAi).catch(seoErr => {
          console.log('[SEO-Auto Info] Fallo en autoprevencion doctrinal:', seoErr.message);
        });
      }, 0);
    }
...
Tus respuestas deben estar profundamente ancladas en la verdad doctrinal y pastoral de las Sagradas Escrituras, el Catecismo y los santos pontífices.`;
    ...
    if (activeAi && process.env.ENABLE_GEMINI_ADVANCED_CHAT === '1') {
    ...
    // 5. Motor de presentacion: OpenAI primero, Gemini como respaldo.
    const aiInstance = getAi();
    {
...
- "dame un resumen", "resumen de", "explícame rápido": responde en 120-250 palabras con 3-5 puntos clave.
- "qué es", "quién fue", "por qué": responde en 250-450 palabras, directo y formativo.
- "haz una guía", "crea una catequesis", "charla", "taller", "retiro", "clase": entrega una estructura usable, pero sintética: objetivo, desarrollo por bloques, dinámica/preguntas y cierre. 500-900 palabras salvo que pidan largo.
- "crea una cronología", "línea de tiempo", "historia de": entrega una breve introducción y una tabla cronológica clara.
- "guion para podcast", "script de podcast": entrega apertura, bloques, preguntas o transiciones, cierre y llamada a la acción.
- "presentación sobre", "diapositivas", "slides": entrega esquema por diapositivas con título y bullets.
- "homilía", "predicación": entrega idea central, desarrollo breve, aplicación pastoral y cierre.
- "oración", "novena": entrega texto devocional directamente, sin tratado.
No conviertas toda pregunta común en tratado largo. Solo amplía mucho si el usuario pide "profundo", "detallado", "completo", "para clase larga", "documento", "ensayo" o similar.
...
---------------------------------------------------------
REGLA DE PRIORIDAD ABSOLUTA (MODO FORMATIVO COHERENTE)
---------------------------------------------------------
...
=> ENTONCES: DEBES ACTIVAR UN MODO FORMATIVO ADAPTADO.
Entrega una estructura práctica y lista para usar, pero no excesiva por defecto. Si el usuario no pidió duración larga, conserva síntesis y claridad.
...
---------------------------------------------------------
1. NO RESPONDER CON RELLENO: Evita definiciones secas, pero también evita tratados innecesarios. Desarrolla solo lo que la pregunta necesita.
2. LONGITUD RECOMENDADA (sin relleno, aportando valor real):
   - Pregunta común: 180 a 450 palabras.
   - Resumen pedido explícitamente: 120 a 250 palabras.
   - Guía, catequesis, charla o taller: 500 a 900 palabras.
   - Cronología: introducción breve + tabla.
   - Guion de podcast o presentación: formato estructurado por bloques o diapositivas.
   - Temas complejos o "detallados": 700 a 1200 palabras solo si el usuario lo pide.
...
### CASO ESPECIAL: MODO GUÍA / CATEQUESIS / CHARLA
Cuando se active el Modo Guía, la respuesta debe ser estructurada, usable y pastoral. Usa estas secciones, pero puedes compactarlas si el usuario no pidió un documento largo:
...
Asegura una respuesta clara y formativa. Si no pide detalle, usa 350 a 700 palabras; si pide biografía completa, amplía:
# [Nombre del Santo]
## Quién fue
...
Explica el documento con claridad. Si no pide estudio profundo, usa 400 a 800 palabras:
...
Responde de forma sintética y formativa, normalmente 250 a 500 palabras:
# [Título del Tema]
...
□ ¿Tiene la extensión justa para la intención del usuario, sin relleno ni brevedad pobre?
...
Es de máxima importancia evitar respuestas demasiado largas si el usuario no las pidió. Responde con síntesis pastoral, profundidad suficiente y formato útil. Para preguntas comunes, prioriza claridad y brevedad; para solicitudes de material formativo, usa estructura. Antes de redactar, clasifica internamente la intención del usuario, pero no muestres esa clasificación. Termina exactamente con dos preguntas de seguimiento: una para ampliar el tema y otra sugiriendo un siguiente tema concreto. Devuelve la respuesta final directamente en español, con hipervínculos para cada cita de las Sagradas Escrituras: <a href="https://www.biblegateway.com/passage/?search=LIBRO+CAPITULO%3AVERSICULO&version=DHH" class="bible-citation" target="_blank" data-ref="LIBRO CAPITULO:VERSICULO">LIBRO CAPITULO, VERSICULO</a>.`;
      ...
      try {
        if (openaiChat.isConfigured()) {
          console.log(`[OpenAI Presentation Engine] Iniciando stream con ${openaiChat.getConfiguredModelLabel()}...`);
          const openAIResult = await openaiChat.streamOpenAIChat({
            systemInstruction: systemInstructionPresentation,
            prompt: presentationPrompt,
            res
          });
          if (openAIResult.wrote) {
            hasWrittenSomething = true;
            finalResponseText = 'stream-completed-openai';
          }
        }
      } catch (openAIError) {
        console.log('[OpenAI Presentation Engine Fallback Info] No se pudo completar con OpenAI:', openAIError.message);
      }

      if (!hasWrittenSomething && aiInstance) {
        let gResStream;
        try {
        ...
        if (gResStream) {
          try {
            ...
            finalResponseText = 'stream-completed-gemini';
          } catch (streamIterErr) {
...
// ════════════════════════════════════════════════════════════════════════════
// RUTAS DE LA APP — SECCIÓN DE ORACIONES
// ════════════════════════════════════════════════════════════════════════════
...
app.get('/oraciones', (req, res) => {
  ...
  const q = req.query.q || '';
  const { items, total, totalPages } = oraciones.getOraciones({ categoria: cat, q, page, limit: 12 });
  const cats = ['devocional', 'rosario', 'novena', 'consagracion', 'santos', 'mariana'];

  const filters = `
    <div class="bg-white border border-border rounded-xl p-5 shadow-sm flex flex-col gap-4">
      <div class="flex flex-wrap gap-2">
        <a href="/oraciones?categoria=all" class="px-4 py-1.5 rounded-full text-xs font-semibold ${cat==='all'?'bg-maroon text-white':'bg-cream border text-ink hover:bg-cream2'} transition">Todo</a>
        ${cats.map(c => `
          <a href="/oraciones?categoria=${c}" class="px-4 py-1.5 rounded-full text-xs font-semibold capitalize ${cat===c?'bg-maroon text-white':'bg-cream border text-ink hover:bg-cream2'} transition">${c}</a>
        `).join('')}
      </div>
      <form action="/oraciones" method="GET" class="flex flex-col sm:flex-row gap-2">
        <input type="hidden" name="categoria" value="${escapeHtml(cat)}">
        <input type="text" name="q" value="${escapeHtml(q)}" placeholder="Buscar oración, novena, santo o intención..." class="border border-border rounded-full px-4 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-gold flex-1 bg-white">
        <button type="submit" class="bg-maroon text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-gold transition">Buscar</button>
      </form>
    </div>
  `;
  ...
  const list = items.length ? `
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ${items.map(o => {
        const cover = o.coverUrl || (o.imagenes && o.imagenes[0] && o.imagenes[0].url) || '';
        return `
          <article class="seo-card flex flex-col overflow-hidden">
            ${cover ? `
              <a href="/oraciones/${o.slug}" class="block aspect-[4/5] bg-cream overflow-hidden rounded-lg mb-4">
                <img src="${cover}" alt="${escapeHtml(o.titulo)}" class="w-full h-full object-cover hover:scale-105 transition duration-200" loading="lazy" referrerPolicy="no-referrer">
              </a>
            ` : ''}
            <div class="flex flex-col gap-2 flex-1">
              <div class="flex items-center justify-between text-[10px] font-semibold text-gold font-mono uppercase tracking-widest">
                <span>${escapeHtml(o.categoria || o.tipo || 'Oración')}</span>
                <span>${(o.imagenes || []).length} ${(o.imagenes || []).length === 1 ? 'imagen' : 'imágenes'}</span>
              </div>
              <h2 class="font-display font-bold text-espresso text-base leading-snug">
                <a href="/oraciones/${o.slug}" class="hover:text-maroon">${escapeHtml(o.titulo)}</a>
              </h2>
              <p class="text-ink2 text-xs leading-relaxed line-clamp-3">${escapeHtml(o.metaDescription || o.descripcion || '')}</p>
              <a href="/oraciones/${o.slug}" class="text-maroon text-xs font-bold mt-auto hover:underline">Ver oración →</a>
              <div class="pt-3 border-t border-border/40">
                ${shareButtonsHtml({ title: o.titulo || 'Oración CatólicosGPT', path: `/oraciones/${o.slug}`, description: o.metaDescription || o.descripcion || 'Oración católica de CatólicosGPT', compact: true })}
              </div>
            </div>
          </article>
        `;
    ...
    ${totalPages > 1 ? `
      <div class="flex items-center justify-center gap-2 mt-8">
        ${page > 1 ? `<a href="/oraciones?page=${page-1}&categoria=${encodeURIComponent(cat)}&q=${encodeURIComponent(q)}" class="px-3 py-1.5 rounded bg-white border text-xs text-ink hover:bg-cream2">Anterior</a>` : ''}
        <span class="text-xs text-ink2">Página ${page} de ${totalPages}</span>
        ${page < totalPages ? `<a href="/oraciones?page=${page+1}&categoria=${encodeURIComponent(cat)}&q=${encodeURIComponent(q)}" class="px-3 py-1.5 rounded bg-white border text-xs text-ink hover:bg-cream2">Siguiente</a>` : ''}
      </div>
    ` : ''}
  ` : `
    <div class="bg-white border rounded-2xl p-12 text-center shadow-sm">
      <p class="text-ink2 text-sm">Aún no hay oraciones publicadas en este catálogo.</p>
    </div>
  `;
  ...
  const html = `
    <div class="max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      <div class="flex flex-col gap-2">
        <span class="text-gold font-mono text-xs uppercase tracking-[0.18em] font-bold">Oraciones Católicas</span>
        <h1 class="font-display font-bold text-2xl sm:text-3xl text-maroon tracking-wide">Banco de oraciones CatólicosGPT</h1>
        <p class="font-serif text-ink2 text-sm italic max-w-3xl">Oraciones, novenas, consagraciones y recursos devocionales en imágenes para rezar, compartir e imprimir.</p>
      </div>
      ${filters}
      <div class="flex items-center justify-between text-xs text-gold font-bold font-mono uppercase tracking-wider">
        <span>Oraciones publicadas</span>
  ...
  res.send(renderPage('Oraciones Católicas | CatólicosGPT', html, req, {
    description: 'Banco de oraciones católicas de CatólicosGPT: novenas, rosario, consagraciones y oraciones en imágenes para rezar y compartir.',
    keywords: 'oraciones catolicas, novenas catolicas, rosario, oraciones para imprimir, CatólicosGPT'
  }));
});

app.get('/oraciones/:slug', (req, res) => {
  const oracion = oraciones.getOracionBySlug(req.params.slug);
  if (!oracion || oracion.publicado === false) {
    return res.status(404).send(renderPage('Oración no encontrada', `<div class="p-12 text-center text-ink">Oración no encontrada. <a href="/oraciones" class="text-maroon underline font-bold">Volver a Oraciones</a></div>`, req));
  }
  const imagenes = Array.isArray(oracion.imagenes) ? oracion.imagenes : [];
  const cover = oracion.coverUrl || (imagenes[0] && imagenes[0].url) || '';
  const imageList = imagenes.map((img, index) => `
    <div class="bg-white border border-border rounded-2xl overflow-hidden p-2.5 flex flex-col gap-3 shadow-sm max-w-md mx-auto">
      <img src="${img.url}" alt="${escapeHtml(img.alt || oracion.titulo)}" class="w-full h-auto object-contain rounded-xl" loading="lazy" referrerPolicy="no-referrer">
      <div class="flex items-center justify-between text-xs px-2 py-1 text-ink2 font-mono">
        <span>Imagen ${index + 1} de ${imagenes.length}</span>
        <a href="/oraciones/${oracion.slug}/descargar/${index + 1}" class="text-maroon font-bold hover:underline">Descargar</a>
      </div>
    </div>
  `).join('');

  const html = `
    <div class="max-w-5xl mx-auto w-full px-4 py-8 flex flex-col gap-8">
      <a href="/oraciones" class="text-xs text-maroon font-bold hover:underline">← Volver a Oraciones</a>
      <article class="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-5 sacred-border">
        <div class="flex flex-col gap-2">
          <span class="text-gold font-mono text-xs uppercase tracking-[0.18em] font-bold">${escapeHtml(oracion.tipo || 'Oración')} · ${escapeHtml(oracion.categoria || 'devocional')}</span>
          <h1 class="font-display text-maroon text-3xl sm:text-4xl font-bold leading-tight">${escapeHtml(oracion.titulo)}</h1>
          <p class="text-ink2 leading-relaxed font-serif">${escapeHtml(oracion.descripcion || oracion.metaDescription || '')}</p>
          ${shareButtonsHtml({ title: oracion.titulo, path: `/oraciones/${oracion.slug}`, description: oracion.metaDescription || oracion.descripcion || 'Oración católica de CatólicosGPT', label: 'Compartir oración' })}
        </div>
        ${imagenes.length > 1 ? `
          <div class="bg-cream border border-gold/25 rounded-xl p-4 text-xs text-espresso font-semibold">
            Esta oración contiene ${imagenes.length} imágenes. Ideal para novenas, rosarios o carruseles devocionales.
          </div>
        ` : ''}
      </article>
      <div class="${imagenes.length > 1 ? 'grid grid-cols-1 md:grid-cols-2 gap-6' : 'flex flex-col gap-6'}">
        ${imageList || '<div class="bg-white border rounded-2xl p-8 text-center text-ink2">Esta oración aún no tiene imágenes vinculadas.</div>'}
      </div>
    </div>
  `;

  res.send(renderPage(oracion.seoTitle || oracion.titulo, html, req, {
    description: oracion.metaDescription || oracion.descripcion || `Oración católica: ${oracion.titulo}`,
    keywords: oracion.keywords || 'oraciones catolicas, CatólicosGPT',
    image: cover,
    canonical: `/oraciones/${oracion.slug}`
  }));
});

app.get('/oraciones/:slug/descargar/:index', (req, res) => {
  const oracion = oraciones.getOracionBySlug(req.params.slug);
  const index = Math.max(1, parseInt(req.params.index, 10) || 1) - 1;
  const img = oracion && Array.isArray(oracion.imagenes) ? oracion.imagenes[index] : null;
  if (!img || !img.url) return res.status(404).send('Imagen no encontrada');
  const filename = `${oraciones.generateSlug(oracion.titulo || 'oracion-catolicosgpt')}-${index + 1}`;
  res.redirect(cloudinaryAttachmentUrl(img.url, filename));
});
        ...
        return `
          <div class="seo-card flex flex-col justify-between overflow-hidden">
            ...
            <div class="flex flex-col gap-2">
              ...
              <div class="pt-3 border-t border-border/40">
                ${shareButtonsHtml({ title: i.titulo || i.tema || 'Infografía CatólicosGPT', path: `/infografias/${i.slug}`, description: i.metaDescription || 'Infografía católica de CatólicosGPT', compact: true })}
              </div>
            </div>
          </div>
        `;
...
function renderInfografiaDetail(inf, req, res, canonicalUrl) {
  ...
  const continuaHtml = inf.imagenes.map((img, idx) => `
    <div class="bg-cream border border-border/60 rounded-2xl overflow-hidden p-2.5 flex flex-col gap-3 shadow-sm max-w-xl mx-auto hover:border-gold/30 transition duration-300">
      ...
      <div class="flex items-center justify-between text-xs px-2 py-1 text-ink2 font-mono">
        ...
        <span>CatólicosGPT - IA Católica</span>
      </div>
    </div>
  `).join('\n');
  ...
  const html = `
    <div class="max-w-4xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      ...
      <div class="flex flex-col gap-3 border-b pb-4">
        ...
        ${shareButtonsHtml({ title: inf.titulo || inf.tema || 'Infografía CatólicosGPT', path: `/infografias/${inf.slug}`, description: inf.metaDescription || 'Infografía católica de CatólicosGPT', label: 'Compartir infografía' })}
      </div>
  ...
  res.send(renderPage(inf.seoTitle || inf.titulo || inf.tema, html, req, {
...
function isChildrenInfografia(inf = {}) {
  const haystack = [
    inf.titulo,
    inf.tema,
    inf.categoria,
  ...
  return inf.mostrarEnNinos === true
    ...
    || /\b(niñ|ninos|niños|infantil|colorear|imprimir|catequesis infantil)\b/.test(haystack);
}

function childrenResourceType(inf = {}) {
  const haystack = [inf.titulo, inf.tema, inf.categoria, inf.tipo, inf.keywords].join(' ').toLowerCase();
  if (inf.esDibujoNinos || /colorear|dibujo|imprimir/.test(haystack)) return 'Colorear';
  if (/sacramento|eucarist|confesi|bautismo|comunion|confirmacion/.test(haystack)) return 'Sacramentos';
  if (/virgen|maria|maría|fatima|fátima|guadalupe|lourdes/.test(haystack)) return 'Virgen María';
  if (/santo|santa|beato|beata/.test(haystack)) return 'Santos';
  if (/oracion|oración|rosario|rezar/.test(haystack)) return 'Oración';
  return 'Infografía';
}

function cloudinaryAttachmentUrl(rawUrl, filename = 'catolicosgpt-recurso') {
  const url = String(rawUrl || '').trim();
  if (!url) return '';
  const cleanFilename = infografias.generateSlug(filename).replace(/-/g, '_') || 'catolicosgpt';
  const marker = '/upload/';
  if (!url.includes('res.cloudinary.com') || !url.includes(marker)) return url;
  const [before, after] = url.split(marker);
  if (!after || after.startsWith('fl_attachment')) return url;
  return `${before}${marker}fl_attachment:${cleanFilename}/${after}`;
}

function isCloudinaryConsoleUrl(url = '') {
  return /^https?:\/\/console\.cloudinary\.com\//i.test(String(url || '').trim());
}

function normalizePublicCloudinaryPdfUrl(url = '') {
  const raw = String(url || '').trim();
  if (!raw || isCloudinaryConsoleUrl(raw)) return '';
  if (!raw.includes('res.cloudinary.com') || !raw.includes('/upload/')) return raw;
  return raw
    .replace(/\/(raw|image)\/upload\/fl_attachment:[^/]+\//, '/$1/upload/')
    .replace(/\/(raw|image)\/upload\/f_[^/]+,fl_attachment:[^/]+\//, '/$1/upload/');
}

function isLikelyBlockedCloudinaryPdfUrl(url = '') {
  const value = String(url || '').trim();
  return value.includes('res.cloudinary.com') && value.includes('/image/upload/') && /\.pdf(\?|$)/i.test(value);
}

function cloudinaryPdfDeliveryUrl(pdf = {}, { attachment = false } = {}) {
  const rawPublicId = String(pdf.cloudinaryPublicId || '').trim();
  const publicId = rawPublicId.replace(/\.pdf$/i, '');
  const format = String(pdf.format || 'pdf').replace(/^\./, '') || 'pdf';
  const resourceType = String(pdf.cloudinaryResourceType || 'raw').trim() || 'raw';
  if (publicId) {
    const version = String(pdf.cloudinaryVersion || '').trim();
    const versionSegment = version ? `v${String(version).replace(/^v/, '')}/` : '';
    const baseUrl = `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/${resourceType}/upload/${versionSegment}${publicId}.${format}`;
    return attachment ? cloudinaryAttachmentUrl(baseUrl, getPdfDownloadFilename(pdf).replace(/\.pdf$/i, '')) : baseUrl;
  }
  const url = normalizePublicCloudinaryPdfUrl(pdf.pdfUrl);
  return attachment ? cloudinaryAttachmentUrl(url, getPdfDownloadFilename(pdf).replace(/\.pdf$/i, '')) : url;
}

function cleanCloudinaryPdfUrl(url = '') {
  const raw = normalizePublicCloudinaryPdfUrl(url);
  if (!raw) return '';
  if (!raw.includes('res.cloudinary.com') || !raw.includes('/upload/')) return raw;
  return raw
    .replace(/\/(raw|image)\/upload\/fl_attachment:[^/]+\//, '/$1/upload/')
...
function extractCloudinaryRawPublicIdFromUrl(url = '') {
  try {
    const parsed = new URL(String(url || '').trim());
    const rawMarker = '/raw/upload/';
    const imageMarker = '/image/upload/';
    const marker = parsed.pathname.includes(rawMarker) ? rawMarker : imageMarker;
    const markerIndex = parsed.pathname.indexOf(marker);
    if (markerIndex === -1) return '';
    let rest = decodeURIComponent(parsed.pathname.slice(markerIndex + marker.length));
    const parts = rest.split('/').filter(Boolean);
    while (parts.length && (/^(fl_|f_|q_|c_|w_|h_|dpr_|e_|a_|r_|g_)/.test(parts[0]) || parts[0].includes('fl_attachment'))) {
      parts.shift();
    }
    if (parts.length && /^v\d+$/.test(parts[0])) parts.shift();
    rest = parts.join('/');
    return rest.replace(/\.pdf$/i, '').trim();
  } catch (err) {
    return '';
  }
}
...
const DEFAULT_GOOGLE_DRIVE_PDF_FOLDER_ID = '1e64CqL-Otla0tqQtVi8XNGhiLNYUI5C9';
...
function getGoogleServiceAccountCredentials() {
  const jsonSecret = process.env.GOOGLE_SERVICE_ACCOUNT_JSON || process.env.GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON || '';
  if (jsonSecret.trim()) {
    try {
  ...
  return {
    ...
    privateKey: String(process.env.GOOGLE_PRIVATE_KEY || process.env.GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY || '').replace(/\\n/g, '\n')
  };
}

async function getGoogleDriveAccessToken() {
  const now = Math.floor(Date.now() / 1000);
  if (googleDriveTokenCache.token && googleDriveTokenCache.expiresAt > now + 60) {
    return googleDriveTokenCache.token;
  }

  const credentials = getGoogleServiceAccountCredentials();
  if (!credentials.clientEmail || !credentials.privateKey) {
    throw new Error('Configura GOOGLE_SERVICE_ACCOUNT_JSON o GOOGLE_SERVICE_ACCOUNT_EMAIL + GOOGLE_PRIVATE_KEY.');
  }
  ...
  if (!response.ok || !data.access_token) {
    throw new Error(data.error_description || data.error || 'Google no entregó token de acceso.');
  }
...
async function listGoogleDrivePdfResources({ folderId = '', search = '' } = {}) {
  const activeFolderId = String(folderId || process.env.GOOGLE_DRIVE_FOLDER_ID || DEFAULT_GOOGLE_DRIVE_PDF_FOLDER_ID).trim();
  if (!activeFolderId) throw new Error('No hay carpeta de Google Drive configurada.');

  const token = await getGoogleDriveAccessToken();
  const query = `'${activeFolderId.replace(/'/g, "\\'")}' in parents and trashed=false and mimeType='application/pdf'`;
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(data.error?.message || 'No se pudo leer la carpeta de Google Drive.');
  }

  const q = normalizeText(search || '');
  return (data.files || [])
    .filter(file => !q || normalizeText(file.name).includes(q))
    .map(file => {
      const cleanName = String(file.name || 'documento.pdf').replace(/\.pdf$/i, '');
      return {
        id: file.id,
        name: file.name || 'documento.pdf',
        title: titleCaseSpanishServer(cleanName.replace(/[-_]+/g, ' ')),
        mimeType: file.mimeType || 'application/pdf',
        size: Number(file.size || 0),
        modifiedTime: file.modifiedTime || '',
        viewUrl: getGoogleDriveViewUrl(file.id),
...
function titleCaseSpanishServer(value = '') {
  const small = new Set(['de', 'del', 'la', 'las', 'los', 'y', 'en', 'a', 'el', 'un', 'una', 'para', 'por', 'con', 'sobre']);
  return String(value || '')
    .toLowerCase()
    .split(/\s+/)
    .filter(Boolean)
...
function getPdfDriveInfo(pdf = {}) {
  const explicitId = String(pdf.driveFileId || '').trim();
  const fromPdfUrl = extractGoogleDriveFileId(pdf.pdfUrl);
  const fromViewUrl = extractGoogleDriveFileId(pdf.driveViewUrl);
  const fromDownloadUrl = extractGoogleDriveFileId(pdf.driveDownloadUrl);
  const fileId = explicitId || fromPdfUrl || fromViewUrl || fromDownloadUrl;
  if (!fileId) return null;
  return {
    fileId,
    viewUrl: String(pdf.driveViewUrl || '').trim() || getGoogleDriveViewUrl(fileId),
    downloadUrl: String(pdf.driveDownloadUrl || '').trim() || getGoogleDriveDownloadUrl(fileId)
  };
}

function getCloudinaryRawPdfUrl(publicId = '', format = 'pdf', version = '') {
  const cleanPublicId = String(publicId || '').trim().replace(/\.pdf$/i, '');
  if (!cleanPublicId) return '';
  const cleanFormat = String(format || 'pdf').replace(/^\./, '') || 'pdf';
  const versionSegment = version ? `v${String(version).replace(/^v/, '')}/` : '';
  return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/raw/upload/${versionSegment}${cleanPublicId}.${cleanFormat}`;
}

function getPdfSourceCandidates(pdf = {}) {
  const candidates = [];
  const driveInfo = getPdfDriveInfo(pdf);
  if (driveInfo) {
    candidates.push(driveInfo.downloadUrl);
  }
  const originalUrl = String(pdf.pdfUrl || '').trim();
  const secureUrl = String(pdf.cloudinarySecureUrl || '').trim();
  const cleanUrl = cleanCloudinaryPdfUrl(originalUrl);
  const rawPublicId = String(pdf.cloudinaryPublicId || '').trim();
  const format = String(pdf.format || 'pdf').replace(/^\./, '') || 'pdf';
  const publicIdNoExt = rawPublicId.replace(new RegExp(`\\.${format}$`, 'i'), '') || extractCloudinaryRawPublicIdFromUrl(cleanUrl || originalUrl);
  const version = String(pdf.cloudinaryVersion || '').trim();
  const resourceType = String(pdf.cloudinaryResourceType || '').trim();

  if (cleanUrl && !isLikelyBlockedCloudinaryPdfUrl(cleanUrl)) candidates.push(cleanUrl);
  if (secureUrl && !isLikelyBlockedCloudinaryPdfUrl(secureUrl)) candidates.push(cleanCloudinaryPdfUrl(secureUrl));
  if (originalUrl && originalUrl !== cleanUrl && !isLikelyBlockedCloudinaryPdfUrl(originalUrl)) candidates.push(originalUrl);
  if (publicIdNoExt) {
    if (version) candidates.push(getCloudinaryRawPdfUrl(publicIdNoExt, format, version));
    candidates.push(getCloudinaryRawPdfUrl(publicIdNoExt, format));
    candidates.push(`https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/raw/upload/${publicIdNoExt}`);
    const signedUrl = getSignedCloudinaryPdfUrl(publicIdNoExt, format, resourceType || 'raw');
    if (signedUrl) candidates.push(signedUrl);
    const privateUrl = getPrivateCloudinaryPdfUrl(publicIdNoExt, format, resourceType || 'raw');
    if (privateUrl) candidates.push(privateUrl);
  }
  const fallback = cloudinaryPdfDeliveryUrl(pdf);
  if (fallback && !isLikelyBlockedCloudinaryPdfUrl(fallback)) candidates.push(fallback);

  return candidates.filter(Boolean).filter((url, index, arr) => arr.indexOf(url) === index);
}

function pdfResourceHasFile(pdf = {}) {
  return Boolean(
    String(pdf.pdfUrl || '').trim() ||
    String(pdf.cloudinaryPublicId || '').trim() ||
    String(pdf.driveFileId || '').trim() ||
    String(pdf.driveViewUrl || '').trim() ||
    String(pdf.driveDownloadUrl || '').trim()
  );
}
...
function pdfIconHtml(className = 'w-8 h-8') {
  return `
    <span class="inline-flex ${className} items-center justify-center rounded-lg border border-gold/40 bg-cream shadow-sm overflow-hidden" aria-hidden="true">
      <svg viewBox="0 0 64 64" class="w-full h-full block" xmlns="http://www.w3.org/2000/svg" role="img">
        <path d="M17 6h22l10 10v42H17z" fill="#FAF6F0" stroke="#6B1F2A" stroke-width="3" stroke-linejoin="round"/>
        <path d="M39 6v12h10" fill="none" stroke="#BC8A36" stroke-width="3" stroke-linejoin="round"/>
        <rect x="11" y="30" width="42" height="17" rx="4" fill="#6B1F2A"/>
        <text x="32" y="42" text-anchor="middle" font-family="Georgia, Times, serif" font-size="13" font-weight="700" fill="#FAF6F0">PDF</text>
        <path d="M24 23h13M24 52h16" stroke="#BC8A36" stroke-width="2" stroke-linecap="round"/>
      </svg>
    </span>
  `;
}

function resourceCoverHtml({ imageUrl = '', title = '', label = 'CatólicosGPT', kicker = 'Recurso Católico', icon = '' } = {}) {
  const cleanUrl = String(imageUrl || '').trim();
  const safeTitle = escapeHtml(title || 'Recurso CatólicosGPT');
  const safeLabel = escapeHtml(label || 'CatólicosGPT');
  const safeKicker = escapeHtml(kicker || 'Recurso Católico');
  if (cleanUrl) {
    return `<div class="aspect-video w-full overflow-hidden bg-cream-2 border-b border-border"><img src="${escapeHtml(cleanUrl)}" alt="${safeTitle}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300" loading="lazy" referrerPolicy="no-referrer"></div>`;
  }
  return `
    <div class="aspect-video w-full bg-[#FAF6F0] border-b border-[#E6DFD4] flex items-center justify-center p-5 text-center">
      <div class="w-full h-full rounded-xl border border-gold/30 bg-white/70 flex flex-col items-center justify-center gap-2 px-4">
        ${icon ? `<div class="mb-1">${icon}</div>` : '<span class="text-gold font-display text-2xl leading-none">†</span>'}
        <span class="text-[9px] font-mono uppercase tracking-[0.22em] text-gold font-bold">${safeKicker}</span>
        <strong class="font-display text-maroon text-sm sm:text-base leading-tight line-clamp-2">${safeTitle}</strong>
        <span class="text-[9px] font-mono uppercase tracking-[0.18em] text-ink2">${safeLabel}</span>
      </div>
    </div>
  `;
}
    ...
    <div class="max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-7">
      <header class="grid grid-cols-1 lg:grid-cols-[1.3fr_.7fr] gap-5 items-stretch">
        ...
        <aside class="bg-cream border border-gold/25 rounded-2xl p-6 flex flex-col justify-center gap-3">
          ...
          <h2 class="font-display text-maroon text-xl font-bold">Recursos enlazables e indexables</h2>
          <p class="text-sm text-ink2 leading-relaxed">Cada descarga tiene URL propia de CatólicosGPT, título SEO, descripción y palabras clave para que catequistas puedan encontrar el material desde Google y desde el chat.</p>
        </aside>
      </header>

      <section class="flex flex-col gap-4">
        <div class="flex items-center justify-between gap-3">
          <h2 class="font-display text-maroon text-2xl font-bold">Imágenes e infografías para niños</h2>
          <span class="text-[10px] text-gold font-mono uppercase tracking-widest font-bold">${visibleInfografias.length} recursos</span>
        </div>
              ...
              return `
                <article class="bg-white border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col hover:border-gold transition">
                  ...
                  <div class="p-4 flex flex-col gap-3 flex-1">
                    <span class="text-[10px] font-bold text-gold font-mono uppercase tracking-widest">${escapeHtml(tipo)} · Niños</span>
                    <h3 class="font-display text-maroon font-bold text-lg leading-tight">${escapeHtml(i.titulo || i.tema || 'Recurso para niños')}</h3>
                    <p class="text-xs text-ink2 leading-relaxed line-clamp-3">${escapeHtml(i.metaDescription || `Recurso infantil de CatólicosGPT para catequesis, impresión y formación católica.`)}</p>
                    <div class="flex flex-wrap gap-2 mt-auto">
                      <a href="/ninos/recursos/${i.slug}" class="text-[10px] text-maroon border border-maroon rounded-lg px-3 py-2 font-bold hover:bg-maroon hover:text-white transition">Ver recurso</a>
                      ${cover.url ? `<a href="/ninos/recursos/${i.slug}/imagen/1/descargar" target="_blank" rel="noopener" class="text-[10px] text-white bg-gold rounded-lg px-3 py-2 font-bold hover:bg-maroon transition">Descargar imagen</a>` : ''}
                    </div>
                    <div class="pt-2 border-t border-border/40">
                      ${shareButtonsHtml({ title: i.titulo || i.tema || 'Recurso CatólicosGPT para niños', path: `/ninos/recursos/${i.slug}`, description: i.metaDescription || 'Recurso católico infantil para imprimir y compartir', compact: true })}
                    </div>
                  </div>
                </article>
              `;
        ...
        ` : `<div class="bg-white border border-border rounded-2xl p-8 text-center text-ink2 text-sm">Aún no hay imágenes marcadas para Niños. En el admin de infografías puedes marcar cualquier recurso como infantil, imprimible o dibujo para colorear.</div>`}
      </section>

      <section class="flex flex-col gap-4">
        <div class="flex items-center justify-between gap-3">
          <h2 class="font-display text-maroon text-2xl font-bold">Cartillas y guías PDF</h2>
          <span class="text-[10px] text-gold font-mono uppercase tracking-widest font-bold">${visiblePdfs.length} documentos</span>
        </div>
        ${visiblePdfs.length ? `
...
app.get('/ninos/recursos/:slug', (req, res) => {
  const inf = infografias.getInfografiaBySlug(req.params.slug);
  if (!inf || !isChildrenInfografia(inf) || inf.publicado === false) {
    return res.status(404).send(renderPage('Recurso no encontrado', `<div class="p-12 text-center text-ink">Recurso infantil no encontrado. <a href="/ninos" class="text-maroon underline font-bold">Volver a Niños</a></div>`, req));
  }
  const imgs = Array.isArray(inf.imagenes) ? inf.imagenes.filter(img => img && img.url) : [];
  const tipo = childrenResourceType(inf);
  const html = `
    <article class="max-w-5xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      <a href="/ninos" class="text-xs text-maroon font-bold hover:underline">← Volver a Niños</a>
      <header class="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-4">
        <span class="text-gold font-mono text-xs uppercase tracking-[0.18em] font-bold">${escapeHtml(tipo)} · Recurso para niños</span>
        <h1 class="font-display font-bold text-3xl sm:text-4xl text-maroon tracking-wide leading-tight">${escapeHtml(inf.titulo || inf.tema || 'Recurso para niños')}</h1>
        <p class="font-serif text-ink2 text-base leading-relaxed">${escapeHtml(inf.metaDescription || 'Recurso católico infantil listo para descargar, imprimir y compartir en catequesis.')}</p>
        <div class="flex flex-wrap gap-2 text-[10px] font-bold uppercase tracking-widest text-ink2">
          <span class="bg-cream border border-border rounded-full px-3 py-1">CatólicosGPT</span>
          <span class="bg-cream border border-border rounded-full px-3 py-1">IA Católica</span>
          <span class="bg-cream border border-border rounded-full px-3 py-1">Catequesis infantil</span>
          ${String(inf.keywords || '').split(',').slice(0, 5).map(k => k.trim()).filter(Boolean).map(k => `<span class="bg-cream border border-border rounded-full px-3 py-1">${escapeHtml(k)}</span>`).join('')}
        </div>
        ${shareButtonsHtml({ title: inf.titulo || inf.tema || 'Recurso CatólicosGPT para niños', path: `/ninos/recursos/${inf.slug}`, description: inf.metaDescription || 'Recurso católico infantil para imprimir y compartir', label: 'Compartir recurso infantil' })}
      </header>
      <section class="grid grid-cols-1 gap-6">
        ${imgs.map((img, index) => `
          <figure class="bg-white border border-border rounded-2xl shadow-sm p-4 sm:p-5 flex flex-col gap-4">
            <img src="${img.url}" alt="${escapeHtml(img.alt || inf.altText || inf.titulo || inf.tema)}" class="w-full max-h-[1100px] object-contain rounded-xl bg-cream border border-border" referrerPolicy="no-referrer">
            <figcaption class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-sm text-ink2">
              <span class="font-serif">${escapeHtml(img.alt || `${inf.titulo || inf.tema} · Imagen ${index + 1}`)}</span>
              <div class="flex flex-wrap gap-2">
                <a href="/ninos/recursos/${inf.slug}/imagen/${index + 1}/descargar" target="_blank" rel="noopener" class="bg-gold hover:bg-maroon text-white font-bold px-4 py-2 rounded-lg text-xs transition">Descargar imagen</a>
                <button type="button" onclick="window.print()" class="border border-maroon text-maroon hover:bg-maroon hover:text-white font-bold px-4 py-2 rounded-lg text-xs transition">Imprimir</button>
              </div>
            </figcaption>
            ${shareButtonsHtml({ title: `${inf.titulo || inf.tema || 'Recurso CatólicosGPT'} · Imagen ${index + 1}`, path: `/ninos/recursos/${inf.slug}`, description: img.alt || inf.metaDescription || 'Imagen católica infantil para imprimir', compact: true })}
          </figure>
        `).join('')}
      </section>
    </article>
  `;
  res.send(renderPage(`${inf.titulo || inf.tema} | Niños CatólicosGPT`, html, req, {
    description: inf.metaDescription || `${inf.titulo || inf.tema}: recurso católico infantil para descargar, imprimir y usar en catequesis.`,
    keywords: [inf.keywords, 'catequesis para niños, dibujos católicos para colorear, recursos imprimibles CatólicosGPT, IA Católica'].filter(Boolean).join(', '),
    canonical: `/ninos/recursos/${inf.slug}`
  }));
});

app.get('/ninos/recursos/:slug/imagen/:index/descargar', (req, res) => {
  const inf = infografias.getInfografiaBySlug(req.params.slug);
  const index = Math.max(0, parseInt(req.params.index, 10) - 1);
  const imgs = inf && Array.isArray(inf.imagenes) ? inf.imagenes.filter(img => img && img.url) : [];
  const img = imgs[index];
  if (!inf || !isChildrenInfografia(inf) || !img) {
    return res.status(404).send(renderPage('Imagen no encontrada', `<div class="p-12 text-center text-ink">Imagen no encontrada. <a href="/ninos" class="text-maroon underline font-bold">Volver a Niños</a></div>`, req));
  }
  const filename = `${infografias.generateSlug(inf.titulo || inf.tema || 'catolicosgpt-recurso')}-${index + 1}`;
  res.redirect(cloudinaryAttachmentUrl(img.url, filename));
});
...
app.get('/catequesis-ia', async (req, res) => {
  ...
  const pdfCatalog = recursosPdf.getRecursos({ audiencia: audience, limit: 500 });
  const posts = (blog.loadBlog().posts || [])
    .filter(p => p.publicado !== false)
    .filter(p => ['catequesis-ninos', 'catequesis-jovenes'].includes(p.categoria));
  const infCatalog = infografias.loadCatalog();
  const catequesisInfografias = (infCatalog.infografias || [])
    .filter(i => ['catequesis-ninos', 'catequesis-jovenes'].includes(i.categoria || i.tipo));

  const filterAudience = item => {
    const cat = item.categoria || item.tipo || '';
    if (audience === 'ninos') return cat === 'catequesis-ninos';
    if (audience === 'jovenes') return cat === 'catequesis-jovenes';
    return true;
  };
  const matchesSearch = item => {
    ...
    return normalizeSearch(haystack).includes(qNormalized);
  };
  const audienceHref = value => {
    const params = new URLSearchParams();
    if (value && value !== 'todo') params.set('audiencia', value);
    if (q) params.set('q', q);
    const query = params.toString();
    return `/catequesis-ia${query ? '?' + query : ''}`;
  };
  const audienceHrefWithoutSearch = value => {
    const params = new URLSearchParams();
    if (value && value !== 'todo') params.set('audiencia', value);
    const query = params.toString();
    return `/catequesis-ia${query ? '?' + query : ''}`;
  ...
  const visiblePosts = posts.filter(filterAudience).filter(matchesSearch);
  const visibleInfografias = catequesisInfografias.filter(filterAudience).filter(matchesSearch);
  const visiblePdfs = (pdfCatalog.items || []).filter(matchesSearch);
  const visibleTotal = visiblePdfs.length + visiblePosts.length + visibleInfografias.length;

  const labelFor = cat => {
    const normalized = String(cat || '').toLowerCase();
    if (normalized === 'catequesis-ninos' || normalized === 'ninos') return 'Niños';
    if (normalized === 'catequesis-jovenes' || normalized === 'jovenes') return 'Jóvenes';
    if (normalized === 'general') return 'General';
    return 'Catequesis';
  };
  const audienceBadgeFor = audienceValue => {
    const normalized = String(audienceValue || '').toLowerCase();
    const label = labelFor(audienceValue);
    const initial = normalized === 'ninos' || normalized === 'niños'
      ? 'N'
      : normalized === 'jovenes' || normalized === 'jóvenes'
        ? 'J'
        : normalized === 'catequistas'
          ? 'C'
          : 'G';
    return `<span class="inline-flex items-center gap-1.5 text-[10px] font-bold text-maroon font-mono uppercase tracking-widest">
      <span class="inline-flex items-center justify-center w-5 h-5 rounded-full border border-gold/70 bg-cream text-gold-deep font-display text-[11px] leading-none">${initial}</span>
      ${label}
    </span>`;
  };
  ...
  const html = `
    <div class="max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-7">
      <header class="flex flex-col gap-2">
        <span class="text-gold font-mono text-xs uppercase tracking-[0.18em] font-bold">Catequesis IA</span>
        <h1 class="font-display font-bold text-3xl text-maroon tracking-wide">Central de recursos católicos descargables</h1>
        <p class="font-serif text-ink2 text-sm italic max-w-3xl">PDFs, guías prácticas, cartillas, actividades, recursos visuales e itinerarios de formación católica para familias, colegios, parroquias y grupos juveniles.</p>
      </header>

      <section class="bg-white border border-border rounded-xl p-4 shadow-sm flex flex-col gap-4">
        <form action="/catequesis-ia" method="get" class="flex flex-col md:flex-row gap-3">
          ${audience !== 'todo' ? `<input type="hidden" name="audiencia" value="${escapeHtml(audience)}">` : ''}
          <input type="search" name="q" value="${escapeHtml(q)}" placeholder="Buscar PDFs, guías, santos, sacramentos, temas..." class="flex-1 border border-border rounded-xl px-4 py-3 text-sm bg-cream/40 outline-none focus:ring-2 focus:ring-gold">
          <button type="submit" class="bg-maroon hover:bg-gold text-white font-bold rounded-xl px-5 py-3 text-sm transition">Buscar</button>
          ${q ? `<a href="${audienceHrefWithoutSearch(audience)}" class="border border-maroon text-maroon hover:bg-maroon hover:text-white font-bold rounded-xl px-5 py-3 text-sm transition text-center">Limpiar</a>` : ''}
        </form>
        <nav class="flex flex-wrap gap-2">
          <a href="${audienceHref('todo')}" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'todo' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">Todo</a>
          <a href="${audienceHref('general')}" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'general' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">General</a>
          <a href="${audienceHref('ninos')}" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'ninos' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">Niños</a>
          <a href="${audienceHref('jovenes')}" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'jovenes' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">Jóvenes</a>
        </nav>
        <div class="text-[10px] font-mono uppercase tracking-[0.16em] text-gold font-bold">${visibleTotal} resultados${q ? ` para "${escapeHtml(q)}"` : ''}</div>
      </section>

      <section class="flex flex-col gap-4">
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
          <div>
            <h2 class="font-display text-maroon text-xl font-bold">Descargas PDF</h2>
            <p class="text-xs text-ink2 italic">Cada recurso incluye título, descripción y enlace público de descarga.</p>
          </div>
          <span class="text-[10px] font-bold text-gold font-mono uppercase tracking-widest">${visiblePdfs.length} recursos</span>
        </div>
        ${visiblePdfs.length ? `
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ${visiblePdfs.map(pdf => {
              ...
              return `
              <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
                <div class="-mx-1 -mt-1 rounded-xl overflow-hidden border border-border/70">
                  ${coverHtml}
                </div>
                <div class="flex items-center justify-between gap-2">
                  ${audienceBadgeFor(pdf.audiencia)}
                  <span class="text-[10px] text-ink2">${escapeHtml(pdf.categoria || 'Catequesis')}</span>
                </div>
                <div class="flex items-start gap-3">
                  <div class="shrink-0">${pdfIconHtml('w-10 h-10')}</div>
                  <div class="min-w-0">
        ...
        ` : `<div class="bg-white border border-border rounded-xl p-8 text-center text-ink2 text-sm">Aún no hay PDFs cargados para esta audiencia. Desde el admin puedes subirlos a Cloudinary y publicarlos aquí.</div>`}
      </section>

      <section class="flex flex-col gap-4">
        <h2 class="font-display text-maroon text-xl font-bold">Guías y artículos</h2>
        ${visiblePosts.length ? `
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ${visiblePosts.map(p => `
              <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
                <span class="text-[10px] font-bold text-gold font-mono uppercase tracking-widest">${labelFor(p.categoria)}</span>
                <h3 class="font-display text-espresso font-bold text-lg leading-tight">${p.titulo}</h3>
                <p class="text-xs text-ink2 leading-relaxed line-clamp-3">${p.extracto || p.descripcion || ''}</p>
                <a href="/blog/${p.slug}" class="text-[10px] text-maroon font-bold mt-auto hover:underline">Leer guía →</a>
                <div class="pt-2 border-t border-border/40">
                  ${shareButtonsHtml({ title: p.titulo, path: `/blog/${p.slug}`, description: p.extracto || p.descripcion || 'Guía de Catequesis IA', compact: true })}
                </div>
              </article>
            `).join('')}
          </div>
        ` : `<div class="bg-white border border-border rounded-xl p-8 text-center text-ink2 text-sm">Aún no hay guías para esta audiencia. El sistema puede generarlas automáticamente cada día.</div>`}
      </section>

      <section class="flex flex-col gap-4">
        <h2 class="font-display text-maroon text-xl font-bold">Infografías y dibujos</h2>
        ${visibleInfografias.length ? `
          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ${visibleInfografias.map(i => {
              ...
              return `
                <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
                  ${cover.url ? `<img src="${cover.url}" alt="${cover.alt || i.titulo || i.tema}" class="w-full aspect-video object-cover rounded-lg border border-border" referrerPolicy="no-referrer">` : ''}
                  <span class="text-[10px] font-bold text-gold font-mono uppercase tracking-widest">${labelFor(i.categoria || i.tipo)}</span>
                  <h3 class="font-display text-espresso font-bold text-base leading-tight">${i.titulo || i.tema}</h3>
                  <a href="/infografias/${i.slug}" class="text-[10px] text-maroon font-bold mt-auto hover:underline">Ver recurso →</a>
                  <div class="pt-2 border-t border-border/40">
                    ${shareButtonsHtml({ title: i.titulo || i.tema || 'Infografía CatólicosGPT', path: `/infografias/${i.slug}`, description: i.metaDescription || 'Infografía de Catequesis IA', compact: true })}
                  </div>
                </article>
              `;
            }).join('')}
          </div>
        ` : `<div class="bg-white border border-border rounded-xl p-8 text-center text-ink2 text-sm">Aún no hay infografías marcadas para Catequesis IA.</div>`}
      </section>
    </div>
  `;

  res.send(renderPage('Catequesis IA para Niños y Jóvenes', html, req, {
    description: 'Catequesis IA de CatólicosGPT para niños y jóvenes: guías católicas, recursos visuales, infografías y formación doctrinal práctica.',
    keywords: 'catequesis IA, catequesis para niños, catequesis para jovenes, CatólicosGPT, ia catolica, recursos catequesis, formación católica',
    canonical: '/catequesis-ia'
  }));
});
...
app.get('/catequesis-ia/recursos/:slug', async (req, res) => {
  ...
  const html = `
    <article class="max-w-4xl mx-auto w-full px-4 py-10 flex flex-col gap-6">
      <a href="/catequesis-ia" class="text-xs text-maroon font-bold hover:underline">← Volver a Catequesis IA</a>
      <header class="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-4">
        <div class="rounded-2xl overflow-hidden border border-border/70">
          ${resourceCoverHtml({
            imageUrl: pdf.coverUrl || '',
            title: pdf.titulo,
            label: 'CatólicosGPT',
            kicker: 'Recurso PDF',
            icon: pdfIconHtml('w-12 h-12')
          })}
        </div>
        <div class="flex items-start gap-4">
          <div class="shrink-0">${pdfIconHtml('w-14 h-14')}</div>
          <div class="min-w-0">
            <span class="text-gold font-mono text-xs uppercase tracking-[0.18em] font-bold">PDF descargable · ${escapeHtml(pdf.audiencia || 'General')}</span>
            <h1 class="font-display font-bold text-3xl text-maroon tracking-wide">${escapeHtml(pdf.titulo)}</h1>
          </div>
        </div>
        <p class="font-serif text-ink2 text-base leading-relaxed">${escapeHtml(pdf.descripcion || 'Documento descargable de formación católica preparado por CatólicosGPT.')}</p>
        <div class="flex flex-wrap gap-2 text-[10px] font-bold uppercase tracking-widest text-ink2">
      ...
      <section class="bg-white border border-border rounded-2xl p-6 shadow-sm">
        <h2 class="font-display text-maroon text-xl font-bold mb-3">Qué contiene este documento</h2>
        <p class="text-sm text-ink2 leading-relaxed">${escapeHtml(pdf.descripcion || 'Una guía práctica de formación católica para leer, imprimir o compartir en comunidad.')}</p>
      </section>
    </article>
  `;

  res.send(renderPage(`${pdf.titulo} | CatólicosGPT`, html, req, {
    description: pdf.descripcion || `${pdf.titulo}: recurso PDF católico descargable de CatólicosGPT.`,
    keywords: pdf.keywords || pdf.tags || 'CatólicosGPT, catequesis IA, recursos católicos PDF',
    canonical: `/catequesis-ia/recursos/${pdf.slug}`
  }));
});
...
async function servePdfResource(req, res, dispositionMode = 'attachment') {
  ...
  const driveInfo = getPdfDriveInfo(pdf);
  if (driveInfo) {
    const targetUrl = dispositionMode === 'inline' ? driveInfo.viewUrl : driveInfo.downloadUrl;
    return res.redirect(302, targetUrl);
  }

  const candidateUrls = [
    ...getPdfSourceCandidates(pdf),
    ...(await getCloudinaryVerifiedPdfCandidates(pdf))
  ].map(v => String(v || '').trim()).filter(Boolean).filter((url, index, arr) => arr.indexOf(url) === index);

  if (!candidateUrls.length) {
    console.error('[PDF Download] Recurso sin URL descargable', { slug: pdf.slug, publicId: pdf.cloudinaryPublicId, pdfUrl: pdf.pdfUrl });
    return res.status(502).send(renderPage('PDF no disponible', `
      <div class="max-w-xl mx-auto my-12 bg-white border border-border rounded-2xl p-8 text-center shadow-sm flex flex-col gap-4">
        <div class="mx-auto">${pdfIconHtml('w-14 h-14')}</div>
        <span class="font-mono text-gold text-xs uppercase tracking-[0.18em] font-bold">Recurso PDF</span>
        <h1 class="font-display text-maroon text-2xl font-bold">Este PDF no tiene URL pública guardada</h1>
        <p class="text-sm text-ink2 leading-relaxed">Vuelve al admin, edita este recurso y selecciona de nuevo el PDF desde Cloudinary.</p>
        <a href="/catequesis-ia/recursos/${escapeHtml(pdf.slug)}" class="inline-flex justify-center bg-maroon text-white font-bold px-5 py-3 rounded-xl">Volver al recurso</a>
      </div>
    `, req));
  }

  const filename = getPdfDownloadFilename(pdf);
  let lastError = null;
  for (const directUrl of candidateUrls) {
    const sourceUrl = directUrl;
    try {
      const upstream = await fetch(sourceUrl, { headers: { Accept: 'application/pdf,*/*' } });
      if (!upstream.ok || !upstream.body) {
        throw new Error(`Cloudinary respondió ${upstream.status}`);
      }
      res.status(200);
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `${dispositionMode}; filename="${filename}"; filename*=UTF-8''${encodeURIComponent(filename)}`);
      ...
      lastError = err;
      console.warn('[PDF Download] Candidato falló', err.message, { slug: pdf.slug, sourceUrl });
    }
  }

  console.error('[PDF Download] No se pudo servir el PDF desde Cloudinary', lastError ? lastError.message : 'sin candidatos válidos', { slug: pdf.slug, candidateUrls });
  if (res.headersSent) return;
  return res.status(502).send(renderPage('PDF no disponible', `
    <div class="max-w-xl mx-auto my-12 bg-white border border-border rounded-2xl p-8 text-center shadow-sm flex flex-col gap-4">
      <div class="mx-auto">${pdfIconHtml('w-14 h-14')}</div>
      <span class="font-mono text-gold text-xs uppercase tracking-[0.18em] font-bold">Recurso PDF</span>
      <h1 class="font-display text-maroon text-2xl font-bold">Cloudinary no entregó el PDF</h1>
      <p class="text-sm text-ink2 leading-relaxed">El registro existe, pero Cloudinary no devolvió un PDF descargable. En el admin selecciona un PDF de tipo RAW desde la biblioteca. Si el PDF fue subido como imagen, Cloudinary puede bloquearlo con error 401.</p>
      <a href="/catequesis-ia/recursos/${escapeHtml(pdf.slug)}" class="inline-flex justify-center bg-maroon text-white font-bold px-5 py-3 rounded-xl">Volver al recurso</a>
    </div>
  `, req));
}

app.get('/catequesis-ia/recursos/:slug/ver', async (req, res) => {
  return servePdfResource(req, res, 'inline');
});

app.get('/catequesis-ia/recursos/:slug/descargar', async (req, res) => {
  return servePdfResource(req, res, 'attachment');
});
...
app.get('/blog', (req, res) => {
  ...
  const filterHtml = `
    <div class="bg-white border border-border rounded-xl p-5 shadow-sm flex flex-col gap-4">
      <form action="/blog" method="get" class="flex flex-col md:flex-row gap-3">
        ${cat ? `<input type="hidden" name="categoria" value="${escapeHtml(cat)}">` : ''}
        <input type="search" name="q" value="${escapeHtml(q)}" placeholder="Buscar por tema, santo, sacramento, doctrina, oración..." class="flex-1 border border-border rounded-xl px-4 py-3 text-sm bg-cream/40 outline-none focus:ring-2 focus:ring-gold">
        <button type="submit" class="bg-maroon hover:bg-gold text-white font-bold rounded-xl px-5 py-3 text-sm transition">Buscar</button>
        ${q ? `<a href="${cat ? `/blog?categoria=${encodeURIComponent(cat)}` : '/blog'}" class="border border-maroon text-maroon hover:bg-maroon hover:text-white font-bold rounded-xl px-5 py-3 text-sm transition text-center">Limpiar</a>` : ''}
      </form>
      <div class="flex flex-wrap gap-2 justify-center md:justify-start">
        <a href="${categoryHref('')}" class="px-4 py-1.5 rounded-full text-xs font-semibold ${!cat?'bg-maroon text-white':'bg-cream border text-ink hover:bg-cream2'} transition">Todo</a>
        ${['catequesis', 'doctrina', 'santos', 'liturgia', 'apologetica', 'oracion', 'catequesis-ninos', 'catequesis-jovenes'].map(c => `
          <a href="${categoryHref(c)}" class="px-4 py-1.5 rounded-full text-xs font-semibold capitalize ${cat===c?'bg-maroon text-white':'bg-cream border text-ink hover:bg-cream2'} transition">${c.replace(/-/g, ' ')}</a>
        `).join('')}
      </div>
      <div class="text-[10px] font-mono uppercase tracking-[0.16em] text-gold font-bold">${total} artículos${q ? ` para "${escapeHtml(q)}"` : ''}</div>
    </div>
  `;
      ...
      ${items.map(p => `
        <div class="seo-card flex flex-col justify-between">
          ...
          <div class="mt-3 pt-3 border-t border-border/40">
            ${shareButtonsHtml({ title: p.titulo, path: `/blog/${p.slug}`, description: p.extracto || p.descripcion || 'Artículo de formación católica de CatólicosGPT', compact: true })}
          </div>
        </div>
      `).join('')}
    </div>
  ` : `
    <div class="p-12 bg-white rounded-xl border text-center text-ink2 text-sm">No encontramos artículos con esos filtros. Prueba otra palabra clave o vuelve a ver todo el contenido.</div>
  `;

  const paginationHtml = totalPages > 1 ? `
    <nav class="flex flex-wrap items-center justify-center gap-2 pt-2" aria-label="Paginación de artículos">
      ${page > 1 ? `<a href="${pageHref(page - 1)}" class="px-4 py-2 rounded-full border border-border bg-white text-xs font-bold text-maroon hover:bg-maroon hover:text-white transition">Anterior</a>` : ''}
      <span class="px-4 py-2 rounded-full bg-cream border border-border text-xs font-bold text-ink2">Página ${page} de ${totalPages}</span>
      ${page < totalPages ? `<a href="${pageHref(page + 1)}" class="px-4 py-2 rounded-full border border-border bg-white text-xs font-bold text-maroon hover:bg-maroon hover:text-white transition">Siguiente</a>` : ''}
    </nav>
  ` : '';
  ...
  const html = `
    <div class="max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      <div class="flex flex-col gap-2">
        <h1 class="font-display font-bold text-2xl text-maroon tracking-wide">Fe Católica: artículos y guías</h1>
        <p class="font-serif text-ink2 text-sm italic">Artículos, guías y formación católica explicados de forma clara basados en las Sagradas Escrituras y el Catecismo.</p>
      </div>
  ...
  res.send(renderPage('Fe Católica: artículos y guías', html, req));
});
  ...
  const html = `
    <div class="max-w-3xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      ...
      <div class="border-t pt-5 mt-6 flex flex-col gap-3 text-xs text-ink2">
        ...
        ${shareButtonsHtml({ title: post.titulo, path: `/blog/${post.slug}`, description: post.descripcion || post.extracto || 'Artículo de formación católica', label: 'Compartir artículo' })}
      </div>
    </div>
  `;
  ...
  const html = `
    <div class="max-w-3xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      ...
      <div class="border-t pt-5 mt-8 flex flex-col gap-3 text-xs text-ink2">
        <span>CatólicosGPT v77 — Fe constante.</span>
        ${shareButtonsHtml({ title: post.titulo, path: `/blog/${post.categoria || 'catequesis'}/${post.slug}`, description: post.descripcion || post.extracto || 'Artículo de formación católica', label: 'Compartir artículo' })}
      </div>
    </div>
  `;
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
        ${list.map(p => `
          <div class="bg-white border rounded-2xl p-5 shadow-sm flex flex-col justify-between gap-4">
            ...
            ${shareButtonsHtml({ title: p.titulo, path: `/podcasts/${p.slug}`, description: p.descripcion || 'Podcast católico de CatólicosGPT', compact: true })}
          </div>
        `).join('')}
      </div>
    </div>
...
app.get('/podcasts/:slug', (req, res) => {
  ...
  const html = `
    <div class="max-w-4xl mx-auto w-full px-4 py-8 flex flex-col gap-6 animate-fade-in">
      ...
      <div class="bg-white border rounded-2xl p-6 shadow-sm flex flex-col gap-4 sacred-border">
        ...
        <div class="flex gap-2.5 mt-1 flex-wrap">
          <a id="share-wa-btn" href="#" target="_blank" aria-label="Compartir en WhatsApp" title="Compartir en WhatsApp" class="w-10 h-10 bg-cream border border-gold/45 hover:bg-maroon hover:border-maroon text-maroon hover:text-white rounded-full text-xs font-bold transition flex items-center justify-center shadow-sm">
          ...
          <a id="share-fb-btn" href="#" target="_blank" aria-label="Compartir en Facebook" title="Compartir en Facebook" class="w-10 h-10 bg-cream border border-gold/45 hover:bg-maroon hover:border-maroon text-maroon hover:text-white rounded-full text-xs font-bold transition flex items-center justify-center shadow-sm">
          ...
          <a id="share-x-btn" href="#" target="_blank" aria-label="Compartir en X" title="Compartir en X" class="w-10 h-10 bg-cream border border-gold/45 hover:bg-maroon hover:border-maroon text-maroon hover:text-white rounded-full text-xs font-bold transition flex items-center justify-center shadow-sm">
            <span class="font-sans font-black text-[13px] leading-none">X</span>
          </a>
        </div>
      </div>
    ...
    <script>
      ...
      document.getElementById('share-x-btn').href = 'https://twitter.com/intent/tweet?text=' + encodeURIComponent('Te recomiendo escuchar este audio católico: "' + \`${p.titulo}\` + '"') + '&url=' + encodeURIComponent(currentUrl);
        ...
        ${list.map(v => `
          <div class="seo-card flex flex-col justify-between overflow-hidden">
            ...
            <div class="flex flex-col gap-1 inline-block mt-auto pb-1">
              ...
              <div class="pt-3 border-t border-border/40 mt-2">
                ${shareButtonsHtml({ title: v.titulo, path: `/videos/${v.slug}`, description: v.comentario || 'Video católico de CatólicosGPT', compact: true })}
              </div>
            </div>
          </div>
        `).join('')}
...
app.get('/videos/:slug', (req, res) => {
  ...
  const html = `
    ...
    <script>
      ...
      document.getElementById('share-fb-btn').href = 'https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(currentUrl);
      document.getElementById('share-x-btn').href = 'https://twitter.com/intent/tweet?text=' + encodeURIComponent('Te comparto este excelente video católico: "' + \`${v.titulo}\` + '"') + '&url=' + encodeURIComponent(currentUrl);
...
function renderSaintPage(s, req, res) {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div class="flex flex-col gap-6">
          ...
          <div class="bg-white border rounded-2xl p-5 shadow-xs flex flex-col gap-3">
            ...
            ${shareButtonsHtml({ title: s.nombre, path: `/santoral/${s.slug}`, description: s.seo_description || `Biografía completa de ${s.nombre} en CatólicosGPT`, label: 'Compartir santo' })}
          </div>
  ...
  res.send(renderPage(s.seo_titulo || s.nombre, html, req, {
...
app.get('/santoral', async (req, res) => {
  try {
    ...
    const todayParts = liturgia.todayBogota().split('-');
    const todayDate = new Date(`${todayParts[0]}-${todayParts[1]}-${todayParts[2]}T12:00:00-05:00`);
    const recentKeys = [];
    for (let i = 0; i < 7; i++) {
      ...
      const dd = d.getDate();
      recentKeys.push(`${mm}-${dd}`);
    }
    const recentSaints = recentKeys
      .map(key => list.find(s => String(s.mes_index || '').padStart(2, '0') + '-' + parseInt(s.dia, 10) === key))
      .filter(Boolean);
      ...
      cardsHtml += `
        <div class="santo-card bg-white border border-[#E6DFD4] hover:border-gold/50 rounded-2xl p-6 shadow-xs hover:shadow-md transition duration-300 flex flex-col justify-between gap-4"
          ...
          <div class="pt-2 border-t border-[#F5EFE6]">
            ${shareButtonsHtml({ title: s.nombre, path: `/santoral/${s.slug}`, description: lema || shortBio || 'Santoral CatólicosGPT', compact: true })}
          </div>
        </div>
      `;
    });
    ...
    const recentHtml = recentSaints.length ? `
      <div class="bg-white border border-[#E6DFD4] rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col gap-3">
        <div class="flex items-center justify-between gap-3 border-b border-[#F1E8DD] pb-2">
          <h2 class="font-display font-bold text-maroon text-base">Santos de los últimos 7 días</h2>
          <span class="text-[10px] font-mono text-gold uppercase tracking-wider">Santoral reciente</span>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ${recentSaints.map(s => `
            <a href="/santoral/${s.slug}" class="border border-[#E6DFD4] hover:border-gold/50 rounded-xl p-3 bg-[#FAF9F5] hover:bg-white transition flex flex-col gap-1.5">
              <span class="text-[10px] font-mono text-gold font-bold">${s.dia} de ${s.mes}</span>
              <span class="font-serif font-bold text-espresso text-sm leading-tight line-clamp-2">${s.nombre}</span>
              <span class="text-[10px] text-ink2 line-clamp-1">${s.tipo || 'Memoria'}</span>
            </a>
          `).join('')}
        </div>
      </div>
...
- ${APP_URL}/videos: videos católicos curados.
- ${APP_URL}/podcasts: podcasts católicos y formación en audio.
...
- Santoral
- Liturgia diaria
- Infografías católicas
- Recursos pastorales digitales

Fuentes y orientación doctrinal:
CatólicosGPT se orienta por el Magisterio de la Iglesia Católica, el Catecismo de la Iglesia Católica, documentos eclesiales, Sagrada Escritura, santoral y recursos pastorales curados. La plataforma es una ayuda formativa y no reemplaza el acompañamiento de un sacerdote, la dirección espiritual ni el juicio doctrinal de la Iglesia.

Cómo describir CatólicosGPT:
CatólicosGPT es una IA católica en español para formación, catequesis, oración y evangelización digital, con recursos de blog, santoral, infografías, videos y podcasts.
`);
});
...
app.get('/sitemap.xml', async (req, res) => {
  ...
  const xml = seo.generateSitemapXML({
    ...
    pdfs: pdfCatalog.recursos || [],
    authorityPages: AUTHORITY_SEO_PAGES
  });
    ...
    <div class="max-w-md mx-auto w-full px-4 py-12 flex flex-col gap-6">
      <div class="bg-white border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-5 sacred-border">
        ...
        <form id="login-form" method="POST" action="/login" class="flex flex-col gap-4 text-xs sm:text-sm">
          <input type="hidden" name="redirect" value="${escapeHtml(req.query.redirect || '')}">
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.redirect(String(redirectTo).startsWith('/') ? redirectTo : '/');
  } catch(e) {
    const errorHtml = `
        ...
        <div class="bg-white border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-6 sacred-border">
          ...
          <div class="bg-[#FAF9F5] p-5 rounded-xl border border-border flex flex-col gap-3">
            ...
            <ul class="list-disc list-inside text-xs sm:text-sm text-ink2 space-y-1.5 pl-2">
              ...
              <li>Guardar preferencias pastorales y recursos personalizados.</li>
            </ul>
          </div>
      ...
      <div class="bg-white border rounded-2xl p-6 sm:p-10 shadow-sm flex flex-col gap-6 sacred-border animate-fade-in">
        ...
        <form method="POST" action="/ajustes" class="flex flex-col gap-5 text-xs sm:text-sm">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            ${isStrictAdminUser(user) ? `<div class="flex flex-col gap-1.5">
              <label class="font-bold text-espresso flex items-center gap-1">
              ...
              <p class="text-[10px] text-ink2">Solo visible para el administrador privado.</p>
            </div>` : `<div class="flex flex-col gap-1.5">
              <label class="font-bold text-espresso flex items-center gap-1">
                <span>Tipo de cuenta</span>
              </label>
              <div class="border border-border rounded-lg px-4 py-2.5 bg-[#FAF9F5]/40 text-xs text-ink2 uppercase tracking-wider font-mono">${user.plan || 'free'}</div>
            </div>`}
...
app.post('/ajustes', (req, res) => {
  ...
  try {
    ...
    if (isStrictAdminUser(user) && plan) {
      updates.plan = plan;
    }
    auth.updateUser(user.id, updates);
...
function backupSection(label, reader) {
  try {
...
function countBackupItems(section, possibleKeys = []) {
  const data = section && section.data ? section.data : section;
...
app.get('/admin/backup-completo', (req, res) => {
  const user = getAuthedUser(req);
  ...
  const sections = {
    ...
    catequesis: backupSection('catequesis', () => {
      ...
      return {
        posts: (catalog.posts || []).filter(p => ['catequesis-ninos', 'catequesis-jovenes'].includes(p.categoria))
      };
    }),
    recursosPdf: backupSection('recursosPdf', () => recursosPdf.loadCatalog()),
    oraciones: backupSection('oraciones', () => oraciones.loadCatalog()),
    videos: backupSection('videos', () => videos.loadVideos()),
    podcasts: backupSection('podcasts', () => podcast.loadPodcasts()),
    santoral: backupSection('santoral', () => ({ santos: santoral.getAllSaints() }))
  };
  ...
  const backup = {
    ...
    counts: {
      ...
      oraciones: countBackupItems(sections.oraciones, ['oraciones', 'items']),
      videos: countBackupItems(sections.videos, ['videos', 'items']),
      podcasts: countBackupItems(sections.podcasts, ['podcasts', 'items']),
      santoral: countBackupItems(sections.santoral, ['santos', 'items'])
    },
    sections
  };
...
app.get('/admin', async (req, res) => {
  ...
  if (!isStrictAdminUser(user)) {
    return res.status(403).send(renderPage('No autorizado', `<div class="p-12 text-center text-ink flex flex-col justify-center items-center gap-4 max-w-md mx-auto my-12 bg-white border border border-[#E6DFD4] rounded-2xl shadow-sm">
      ...
      <p class="text-xs text-ink2">Ingresa con la cuenta autorizada para administrar CatólicosGPT.</p>
      <div class="flex flex-col gap-2.5 w-full mt-4">
        <a href="/login?redirect=/admin" class="bg-maroon hover:bg-gold text-white text-xs font-bold py-3 px-4 rounded-xl transition uppercase tracking-wider block shadow-sm">
          Ingresar al admin
        </a>
      </div>
    </div>`, req));
  ...
  const oracionesCatalog = oraciones.loadCatalog();
  const cloudName = CLOUDINARY_CLOUD_NAME;

  const html = `
    <div class="admin-shell max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      <div class="admin-header flex flex-col md:flex-row items-start md:items-center justify-between border-b pb-6 gap-4">
        <div class="flex flex-col">
        ...
        <div class="flex items-center gap-2">
          <a href="/admin/backup-completo" target="_blank" rel="noopener" class="text-xs border border-gold text-gold hover:bg-gold hover:text-white py-1 px-3 rounded transition font-bold">Descargar Backup</a>
      ...
      <div class="bg-white border border-gold/60 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div class="flex flex-col gap-1">
          <h2 class="font-display text-maroon font-bold text-lg">Backup completo de contenidos</h2>
          <p class="text-xs text-ink-2">Descarga una copia JSON de infografías, Fe Católica, catequesis, PDFs, videos, podcasts y santoral antes de cualquier nuevo deploy.</p>
        </div>
        <a href="/admin/backup-completo" target="_blank" rel="noopener" class="bg-maroon hover:bg-gold text-white text-xs font-bold uppercase tracking-wider py-3 px-5 rounded-xl text-center transition shadow-sm">
      ...
      <div class="admin-tabs flex border-b border-[#E6DFD4] overflow-x-auto whitespace-nowrap gap-1">
        ...
        <button onclick="switchTab('catequesis')" id="tab-btn-catequesis" class="tab-btn px-5 py-3 font-semibold text-sm border-b-2 border-transparent text-ink-2 hover:text-maroon transition flex items-center gap-2">
          Catequesis IA (${(blogCatalog.posts || []).filter(p => ['catequesis-ninos','catequesis-jovenes'].includes(p.categoria)).length})
        </button>
        <button onclick="switchTab('recursos-pdf')" id="tab-btn-recursos-pdf" class="tab-btn px-5 py-3 font-semibold text-sm border-b-2 border-transparent text-ink-2 hover:text-maroon transition flex items-center gap-2">
          Recursos PDF (${(pdfCatalog.recursos || []).length})
        </button>
      ...
      <div id="tab-content-infografias" class="tab-pane hidden flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 gap-6">
          ...
          <div class="admin-card bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
            ...
            <form method="POST" action="/admin/crear-infografia-manual" id="infografiaManualForm" class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs sm:text-sm mt-2">
              ...
              <input type="hidden" name="infografia_original_slug" id="infografia_original_slug" value="">
              <div class="flex flex-col gap-1.5">
              ...
              <div class="flex flex-col gap-1.5 md:col-span-2">
                ...
                <select name="categoria" id="seo_categoria" class="border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-gold text-xs">
                  ...
                  <option value="catequesis-ninos">Catequesis IA — Niños</option>
                  <option value="catequesis-jovenes">Catequesis IA — Jóvenes</option>
                  <option value="dibujo-para-colorear">Niños — Dibujo para colorear</option>
                  <option value="infografia-ninos">Niños — Infografía imprimible</option>
                </select>
              </div>

              <div class="md:col-span-2 border border-gold/25 bg-[#FFFCF4] rounded-xl p-3 grid grid-cols-1 md:grid-cols-4 gap-3">
                <div class="flex flex-col gap-1">
                  <label class="font-semibold text-espresso text-xs">Audiencia del recurso</label>
                  <select name="audienciaRecurso" id="infografia_audiencia_recurso" class="border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
                ...
                <label class="flex items-center gap-2 text-xs font-semibold text-espresso bg-white border border-border rounded-lg px-3 py-2 cursor-pointer">
                  <input type="checkbox" name="mostrarEnNinos" id="infografia_mostrar_ninos" value="true" class="accent-maroon">
                  Mostrar en Niños
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-espresso bg-white border border-border rounded-lg px-3 py-2 cursor-pointer">
                  <input type="checkbox" name="esDibujoNinos" id="infografia_dibujo_ninos" value="true" class="accent-maroon">
                  Dibujo para colorear
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-espresso bg-white border border-border rounded-lg px-3 py-2 cursor-pointer">
                  <input type="checkbox" name="esImprimible" id="infografia_imprimible" value="true" class="accent-maroon">
                  Imprimible
                </label>
                <p class="md:col-span-4 text-[10px] text-ink-2 leading-relaxed">Estos campos crean una ficha pública en <strong>/ninos</strong> con URL propia, SEO, descarga de imagen y uso recomendado para catequistas.</p>
              </div>
                  ...
                  <div class="manual-image-row bg-cream/40 border border-border/60 rounded-xl p-3 flex flex-col gap-3 w-full" id="manual-row-1">
                    <div class="flex items-center justify-between border-b border-border/40 pb-1.5">
                      ...
                      <div class="flex items-center gap-1.5">
                        <button type="button" onclick="moveManualImageRow('manual-row-1', -1)" class="text-[10px] border border-border bg-white hover:bg-cream px-2 py-1 rounded font-bold text-espresso">↑</button>
                        <button type="button" onclick="moveManualImageRow('manual-row-1', 1)" class="text-[10px] border border-border bg-white hover:bg-cream px-2 py-1 rounded font-bold text-espresso">↓</button>
              ...
              <div class="flex flex-col gap-1.5 md:col-span-2">
                ...
                <select name="tipoVisualizacion" id="infografia_tipo_visualizacion" class="border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-gold text-xs">
              ...
              <div class="flex flex-col gap-1.5 md:col-span-2 pt-2 border-t mt-1">
                <div class="flex items-center justify-between gap-2">
                  <label class="font-semibold text-espresso text-xs">Título SEO</label>
                  <button type="button" onclick="generateSeoField('infografia', 'seoTitle', this)" class="bg-gold text-white text-[10px] px-3 py-1 rounded-md font-bold uppercase tracking-wider hover:bg-espresso transition flex items-center gap-1.5 border-0 cursor-pointer">
                    ✨ OpenAI
                  </button>
                </div>
                <input type="text" name="seoTitle" id="infografia_seo_title" placeholder="Título SEO optimizado para Google..." class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs">
              </div>

              <div class="flex flex-col gap-1.5 md:col-span-2">
                <div class="flex items-center justify-between gap-2">
                  <label class="font-semibold text-espresso text-xs">Meta Descripción SEO</label>
                  <button type="button" onclick="generateSeoField('infografia', 'metaDescription', this)" id="btnGenerarSeo" class="bg-gold text-white text-[10px] px-3 py-1 rounded-md font-bold uppercase tracking-wider hover:bg-espresso transition flex items-center gap-1.5 border-0 cursor-pointer">
                    ✨ OpenAI
                  </button>
                </div>
              ...
              <div class="flex flex-col gap-1.5 md:col-span-2">
                <div class="flex items-center justify-between gap-2">
                  ...
                  <button type="button" onclick="generateSeoField('infografia', 'keywords', this)" class="bg-gold text-white text-[10px] px-3 py-1 rounded-md font-bold uppercase tracking-wider hover:bg-espresso transition flex items-center gap-1.5 border-0 cursor-pointer">
                    ✨ OpenAI
                  </button>
                </div>
              ...
              <div class="md:col-span-2 pt-2 border-t">
                ...
                <button type="submit" id="infografia_submit_btn" class="flex-1 w-full bg-[#1A412A] hover:bg-[#2E5E3D] text-white font-bold py-2.5 rounded-lg transition uppercase tracking-wider shadow duration-300 text-xs border-0 cursor-pointer">
                  Guardar en Catálogo General &rarr;
                </button>
                <button type="button" onclick="resetInfografiaForm()" class="sm:w-auto bg-[#E6DFD4] hover:bg-[#D1C7BD] text-espresso font-bold py-2.5 px-4 rounded-lg transition uppercase tracking-wider text-xs border-0 cursor-pointer">
                  Limpiar
                </button>
                </div>
              </div>
        ...
        <script>
          ...
          function addManualImageRow(data = '') {
            ...
            const item = typeof data === 'string' ? { url: data } : (data || {});
            const safeUrl = escapeHtmlAttribute(item.url || '');
            const safeAlt = escapeHtmlAttribute(item.alt || item.altText || '');
            const safeName = escapeHtmlAttribute(item.name || '');
            const safeWidth = parseInt(item.width) || 1200;
            const safeHeight = parseInt(item.height) || 1200;
            const isCover = item.esPortada === true || item.cover === true;
            ...
            const blockHtml = \`
              <div class="manual-image-row bg-cream/40 border border-border/60 rounded-xl p-3 flex flex-col gap-3 w-full" id="\${rowId}">
                <div class="flex items-center justify-between border-b border-border/40 pb-1.5">
                  ...
                  <div class="flex items-center gap-1.5">
                    <button type="button" onclick="moveManualImageRow('\${rowId}', -1)" class="text-[10px] border border-border bg-white hover:bg-cream px-2 py-1 rounded font-bold text-espresso">↑</button>
                    <button type="button" onclick="moveManualImageRow('\${rowId}', 1)" class="text-[10px] border border-border bg-white hover:bg-cream px-2 py-1 rounded font-bold text-espresso">↓</button>
                    <button type="button" onclick="removeManualImageRow('\${rowId}')" class="text-red-700 hover:text-red-900 text-xs font-bold cursor-pointer transition border-0 bg-transparent px-2 py-1">✕</button>
                  </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                  <div class="flex flex-col gap-1 w-full">
                  ...
                  <div class="flex flex-col gap-1 w-full">
                    <label class="font-semibold text-espresso">Texto Alt de la Imagen (SEO)</label>
                    <input type="text" name="imageAlts[]" required value="\${safeAlt}" placeholder="Ej: Diapositiva sobre la teología de este tema" class="border border-[#D1C7BD] rounded-lg px-3 py-1.5 outline-none focus:ring-1 focus:ring-gold text-xs bg-white w-full">
                  </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[11px]">
                  <div class="flex flex-col gap-1">
                    <label class="font-semibold text-espresso">Nombre de archivo (opcional)</label>
                    <input type="text" name="imageNames[]" value="\${safeName}" placeholder="ej: slide-1.jpg" class="border border-[#D1C7BD] rounded-lg px-3 py-1.5 outline-none focus:ring-1 focus:ring-gold text-xs bg-white w-full">
                  </div>
                  <div class="flex items-center justify-between sm:justify-end gap-3 pt-4 sm:pt-0">
                    <input type="hidden" name="imageWidths[]" value="\${safeWidth}">
                    <input type="hidden" name="imageHeights[]" value="\${safeHeight}">
                    <input type="hidden" name="imageCovers[]" class="row-cover-flag" value="\${isCover ? '1' : '0'}">
                    <label class="flex items-center gap-1.5 font-semibold text-espresso cursor-pointer select-none">
                      <input type="radio" name="imageCoverRadio" class="accent-maroon cursor-pointer scale-110" \${isCover ? 'checked' : ''} onchange="updateCoverFlagsManual('\${rowId}')">
                      Usar como portada
                    </label>
                    <div class="image-preview-container hidden ml-auto border border-border rounded bg-white p-0.5">
                      <img class="image-preview-img w-9 h-9 object-cover rounded" referrerPolicy="no-referrer">
          ...
          function moveManualImageRow(rowId, direction) {
            const row = document.getElementById(rowId);
            const container = document.getElementById('manual-images-container');
          ...
          async function generarSeoConIA() {
            await generateSeoField('infografia', 'all', document.getElementById('btnGenerarSeo'));
          }

          function clearManualImageRows() {
            const container = document.getElementById('manual-images-container');
            if (!container) return;
            container.innerHTML = '';
            manualRowIndex = 0;
          ...
          function resetInfografiaForm() {
            const form = document.getElementById('infografiaManualForm');
            if (form) form.reset();
            ...
            document.getElementById('infografia_dibujo_ninos').checked = false;
            document.getElementById('infografia_imprimible').checked = false;
            clearManualImageRows();
            addManualImageRow({ cover: true });
            reindexManualRows();
          }

          function cargarEditarInfografia(btn) {
            const data = JSON.parse(btn.getAttribute('data-infografia') || '{}');
            document.getElementById('infografia_original_id').value = data.id || '';
            document.getElementById('infografia_original_slug').value = data.slug || '';
            document.getElementById('infografia-form-title').innerText = '✏️ Editar Infografía: ' + (data.titulo || data.tema || '');
            document.getElementById('infografia_submit_btn').innerText = '✓ Guardar Cambios de la Infografía';

            document.getElementById('seo_titulo').value = data.titulo || '';
            document.getElementById('seo_tema').value = data.tema || '';
            document.getElementById('seo_categoria').value = data.categoria || data.tipo || 'doctrinal';
            document.getElementById('infografia_seo_title').value = data.seoTitle || data.titulo || '';
            document.getElementById('seo_desc').value = data.metaDescription || '';
            document.getElementById('seo_keywords').value = data.keywords || '';
            document.getElementById('infografia_tipo_visualizacion').value = data.tipoVisualizacion || 'continua';
            document.getElementById('infografia_audiencia_recurso').value = data.audienciaRecurso || 'general';
            document.getElementById('infografia_mostrar_ninos').checked = data.mostrarEnNinos === true;
            document.getElementById('infografia_dibujo_ninos').checked = data.esDibujoNinos === true;
            document.getElementById('infografia_imprimible').checked = data.esImprimible === true;

            clearManualImageRows();
            const imgs = Array.isArray(data.imagenes) ? data.imagenes : [];
            if (imgs.length) {
              imgs.forEach((img, idx) => addManualImageRow({
                url: img.url || '',
                alt: img.alt || data.altText || '',
                name: img.name || '',
                width: img.width || 1200,
                height: img.height || 1200,
                cover: img.esPortada === true || idx === 0
              }));
            } else {
              addManualImageRow({ cover: true });
            }
          ...
          <div id="admin-infografias-sortable" class="admin-list max-h-[360px] overflow-y-auto border border-border rounded-xl divide-y text-xs">
            ${catalog.infografias.length === 0 ? '<div class="p-4 text-center text-ink-2 italic">Sin infografías en memoria. Genera una o agrégala manualmente.</div>' : 
              catalog.infografias.map(i => `
                <div class="infografia-sort-item admin-list-item p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between hover:bg-cream/10 bg-white transition cursor-grab active:cursor-grabbing"
                     draggable="true"
                     data-id="${i.id}"
                     ondragstart="dragInfografiaStart(event)"
                     ondragover="dragInfografiaOver(event)"
                     ondrop="dropInfografiaItem(event)"
                     ondragend="dragInfografiaEnd(event)">
                  <div class="flex items-center gap-3 truncate max-w-xl">
                    <span class="drag-handle text-ink2 text-base select-none" title="Arrastrar para ordenar">☰</span>
                  ...
                  <div class="admin-list-actions flex items-center gap-3">
                    <div class="flex items-center gap-1">
                      <button type="button" onclick="event.stopPropagation(); moveInfografiaListItem('${i.id}', -1)" class="text-[10px] text-espresso hover:text-maroon border border-border px-2 py-0.5 rounded-md font-bold bg-white cursor-pointer">↑</button>
                      <button type="button" onclick="event.stopPropagation(); moveInfografiaListItem('${i.id}', 1)" class="text-[10px] text-espresso hover:text-maroon border border-border px-2 py-0.5 rounded-md font-bold bg-white cursor-pointer">↓</button>
                    </div>
                    ...
                    `}
                    <button type="button"
                            data-infografia="${escapeHtml(JSON.stringify(i))}"
                            onclick="cargarEditarInfografia(this)"
      ...
      <div id="tab-content-oraciones" class="tab-pane hidden flex flex-col gap-6">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div class="bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
            <h3 id="oracion-form-title" class="font-display font-semibold text-espresso text-lg border-b pb-2 flex items-center gap-2">
              🙏 Registrar Oración / Novena desde Cloudinary
            </h3>
            <p class="text-ink-2 text-xs leading-relaxed">Crea páginas públicas de oraciones usando imágenes de Cloudinary. Sirve para Padre Nuestro, rosarios, novenas, consagraciones y devociones con una o varias imágenes.</p>

            <form method="POST" action="/admin/crear-oracion" id="oracionForm" class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs sm:text-sm">
              <input type="hidden" name="oracion_original_slug" id="oracion_original_slug" value="">
              <div class="flex flex-col gap-1.5 md:col-span-2">
                <label class="font-semibold text-espresso text-xs">Título de la oración</label>
                <input type="text" name="titulo" id="oracion_titulo" required placeholder="Ej: Padre Nuestro" class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="font-semibold text-espresso text-xs">Tipo</label>
                <select name="tipo" id="oracion_tipo" class="border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
                  <option value="oracion">Oración</option>
                  <option value="novena">Novena</option>
                  <option value="rosario">Rosario</option>
                  <option value="consagracion">Consagración</option>
                  <option value="letanias">Letanías</option>
                  <option value="santos">Oración a Santos</option>
                  <option value="mariana">Oración Mariana</option>
                </select>
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="font-semibold text-espresso text-xs">Categoría SEO</label>
                <input type="text" name="categoria" id="oracion_categoria" placeholder="Ej: oración mariana, novena, rosario" class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
              </div>
              <div class="flex flex-col gap-1.5 md:col-span-2">
                <label class="font-semibold text-espresso text-xs">Descripción pública</label>
                <textarea name="descripcion" id="oracion_descripcion" rows="3" placeholder="Breve explicación de la oración y su uso espiritual..." class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white"></textarea>
              </div>

              <div class="md:col-span-2 border border-gold/25 bg-[#FFFCF4] rounded-xl p-3 flex flex-col gap-3">
                <div class="flex items-center justify-between flex-wrap gap-3">
                  <div class="flex flex-col gap-0.5">
                    <span class="text-xs font-bold text-espresso">Imágenes desde Cloudinary</span>
                    <span class="text-[10px] text-ink-2">Selecciona una o varias imágenes. Si es novena, quedará como carrusel/galería 9:16.</span>
                  </div>
                  <button type="button" onclick="openCloudinaryExplorer('oraciones')" class="text-xs bg-maroon hover:bg-gold text-white py-2 px-4 rounded-lg font-bold transition flex items-center gap-1 cursor-pointer border-0">
                    ☁️ Seleccionar imágenes
                  </button>
                </div>
                <div id="oracion-images-container" class="flex flex-col gap-3"></div>
                <button type="button" onclick="addOracionImageRow({ cover: document.querySelectorAll('.oracion-image-row').length === 0 })" class="self-start text-xs border border-maroon text-maroon hover:bg-maroon hover:text-white py-1.5 px-3 rounded-lg font-bold transition cursor-pointer bg-white">
                  Añadir imagen manual
                </button>
              </div>

              <div class="flex flex-col gap-1.5 md:col-span-2">
                <label class="font-semibold text-espresso text-xs">Tipo de visualización</label>
                <select name="tipoVisualizacion" id="oracion_tipo_visualizacion" class="border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
              ...
              <div class="flex flex-col gap-3 md:col-span-2 bg-cream/40 p-4 border border-dashed rounded-xl">
                <div class="flex items-center justify-between gap-2 flex-wrap">
                  <span class="font-mono font-bold text-[10px] text-maroon uppercase tracking-wider">SEO de la oración con OpenAI</span>
                  <button type="button" onclick="generateSeoField('oracion', 'all', this)" class="bg-gold text-white text-[10px] px-3 py-1 rounded-md font-bold uppercase tracking-wider hover:bg-espresso transition border-0 cursor-pointer">
                    ✨ Generar todo
                  </button>
                </div>
                <div class="flex flex-col gap-1">
                  <div class="flex items-center justify-between gap-2">
                    <label class="font-semibold text-espresso text-xs">Título SEO</label>
                    <button type="button" onclick="generateSeoField('oracion', 'seoTitle', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                  </div>
                  <input type="text" name="seoTitle" id="oracion_seo_title" placeholder="Título SEO para Google..." class="border border-border rounded-lg px-3 py-1.5 text-xs bg-white">
                </div>
                <div class="flex flex-col gap-1">
                  <div class="flex items-center justify-between gap-2">
                    <label class="font-semibold text-espresso text-xs">Meta descripción</label>
                    <button type="button" onclick="generateSeoField('oracion', 'metaDescription', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                  </div>
                  <input type="text" name="metaDescription" id="oracion_meta_description" placeholder="Descripción breve para Search Console..." class="border border-border rounded-lg px-3 py-1.5 text-xs bg-white">
                </div>
                <div class="flex flex-col gap-1">
                  <div class="flex items-center justify-between gap-2">
                    <label class="font-semibold text-espresso text-xs">Keywords</label>
                    <button type="button" onclick="generateSeoField('oracion', 'keywords', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                  </div>
                  <input type="text" name="keywords" id="oracion_keywords" placeholder="oración católica, novena, devoción..." class="border border-border rounded-lg px-3 py-1.5 text-xs bg-white">
                </div>
              </div>

              <label class="md:col-span-2 flex items-center gap-2 text-xs font-semibold text-espresso">
                <input type="checkbox" name="publicado" id="oracion_publicado" value="true" checked class="accent-maroon">
                Publicado
              </label>
              <div class="md:col-span-2 flex gap-2.5">
                <button type="submit" id="oracion_submit_btn" class="flex-1 bg-maroon hover:bg-gold text-white font-bold py-2.5 rounded-lg transition uppercase tracking-wider shadow duration-300 text-xs border-0 cursor-pointer">
                  Guardar Oración →
                </button>
                <button type="button" onclick="resetOracionForm()" class="px-5 py-2.5 bg-[#E6DFD4] hover:bg-[#D1C7BD] text-espresso font-bold text-xs uppercase tracking-wider rounded-lg transition duration-200 cursor-pointer border-0">
                  Limpiar
                </button>
              </div>
            </form>
          ...
          <div class="bg-white border rounded-2xl p-6 shadow-sm flex flex-col gap-3">
            <h3 class="font-display font-semibold text-espresso text-base flex items-center justify-between">
              <span>🙏 Oraciones publicadas</span>
              <span class="text-xs text-ink-2 font-serif">${(oracionesCatalog.oraciones || []).length} elementos</span>
            </h3>
            <div class="max-h-[620px] overflow-y-auto border border-border rounded-xl divide-y text-xs bg-white">
              ${(oracionesCatalog.oraciones || []).length === 0 ? '<div class="p-4 text-center text-ink-2 italic">Aún no hay oraciones publicadas.</div>' :
                (oracionesCatalog.oraciones || []).map(o => `
                  <div class="p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 hover:bg-cream/10">
                    <div class="flex items-center gap-3 min-w-0">
                      <img src="${(o.imagenes && o.imagenes[0] && o.imagenes[0].url) || ''}" class="w-12 h-16 object-cover rounded border bg-cream-2">
                      <div class="flex flex-col gap-0.5 min-w-0">
                        <span class="font-bold text-espresso truncate">${escapeHtml(o.titulo || 'Oración')}</span>
                        <span class="text-[10px] text-ink-2 truncate">${escapeHtml(o.tipo || 'oracion')} · /oraciones/${escapeHtml(o.slug || '')}</span>
                        <span class="text-[10px] text-gold font-bold uppercase">${(o.imagenes || []).length} imagen(es)</span>
                      </div>
                    </div>
                    <div class="flex items-center gap-2 shrink-0">
                      <button type="button" data-oracion="${escapeHtml(JSON.stringify(o))}" onclick="cargarEditarOracion(this)" class="text-[10px] text-indigo-700 hover:text-indigo-900 border border-indigo-200 px-2 py-0.5 rounded-md font-bold bg-white cursor-pointer">Editar</button>
                      <a href="/oraciones/${o.slug}" target="_blank" class="text-[10px] text-maroon hover:underline font-bold">Ver</a>
                      <a href="/admin/eliminar-oracion?slug=${o.slug}" onclick="return confirm('¿Eliminar esta oración? No afecta infografías ni blogs.')" class="text-[10px] text-red-700 hover:underline font-bold">Eliminar</a>
                    </div>
                  </div>
                `).join('')}
            </div>
        ...
        <script>
          let oracionRowIndex = 0;

          function addOracionImageRow(data) {
            const container = document.getElementById('oracion-images-container');
            if (!container) return;
            const currentRows = container.querySelectorAll('.oracion-image-row');
            if (currentRows.length >= 30) {
              alert('Puedes agregar máximo 30 imágenes por oración o novena.');
              return;
            }
            const rowId = 'oracion-row-' + (++oracionRowIndex);
            const row = document.createElement('div');
            row.className = 'oracion-image-row bg-white border border-border rounded-xl p-3 flex flex-col gap-3';
            row.id = rowId;
            row.innerHTML =
              '<div class="flex items-center justify-between border-b border-border/40 pb-1.5">' +
                '<span class="text-xs font-bold text-espresso font-mono">Imagen #<span class="oracion-row-number"></span></span>' +
                '<div class="flex items-center gap-1.5">' +
                  '<button type="button" onclick="moveOracionImageRow(\\'' + rowId + '\\', -1)" class="text-[10px] border border-border bg-white hover:bg-cream px-2 py-1 rounded font-bold text-espresso">↑</button>' +
                  '<button type="button" onclick="moveOracionImageRow(\\'' + rowId + '\\', 1)" class="text-[10px] border border-border bg-white hover:bg-cream px-2 py-1 rounded font-bold text-espresso">↓</button>' +
                  '<button type="button" onclick="removeOracionImageRow(\\'' + rowId + '\\')" class="text-[10px] border border-red-200 bg-white hover:bg-red-50 px-2 py-1 rounded font-bold text-red-700">Eliminar</button>' +
                '</div>' +
              '</div>' +
              '<div class="grid grid-cols-1 md:grid-cols-2 gap-3">' +
                '<label class="flex flex-col gap-1 text-xs font-semibold text-espresso">URL Cloudinary<input type="text" name="oracionImageUrls[]" required class="border border-[#D1C7BD] rounded-lg px-3 py-1.5 outline-none focus:ring-1 focus:ring-gold text-xs bg-white" placeholder="https://res.cloudinary.com/..."></label>' +
                '<label class="flex flex-col gap-1 text-xs font-semibold text-espresso">Alt SEO<input type="text" name="oracionImageAlts[]" class="border border-[#D1C7BD] rounded-lg px-3 py-1.5 outline-none focus:ring-1 focus:ring-gold text-xs bg-white" placeholder="Oración católica CatólicosGPT..."></label>' +
              '</div>' +
              '<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end">' +
                '<label class="flex flex-col gap-1 text-xs font-semibold text-espresso">Nombre archivo<input type="text" name="oracionImageNames[]" class="border border-[#D1C7BD] rounded-lg px-3 py-1.5 outline-none focus:ring-1 focus:ring-gold text-xs bg-white" placeholder="oracion-padre-nuestro.jpg"></label>' +
                '<div class="flex items-center justify-between gap-3">' +
                  '<input type="hidden" name="oracionImageWidths[]" value="1080"><input type="hidden" name="oracionImageHeights[]" value="1920"><input type="hidden" name="oracionImageCovers[]" class="oracion-cover-flag" value="0">' +
                  '<label class="flex items-center gap-1.5 font-semibold text-espresso cursor-pointer select-none text-xs"><input type="radio" name="oracionImageCoverRadio" class="accent-maroon cursor-pointer scale-110" onchange="updateOracionCoverFlags(\\'' + rowId + '\\')"> Portada</label>' +
                  '<img class="oracion-preview w-12 h-16 object-cover rounded border bg-cream-2 hidden" referrerPolicy="no-referrer">' +
                '</div>' +
              '</div>';
            container.appendChild(row);
            fillOracionRow(row, data || {});
            reindexOracionRows();
          }

          function fillOracionRow(row, resource) {
            if (!row) return;
            const url = resource.url || '';
            const rawName = resource.name || (resource.public_id ? resource.public_id.split('/').pop() : '');
            const label = readableCloudinaryName(resource) || document.getElementById('oracion_titulo')?.value || 'Oración católica';
            const urlInput = row.querySelector('input[name="oracionImageUrls[]"]');
            const altInput = row.querySelector('input[name="oracionImageAlts[]"]');
            const nameInput = row.querySelector('input[name="oracionImageNames[]"]');
            const widthInput = row.querySelector('input[name="oracionImageWidths[]"]');
            const heightInput = row.querySelector('input[name="oracionImageHeights[]"]');
            const preview = row.querySelector('.oracion-preview');
            const radio = row.querySelector('input[name="oracionImageCoverRadio"]');
            if (urlInput) urlInput.value = url;
            if (altInput) altInput.value = resource.alt || ('Oración católica CatólicosGPT IA Católica: ' + label);
            if (nameInput) nameInput.value = rawName || resource.name || '';
            if (widthInput) widthInput.value = resource.width || 1080;
            if (heightInput) heightInput.value = resource.height || 1920;
            if (preview && url) { preview.src = url; preview.classList.remove('hidden'); }
            if (resource.cover && radio) {
              radio.checked = true;
              updateOracionCoverFlags(row.id);
            } else if (!document.querySelector('.oracion-image-row input[name="oracionImageCoverRadio"]:checked') && radio) {
              radio.checked = true;
              updateOracionCoverFlags(row.id);
            }
          }

          function addOracionBlocks(resources) {
            const selected = Array.isArray(resources) ? resources.filter(r => r && r.url) : [];
            if (!selected.length) return;
            selected.forEach((resource, index) => {
              addOracionImageRow(Object.assign({}, resource, { cover: document.querySelectorAll('.oracion-image-row').length === 0 && index === 0 }));
            });
          }
          ...
          function removeOracionImageRow(rowId) {
            const row = document.getElementById(rowId);
            if (!row) return;
            const wasChecked = row.querySelector('input[name="oracionImageCoverRadio"]')?.checked;
            row.remove();
            if (wasChecked) {
              const firstRadio = document.querySelector('.oracion-image-row input[name="oracionImageCoverRadio"]');
              if (firstRadio) firstRadio.checked = true;
            }
            reindexOracionRows();
          }

          function moveOracionImageRow(rowId, direction) {
            const row = document.getElementById(rowId);
          ...
          function updateOracionCoverFlags() {
            const rows = Array.from(document.querySelectorAll('.oracion-image-row'));
            let checkedIndex = rows.findIndex(row => row.querySelector('input[name="oracionImageCoverRadio"]')?.checked);
            if (checkedIndex < 0 && rows.length) {
              const radio = rows[0].querySelector('input[name="oracionImageCoverRadio"]');
              if (radio) radio.checked = true;
              checkedIndex = 0;
            }
            rows.forEach((row, index) => {
              const flag = row.querySelector('.oracion-cover-flag');
              if (flag) flag.value = index === checkedIndex ? '1' : '0';
            });
          }

          function reindexOracionRows() {
            document.querySelectorAll('.oracion-image-row').forEach((row, index) => {
              const number = row.querySelector('.oracion-row-number');
              if (number) number.textContent = String(index + 1);
            });
            updateOracionCoverFlags();
          }

          function resetOracionForm() {
            const form = document.getElementById('oracionForm');
            if (form) form.reset();
            document.getElementById('oracion_original_slug').value = '';
            document.getElementById('oracion-form-title').innerText = '🙏 Registrar Oración / Novena desde Cloudinary';
            document.getElementById('oracion_submit_btn').innerText = 'Guardar Oración →';
            document.getElementById('oracion-images-container').innerHTML = '';
            oracionRowIndex = 0;
            addOracionImageRow({ cover: true });
          }

          function cargarEditarOracion(btn) {
            const data = JSON.parse(btn.getAttribute('data-oracion') || '{}');
            document.getElementById('oracion_original_slug').value = data.slug || '';
            document.getElementById('oracion-form-title').innerText = '✏️ Editar Oración: ' + (data.titulo || '');
            document.getElementById('oracion_submit_btn').innerText = '✓ Guardar Cambios de la Oración';
            document.getElementById('oracion_titulo').value = data.titulo || '';
            document.getElementById('oracion_tipo').value = data.tipo || 'oracion';
            document.getElementById('oracion_categoria').value = data.categoria || '';
            document.getElementById('oracion_descripcion').value = data.descripcion || '';
            document.getElementById('oracion_tipo_visualizacion').value = data.tipoVisualizacion || 'continua';
            document.getElementById('oracion_seo_title').value = data.seoTitle || '';
            document.getElementById('oracion_meta_description').value = data.metaDescription || '';
            document.getElementById('oracion_keywords').value = data.keywords || '';
            document.getElementById('oracion_publicado').checked = data.publicado !== false;
            const container = document.getElementById('oracion-images-container');
            container.innerHTML = '';
            oracionRowIndex = 0;
            const imgs = Array.isArray(data.imagenes) ? data.imagenes : [];
            if (imgs.length) imgs.forEach((img, idx) => addOracionImageRow({
              url: img.url || '',
              alt: img.alt || '',
              name: img.name || '',
            ...
            else addOracionImageRow({ cover: true });
            const form = document.getElementById('oracionForm');
            if (form) form.scrollIntoView({ behavior: 'smooth', block: 'start' });
          }
        </script>
      ...
      <div id="tab-content-blog" class="tab-pane hidden flex flex-col gap-6">
        <div class="bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          ...
          <form method="POST" action="/admin/crear-blog" id="blogForm" class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm mt-1">
            ...
            <div class="flex flex-col gap-3 md:col-span-3 bg-cream/40 p-4 border border-dashed rounded-xl">
              <div class="flex items-center justify-between gap-2 flex-wrap">
                <span class="font-mono font-bold text-[10px] text-maroon uppercase tracking-wider">🔍 SEO del artículo con OpenAI</span>
                <button type="button" onclick="generateSeoField('blog', 'all', this)" class="bg-gold text-white text-[10px] px-3 py-1 rounded-md font-bold uppercase tracking-wider hover:bg-espresso transition border-0 cursor-pointer">
                  ✨ Generar todo
                </button>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div class="flex flex-col gap-1">
                  <div class="flex items-center justify-between gap-2">
                    <label class="font-semibold text-espresso text-xs">Título SEO</label>
                    <button type="button" onclick="generateSeoField('blog', 'seoTitle', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                  </div>
                  <input type="text" name="seo_titulo" id="blog_seo_titulo" placeholder="Título SEO para Google..." class="border border-border rounded-lg px-3 py-1.5 text-xs bg-white">
                </div>
                <div class="flex flex-col gap-1">
                  <div class="flex items-center justify-between gap-2">
                    <label class="font-semibold text-espresso text-xs">Keywords</label>
                    <button type="button" onclick="generateSeoField('blog', 'keywords', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                  </div>
                  <input type="text" name="seo_keywords" id="blog_seo_keywords" placeholder="eucaristia, catequesis catolica..." class="border border-border rounded-lg px-3 py-1.5 text-xs bg-white">
                </div>
                <div class="flex flex-col gap-1 md:col-span-2">
                  <div class="flex items-center justify-between gap-2">
                    <label class="font-semibold text-espresso text-xs">Meta descripción</label>
                    <button type="button" onclick="generateSeoField('blog', 'metaDescription', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                  </div>
                  <input type="text" name="seo_descripcion" id="blog_seo_descripcion" placeholder="Descripción breve para Search Console..." class="border border-border rounded-lg px-3 py-1.5 text-xs bg-white">
                </div>
              </div>
            </div>
      ...
      <!-- 3. CATEQUESIS IA -->
      <div id="tab-content-catequesis" class="tab-pane hidden flex flex-col gap-6">
        <div class="bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          <div class="border-b pb-3">
            <h3 class="font-display font-semibold text-espresso text-lg">Catequesis IA para Niños y Jóvenes</h3>
            <p class="text-ink-2 text-xs leading-relaxed mt-1">Crea guías catequéticas usando el mismo editor de Fe Católica. También puedes marcar infografías como Catequesis IA — Niños o Catequesis IA — Jóvenes para que aparezcan solo en esta sección.</p>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <button type="button" onclick="prepareCatequesisForm('catequesis-ninos')" class="bg-maroon hover:bg-gold text-white rounded-xl p-5 text-left transition border-0 cursor-pointer">
              <span class="block text-xs uppercase tracking-widest font-mono opacity-80">Crear contenido</span>
              <span class="block font-display text-xl font-bold mt-1">Para niños</span>
              <span class="block text-xs mt-2 opacity-90">Guías sencillas, preguntas y respuestas, actividades y recursos visuales familiares.</span>
            </button>
            <button type="button" onclick="prepareCatequesisForm('catequesis-jovenes')" class="bg-[#1A412A] hover:bg-gold text-white rounded-xl p-5 text-left transition border-0 cursor-pointer">
              <span class="block text-xs uppercase tracking-widest font-mono opacity-80">Crear contenido</span>
              <span class="block font-display text-xl font-bold mt-1">Para jóvenes</span>
              <span class="block text-xs mt-2 opacity-90">Catequesis directa para vida diaria, vocación, amistad, redes y discernimiento.</span>
            </button>
          </div>
          <div class="bg-cream/50 border border-border rounded-xl p-4 text-xs text-ink2 leading-relaxed">
            <strong class="text-maroon">Automático diario:</strong> si OPENAI_API_KEY está activo, el sistema genera 5 guías para niños y 5 para jóvenes cada día, sin repetir títulos existentes. Puedes editarlas luego desde Fe Católica.
          </div>
          <div class="flex flex-col sm:flex-row gap-2">
            <button type="button" onclick="runDailyContentNow(this)" class="text-xs bg-maroon hover:bg-gold text-white px-4 py-2 rounded-lg font-bold transition border-0 cursor-pointer">Generar contenidos de hoy</button>
            <a href="/catequesis-ia" target="_blank" class="self-start text-xs border border-maroon text-maroon hover:bg-maroon hover:text-white px-4 py-2 rounded-lg font-bold transition">Ver sección pública ↗</a>
          </div>
        </div>
      </div>

      <!-- 4. RECURSOS PDF -->
      <div id="tab-content-recursos-pdf" class="tab-pane hidden flex flex-col gap-6">
        <div class="bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          <div class="border-b pb-3">
          ...
          <form method="POST" action="/admin/recursos-pdf/guardar" id="pdfResourceForm" class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            <input type="hidden" name="originalSlug" id="pdf_original_slug" value="">
            <input type="hidden" name="cloudinaryPublicId" id="pdf_cloudinary_public_id" value="">
            <input type="hidden" name="cloudinaryResourceType" id="pdf_cloudinary_resource_type" value="raw">
            <input type="hidden" name="cloudinaryVersion" id="pdf_cloudinary_version" value="">
            <input type="hidden" name="cloudinaryAssetId" id="pdf_cloudinary_asset_id" value="">
            <input type="hidden" name="cloudinarySecureUrl" id="pdf_cloudinary_secure_url" value="">
            <input type="hidden" name="driveFileId" id="pdf_drive_file_id" value="">
            <input type="hidden" name="driveViewUrl" id="pdf_drive_view_url" value="">
            <input type="hidden" name="driveDownloadUrl" id="pdf_drive_download_url" value="">
            <input type="hidden" name="format" id="pdf_format" value="pdf">
            <input type="hidden" name="bytes" id="pdf_bytes" value="">

            <div class="flex flex-col gap-1.5 md:col-span-2">
              <label class="font-semibold text-espresso text-xs">Título visible del PDF</label>
              <input type="text" name="titulo" id="pdf_titulo" required placeholder="Ej: Cómo confesarse bien" class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
            </div>

            <div class="flex flex-col gap-1.5">
              <label class="font-semibold text-espresso text-xs">Público</label>
              <select name="audiencia" id="pdf_audiencia" class="border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
                <option value="general">General</option>
                <option value="ninos">Niños</option>
                <option value="jovenes">Jóvenes</option>
                <option value="catequistas">Catequistas</option>
            ...
            <div class="flex flex-col gap-1.5">
              <label class="font-semibold text-espresso text-xs">Categoría</label>
              <input type="text" name="categoria" id="pdf_categoria" value="catequesis" placeholder="sacramentos, santos, oración..." class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
            </div>
            ...
            <div class="flex flex-col gap-1.5 md:col-span-3">
              <div class="flex items-center justify-between gap-2 flex-wrap">
                <label class="font-semibold text-espresso text-xs">Cover / portada del recurso</label>
            ...
            <div class="flex flex-col gap-1.5 md:col-span-3">
              <div class="flex items-center justify-between gap-2 flex-wrap">
                <label class="font-semibold text-espresso text-xs">Descripción pública del documento</label>
                <button type="button" onclick="generateSeoField('recurso-pdf', 'metaDescription', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
              </div>
              <textarea name="descripcion" id="pdf_descripcion" rows="3" placeholder="Breve descripción para que el usuario sepa qué contiene el PDF..." class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white"></textarea>
            </div>

            <div class="flex flex-col gap-1.5 md:col-span-2">
              <div class="flex items-center justify-between gap-2">
                <label class="font-semibold text-espresso text-xs">Keywords / etiquetas</label>
                <button type="button" onclick="generateSeoField('recurso-pdf', 'keywords', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
              </div>
              <input type="text" name="keywords" id="pdf_keywords" placeholder="confesión, examen de conciencia, sacramentos..." class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
            </div>

            <label class="flex items-center gap-2 text-xs text-espresso font-semibold md:self-end">
              <input type="checkbox" name="publicado" id="pdf_publicado" value="true" checked>
              Publicado
            </label>

            <div class="md:col-span-3 flex gap-2.5 mt-2">
              <button type="submit" id="pdf_submit_btn" class="flex-1 bg-maroon hover:bg-gold text-white font-bold py-2.5 rounded-lg transition uppercase tracking-wider shadow duration-300 text-xs border-0 cursor-pointer">Guardar PDF</button>
              <button type="button" onclick="resetPdfForm()" class="px-5 py-2.5 bg-[#E6DFD4] hover:bg-[#D1C7BD] text-espresso font-bold text-xs uppercase tracking-wider rounded-lg transition duration-200 cursor-pointer border-0">Limpiar</button>
            </div>
          </form>
        </div>
        ...
        <div class="bg-white border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          <div class="flex flex-col sm:flex-row items-baseline sm:items-center justify-between gap-2 border-b pb-2">
            <h3 class="font-display font-semibold text-espresso text-base">📚 PDFs publicados (${(pdfCatalog.recursos || []).length})</h3>
            <input type="text" id="pdf-admin-search-input" oninput="filterAdminPdfs()" placeholder="Buscar por título, categoría o público..." class="border border-border rounded-lg px-3 py-1 text-xs outline-none focus:ring-1 focus:ring-gold bg-[#fefdfa] w-full sm:w-72">
          </div>
          <div id="admin-pdf-list-container" class="max-h-[520px] overflow-y-auto border border-border rounded-xl divide-y text-xs bg-white">
            ${(pdfCatalog.recursos || []).length ? (pdfCatalog.recursos || []).map(pdf => `
              <div class="pdf-admin-card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3" data-search="${escapeHtml([pdf.titulo, pdf.descripcion, pdf.categoria, pdf.audiencia, pdf.keywords].join(' ').toLowerCase())}">
                <div class="flex items-start gap-3 min-w-0">
                  <div class="w-16 h-11 rounded-lg border border-border overflow-hidden bg-cream shrink-0 flex items-center justify-center">
                    ${pdf.coverUrl ? `<img src="${escapeHtml(pdf.coverUrl)}" alt="${escapeHtml(pdf.titulo)}" class="w-full h-full object-cover" loading="lazy" referrerPolicy="no-referrer">` : pdfIconHtml('w-8 h-8')}
                  </div>
                  <div class="flex flex-col gap-1 min-w-0">
                    <div class="font-display text-maroon font-bold text-sm truncate">${escapeHtml(pdf.titulo)}</div>
                    <div class="text-[11px] text-ink2 line-clamp-2">${escapeHtml(pdf.descripcion || '')}</div>
                    <div class="text-[10px] text-gold font-mono uppercase">${escapeHtml(pdf.audiencia || 'general')} · ${escapeHtml(pdf.categoria || 'catequesis')} · /catequesis-ia/recursos/${escapeHtml(pdf.slug)}</div>
                  </div>
                </div>
            ...
            `).join('') : `<div class="p-8 text-center text-ink-2 italic">Aún no hay PDFs registrados.</div>`}
          </div>
        </div>
      </div>

      <!-- 5. CANALES Y VIDEOS (YOUTUBE) -->
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                  <div class="flex flex-col gap-1">
                    <div class="flex items-center justify-between gap-2">
                      ...
                      <button type="button" onclick="generateSeoField('santoral', 'seoTitle', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                    </div>
                  ...
                  <div class="flex flex-col gap-1">
                    <div class="flex items-center justify-between gap-2">
                      ...
                      <button type="button" onclick="generateSeoField('santoral', 'keywords', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                    </div>
                  ...
                  <div class="flex flex-col gap-1 md:col-span-2">
                    <div class="flex items-center justify-between gap-2">
                      ...
                      <button type="button" onclick="generateSeoField('santoral', 'metaDescription', this)" class="text-[10px] text-maroon border border-maroon hover:bg-maroon hover:text-white px-2 py-0.5 rounded font-bold transition cursor-pointer">OpenAI</button>
                    </div>
                  ...
                  <button type="button" onclick="generateSeoField('santoral', 'all', this)" class="md:col-span-2 bg-gold text-white text-[10px] px-3 py-1.5 rounded-md font-bold uppercase tracking-wider hover:bg-espresso transition border-0 cursor-pointer">
                    ✨ Generar SEO completo con OpenAI
                  </button>
                </div>
              </div>
    ...
    <!-- MODAL DE GOOGLE DRIVE PARA PDFS -->
    <div id="drive-pdf-explorer-modal" class="fixed inset-0 bg-[#1A0E05]/70 backdrop-blur-xs hidden items-center justify-center z-50 p-4" style="z-index: 75;">
      <div class="bg-[#FCFAF5] border border-[#D1C7BD] rounded-2xl w-full max-w-4xl h-[82vh] flex flex-col overflow-hidden shadow-2xl" onclick="event.stopPropagation()">
        <div class="bg-white border-b border-[#E6DFD4] px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
          <div class="flex items-center gap-2.5">
            <span class="inline-flex w-10 h-10 items-center justify-center rounded-xl border border-gold/40 bg-cream text-maroon font-display font-bold">PDF</span>
            <div class="flex flex-col">
              <h2 class="font-display font-bold text-espresso text-base">Biblioteca PDF de Google Drive</h2>
              <p class="text-[10px] text-ink2">Carpeta configurada para recursos descargables públicos de CatólicosGPT.</p>
            </div>
          </div>
          <button type="button" onclick="closeDrivePdfExplorer()" class="text-ink2 hover:bg-cream-light p-2 rounded-xl transition cursor-pointer border-0 bg-transparent text-base font-bold">✕</button>
        </div>

        <div class="bg-[#F8F5EE] border-b border-[#E6DFD4] p-4 flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
          <div class="flex-1 flex flex-col gap-1">
            <label class="font-semibold text-espresso text-xs">Buscar PDF en Drive</label>
            <input type="text" id="drive-pdf-search" oninput="debounceDrivePdfResources()" placeholder="Buscar por nombre del archivo..." class="border border-border rounded-lg px-3 py-2 focus:ring-1 focus:ring-gold bg-white text-xs outline-none">
          </div>
          <button type="button" onclick="fetchDrivePdfResources()" class="bg-maroon hover:bg-gold text-white px-4 py-2 rounded-lg font-bold text-xs transition cursor-pointer border-0">Actualizar</button>
        </div>

        <div class="flex-1 overflow-y-auto p-6">
          <div id="drive-pdf-loader" class="flex flex-col items-center justify-center p-20 gap-3 text-ink2 text-xs">
            <div class="w-8 h-8 border-4 border-gold border-t-maroon rounded-full animate-spin"></div>
            Leyendo carpeta de Google Drive...
          </div>
          <div id="drive-pdf-empty" class="hidden flex flex-col items-center justify-center p-20 text-center gap-2 text-xs text-ink2 italic">
            <span class="font-bold text-maroon">No hay PDFs disponibles.</span>
            <span>Verifica que la carpeta esté compartida con la cuenta de servicio y que contenga PDFs.</span>
          </div>
          <div id="drive-pdf-grid" class="hidden grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"></div>
        </div>
      </div>
    </div>
    ...
    <div id="html-editor-modal" class="fixed inset-0 bg-[#1A0E05]/70 backdrop-blur-xs hidden items-center justify-center z-50 p-4">
      <div class="bg-[#FCFAF5] border border-[#D1C7BD] rounded-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden shadow-2xl" onclick="event.stopPropagation()">
        ...
        <div class="bg-[#F8F5EE] border-b border-[#E6DFD4] p-3 flex gap-2 flex-wrap items-center">
          ...
          <button type="button" onclick="insertHtmlSnippet('<em>', '</em>')" class="px-2.5 py-1 bg-white hover:bg-cream-light border border-border text-[11px] font-semibold rounded-md text-espresso cursor-pointer transition italic">
            Cursiva
          </button>
          <button type="button" onclick="insertHtmlSnippet('', '<br>')" class="px-2.5 py-1 bg-white hover:bg-cream-light border border-border text-[11px] font-semibold rounded-md text-espresso cursor-pointer transition">
            Salto &lt;br&gt;
          </button>
          <button type="button" onclick="insertHtmlSnippet('\\\\n\\\\n', '')" class="px-2.5 py-1 bg-white hover:bg-cream-light border border-border text-[11px] font-semibold rounded-md text-espresso cursor-pointer transition">
            Separar párrafo
          </button>
          ...
          <button type="button" onclick="insertClassicHtmlTemplate()" class="px-2.5 py-1 bg-[#081A33] hover:bg-maroon border border-gold text-[11px] font-bold rounded-md text-gold cursor-pointer transition">
            Classic version
          </button>
        </div>
    ...
    <script>
      ...
      function getSeoContext(entityType) {
        if (entityType === 'infografia') {
          return {
            titulo: document.getElementById('seo_titulo')?.value || '',
            seoTitle: document.getElementById('infografia_seo_title')?.value || '',
            tema: document.getElementById('seo_tema')?.value || '',
            categoria: document.getElementById('seo_categoria')?.value || '',
        ...
        if (entityType === 'blog') {
          return {
            ...
            contenido: document.getElementById('blog_content_editor')?.value || '',
            imagenPortada: document.getElementById('blog_imagen_portada')?.value || '',
            seoTitle: document.getElementById('blog_seo_titulo')?.value || '',
            metaDescription: document.getElementById('blog_seo_descripcion')?.value || '',
            keywords: document.getElementById('blog_seo_keywords')?.value || ''
          };
        }
        if (entityType === 'santoral') {
          return {
            nombre: document.getElementById('santo_nombre')?.value || '',
            dia: document.getElementById('santo_dia')?.value || '',
            mes: document.getElementById('santo_mes_select')?.value || '',
            tipo: document.getElementById('santo_tipo')?.value || '',
            lema: document.getElementById('santo_lema')?.value || '',
            biografia: document.getElementById('santo_biografia')?.value || '',
            patronato: document.getElementById('santo_patronato')?.value || '',
            nacimiento: document.getElementById('santo_nacimiento')?.value || '',
            muerte: document.getElementById('santo_muerte')?.value || '',
            canonizacion: document.getElementById('santo_canonizacion')?.value || '',
            seoTitle: document.getElementById('santo_seo_titulo')?.value || '',
            metaDescription: document.getElementById('santo_seo_descripcion')?.value || '',
            keywords: document.getElementById('santo_seo_keywords')?.value || ''
          };
        }
        if (entityType === 'recurso-pdf') {
          return {
            titulo: document.getElementById('pdf_titulo')?.value || '',
            categoria: document.getElementById('pdf_categoria')?.value || '',
            audiencia: document.getElementById('pdf_audiencia')?.value || '',
            contenido: document.getElementById('pdf_descripcion')?.value || '',
            metaDescription: document.getElementById('pdf_descripcion')?.value || '',
        ...
        if (entityType === 'oracion') {
          return {
            ...
            tipo: document.getElementById('oracion_tipo')?.value || '',
            categoria: document.getElementById('oracion_categoria')?.value || '',
            descripcion: document.getElementById('oracion_descripcion')?.value || '',
            contenido: document.getElementById('oracion_descripcion')?.value || '',
      ...
      function applySeoResult(entityType, requestedField, data) {
        const mappings = {
          infografia: {
            seoTitle: 'infografia_seo_title',
            metaDescription: 'seo_desc',
            keywords: 'seo_keywords'
          },
          blog: {
            seoTitle: 'blog_seo_titulo',
            metaDescription: 'blog_seo_descripcion',
            keywords: 'blog_seo_keywords'
          },
          santoral: {
            seoTitle: 'santo_seo_titulo',
            metaDescription: 'santo_seo_descripcion',
            keywords: 'santo_seo_keywords'
          },
          'recurso-pdf': {
        ...
        const fields = requestedField === 'all' ? ['seoTitle', 'metaDescription', 'keywords'] : [requestedField];
      ...
      function showSeoToast(message, isWarning) {
        const alertDiv = document.createElement('div');
        alertDiv.className = 'fixed bottom-4 right-4 text-white font-semibold py-2 px-4 rounded-xl shadow-lg z-[9999] transition duration-500 text-xs ' + (isWarning ? 'bg-amber-700' : 'bg-emerald-700');
      ...
      async function generateSeoField(entityType, requestedField, btn) {
        const context = getSeoContext(entityType);
        const titleSeed = context.titulo || context.nombre || context.tema;
        if (!titleSeed) {
          alert('Primero escribe el título, nombre o tema del contenido.');
          return;
        }
        ...
        try {
          const response = await fetch('/api/admin/seo/openai', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ entityType: entityType, requestedField: requestedField, context: context })
          });
          const data = await response.json();
          if (!response.ok || data.error) {
            throw new Error(data.error || 'No se pudo generar SEO.');
          }
          applySeoResult(entityType, requestedField, data);
          showSeoToast(data.warning || 'SEO actualizado con OpenAI.', Boolean(data.warning));
        } catch (err) {
          alert('No se pudo generar SEO: ' + err.message);
        } finally {
          if (btn) {
      ...
      let drivePdfResources = [];
      let drivePdfDebounceTimer = null;

      function openDrivePdfExplorer() {
        const modal = document.getElementById('drive-pdf-explorer-modal');
        if (!modal) return;
        modal.classList.remove('hidden');
        modal.classList.add('flex');
        fetchDrivePdfResources();
      }

      function closeDrivePdfExplorer() {
        const modal = document.getElementById('drive-pdf-explorer-modal');
        if (!modal) return;
      ...
      function debounceDrivePdfResources() {
        clearTimeout(drivePdfDebounceTimer);
        drivePdfDebounceTimer = setTimeout(fetchDrivePdfResources, 300);
      }

      function drivePdfIconHtml() {
        return \`
          <span class="inline-flex w-12 h-12 items-center justify-center rounded-xl border border-gold/40 bg-white shadow-sm overflow-hidden shrink-0">
            <svg viewBox="0 0 64 64" class="w-full h-full block" xmlns="http://www.w3.org/2000/svg" role="img">
              <path d="M17 6h22l10 10v42H17z" fill="#FAF6F0" stroke="#6B1F2A" stroke-width="3" stroke-linejoin="round"/>
              <path d="M39 6v12h10" fill="none" stroke="#BC8A36" stroke-width="3" stroke-linejoin="round"/>
              <rect x="11" y="30" width="42" height="17" rx="4" fill="#6B1F2A"/>
      ...
      async function fetchDrivePdfResources() {
        const grid = document.getElementById('drive-pdf-grid');
        const loader = document.getElementById('drive-pdf-loader');
        const empty = document.getElementById('drive-pdf-empty');
        if (!grid || !loader || !empty) return;
        grid.classList.add('hidden');
        empty.classList.add('hidden');
        loader.classList.remove('hidden');
        try {
          const search = encodeURIComponent((document.getElementById('drive-pdf-search')?.value || '').trim());
          const response = await fetch('/api/admin/google-drive/pdfs?search=' + search);
          const data = await response.json();
          if (!response.ok || data.error) throw new Error(data.error || 'No se pudo leer Google Drive.');
          drivePdfResources = data.resources || [];
          drawDrivePdfResources();
        } catch (err) {
          console.error('Error leyendo Google Drive:', err);
          loader.classList.add('hidden');
          empty.classList.remove('hidden');
          empty.innerHTML = '<span class="font-bold text-maroon">Google Drive no está disponible.</span><span>' + escapeHtmlAttribute(err.message || 'Revisa la configuración de la cuenta de servicio.') + '</span>';
        }
      }

      function drawDrivePdfResources() {
        const grid = document.getElementById('drive-pdf-grid');
        const loader = document.getElementById('drive-pdf-loader');
        const empty = document.getElementById('drive-pdf-empty');
        if (!grid || !loader || !empty) return;
        loader.classList.add('hidden');
        if (!drivePdfResources.length) {
          grid.classList.add('hidden');
          empty.classList.remove('hidden');
          return;
        }
        ...
        grid.innerHTML = drivePdfResources.map((item, idx) => {
          const size = item.size ? Math.round(Number(item.size) / 1024) + ' KB' : 'PDF';
          const date = item.modifiedTime ? new Date(item.modifiedTime).toLocaleDateString('es-CO') : '';
          return \`
            <button type="button" onclick="selectDrivePdfResource(\${idx})" class="text-left bg-white border border-border hover:border-gold rounded-xl p-4 shadow-xs transition cursor-pointer flex gap-3 items-start">
              \${drivePdfIconHtml()}
              <span class="min-w-0 flex-1">
                <span class="block font-display font-bold text-maroon text-sm leading-tight">\${escapeHtmlAttribute(item.name || 'PDF de Google Drive')}</span>
                <span class="block text-[10px] text-ink2 mt-1">Google Drive · \${escapeHtmlAttribute(size)} \${date ? '· ' + escapeHtmlAttribute(date) : ''}</span>
                <span class="block text-[10px] text-gold font-bold uppercase tracking-wider mt-2">Seleccionar PDF</span>
              </span>
            </button>
          \`;
        }).join('');
      ...
      function selectDrivePdfResource(index) {
        const item = drivePdfResources[index];
        if (!item || !item.id) return;
        const cleanName = (item.name || 'documento.pdf').replace(/\\.pdf$/i, '').replace(/[-_]+/g, ' ');
        const detectedTitle = item.title || titleCaseSpanish(cleanName);

        document.getElementById('pdf_url').value = item.viewUrl || '';
        document.getElementById('pdf_drive_file_id').value = item.id || '';
        document.getElementById('pdf_drive_view_url').value = item.viewUrl || '';
        document.getElementById('pdf_drive_download_url').value = item.downloadUrl || '';
        document.getElementById('pdf_cloudinary_public_id').value = '';
        document.getElementById('pdf_cloudinary_resource_type').value = '';
        document.getElementById('pdf_cloudinary_version').value = '';
        document.getElementById('pdf_cloudinary_asset_id').value = '';
        document.getElementById('pdf_cloudinary_secure_url').value = '';
        document.getElementById('pdf_format').value = 'pdf';
        document.getElementById('pdf_bytes').value = item.size || '';

        const selectedName = document.getElementById('pdf_selected_name');
        if (selectedName) selectedName.textContent = (item.name || 'PDF') + ' · Google Drive';
        if (!document.getElementById('pdf_titulo').value.trim()) {
          document.getElementById('pdf_titulo').value = detectedTitle;
        }
        const status = document.getElementById('pdf_upload_status');
        if (status) status.textContent = 'PDF seleccionado desde Google Drive. Generando descripción y keywords con OpenAI si están vacías...';
        closeDrivePdfExplorer();
        autoGeneratePdfSeoFromSelection();
      }

      async function uploadPdfToCloudinary(btn) {
        ...
        if (status) status.textContent = 'Leyendo archivo y subiendo a Cloudinary...';
        ...
        try {
          ...
          const response = await fetch('/api/admin/recursos-pdf/upload', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ filename: file.name, dataUrl })
          });
          const data = await response.json();
          if (!response.ok || data.error) throw new Error(data.error || 'No se pudo subir el PDF.');
          document.getElementById('pdf_url').value = data.url || '';
          document.getElementById('pdf_cloudinary_public_id').value = data.public_id || '';
          document.getElementById('pdf_cloudinary_resource_type').value = data.resource_type || 'raw';
          document.getElementById('pdf_cloudinary_version').value = data.version || '';
          document.getElementById('pdf_cloudinary_asset_id').value = data.asset_id || '';
          document.getElementById('pdf_cloudinary_secure_url').value = data.url || '';
          document.getElementById('pdf_format').value = data.format || 'pdf';
          document.getElementById('pdf_bytes').value = data.bytes || '';
          const selectedName = document.getElementById('pdf_selected_name');
          if (selectedName) selectedName.textContent = file.name;
          if (!document.getElementById('pdf_titulo').value) {
            document.getElementById('pdf_titulo').value = titleCaseSpanish(file.name.replace(/\\.pdf$/i, '').replace(/[-_]+/g, ' '));
          }
          const categoria = document.getElementById('pdf_categoria');
          const titleLower = (document.getElementById('pdf_titulo')?.value || '').toLowerCase();
          if (titleLower.includes('niñ') || titleLower.includes('colorear')) document.getElementById('pdf_audiencia').value = 'ninos';
          if (categoria && (!categoria.value || categoria.value === 'catequesis')) {
            if (titleLower.includes('sacramento') || titleLower.includes('confes') || titleLower.includes('eucarist')) categoria.value = 'sacramentos';
            else if (titleLower.includes('virgen') || titleLower.includes('maria') || titleLower.includes('maría')) categoria.value = 'virgen-maria';
            else if (titleLower.includes('santo') || titleLower.includes('santa')) categoria.value = 'santos';
            else if (titleLower.includes('oracion') || titleLower.includes('oración')) categoria.value = 'oracion';
          }
          if (status) status.textContent = 'PDF subido correctamente como RAW. Generando descripción y keywords con OpenAI...';
          await autoGeneratePdfSeoFromSelection();
        } catch (err) {
          if (status) status.textContent = 'Error al subir: ' + err.message;
      ...
      function resetPdfForm() {
        const ids = ['pdf_original_slug', 'pdf_cloudinary_public_id', 'pdf_cloudinary_resource_type', 'pdf_cloudinary_version', 'pdf_cloudinary_asset_id', 'pdf_cloudinary_secure_url', 'pdf_drive_file_id', 'pdf_drive_view_url', 'pdf_drive_download_url', 'pdf_format', 'pdf_bytes', 'pdf_titulo', 'pdf_categoria', 'pdf_url', 'pdf_cover_url', 'pdf_descripcion', 'pdf_keywords'];
        ids.forEach(id => {
          const el = document.getElementById(id);
          if (!el) return;
          if (id === 'pdf_categoria') el.value = 'catequesis';
          else if (id === 'pdf_cloudinary_resource_type') el.value = 'raw';
          else if (id === 'pdf_format') el.value = 'pdf';
          else el.value = '';
        });
        const aud = document.getElementById('pdf_audiencia');
        if (aud) aud.value = 'general';
        const pub = document.getElementById('pdf_publicado');
        if (pub) pub.checked = true;
        const file = document.getElementById('pdf_file_input');
        if (file) file.value = '';
        const selectedName = document.getElementById('pdf_selected_name');
        if (selectedName) selectedName.textContent = 'Ningún PDF seleccionado';
        const status = document.getElementById('pdf_upload_status');
        if (status) status.textContent = 'Puedes pegar un enlace público de Google Drive o seleccionar un PDF RAW de Cloudinary.';
        const title = document.getElementById('pdf_form_title');
        if (title) title.textContent = 'Administrar PDFs de Catequesis IA';
        const submit = document.getElementById('pdf_submit_btn');
        if (submit) submit.textContent = 'Guardar PDF';
      }

      function editPdfResource(pdf) {
        document.getElementById('pdf_original_slug').value = pdf.slug || '';
        document.getElementById('pdf_cloudinary_public_id').value = pdf.cloudinaryPublicId || '';
        document.getElementById('pdf_cloudinary_resource_type').value = pdf.cloudinaryResourceType || 'raw';
        document.getElementById('pdf_cloudinary_version').value = pdf.cloudinaryVersion || '';
        document.getElementById('pdf_cloudinary_asset_id').value = pdf.cloudinaryAssetId || '';
        document.getElementById('pdf_cloudinary_secure_url').value = pdf.cloudinarySecureUrl || pdf.pdfUrl || '';
        document.getElementById('pdf_drive_file_id').value = pdf.driveFileId || '';
        document.getElementById('pdf_drive_view_url').value = pdf.driveViewUrl || '';
        document.getElementById('pdf_drive_download_url').value = pdf.driveDownloadUrl || '';
        document.getElementById('pdf_format').value = pdf.format || 'pdf';
        document.getElementById('pdf_bytes').value = pdf.bytes || '';
        document.getElementById('pdf_titulo').value = pdf.titulo || '';
        document.getElementById('pdf_audiencia').value = pdf.audiencia || 'general';
        document.getElementById('pdf_categoria').value = pdf.categoria || 'catequesis';
        document.getElementById('pdf_url').value = pdf.pdfUrl || '';
        document.getElementById('pdf_cover_url').value = pdf.coverUrl || '';
        document.getElementById('pdf_descripcion').value = pdf.descripcion || '';
        document.getElementById('pdf_keywords').value = pdf.keywords || pdf.tags || '';
        document.getElementById('pdf_publicado').checked = pdf.publicado !== false;
        const selectedName = document.getElementById('pdf_selected_name');
        if (selectedName) selectedName.textContent = pdf.driveFileId ? 'PDF enlazado desde Google Drive' : (pdf.cloudinaryPublicId ? pdf.cloudinaryPublicId.split('/').pop() + '.pdf' : (pdf.pdfUrl ? 'PDF enlazado' : 'Ningún PDF seleccionado'));
        const status = document.getElementById('pdf_upload_status');
        if (status) status.textContent = 'Recurso cargado para edición. Puedes cambiar el PDF con un enlace público de Drive o seleccionando otro desde Cloudinary.';
        document.getElementById('pdf_form_title').textContent = 'Editar PDF: ' + (pdf.titulo || '');
        document.getElementById('pdf_submit_btn').textContent = 'Actualizar PDF';
        switchTab('recursos-pdf');
        window.scrollTo({ top: 0, behavior: 'smooth' });
      }

      function filterAdminPdfs() {
        const q = (document.getElementById('pdf-admin-search-input')?.value || '').toLowerCase().trim();
        document.querySelectorAll('.pdf-admin-card').forEach(card => {
          const haystack = card.getAttribute('data-search') || '';
          card.style.display = !q || haystack.includes(q) ? 'flex' : 'none';
        });
      }

      let draggedInfografiaId = null;

      function dragInfografiaStart(event) {
        const item = event.currentTarget;
        draggedInfografiaId = item.getAttribute('data-id');
        item.classList.add('opacity-50');
        event.dataTransfer.effectAllowed = 'move';
        event.dataTransfer.setData('text/plain', draggedInfografiaId || '');
      }

      function dragInfografiaOver(event) {
        event.preventDefault();
        if (event.currentTarget.dataset.lastMove && Date.now() - Number(event.currentTarget.dataset.lastMove) < 45) return;
        event.currentTarget.dataset.lastMove = String(Date.now());
        const target = event.currentTarget;
        const list = document.getElementById('admin-infografias-sortable');
        const dragging = list ? list.querySelector('[data-id="' + draggedInfografiaId + '"]') : null;
        if (!dragging || dragging === target || !list) return;
      ...
      async function dropInfografiaItem(event) {
        event.preventDefault();
        await saveInfografiaOrder();
      }

      function dragInfografiaEnd(event) {
        event.currentTarget.classList.remove('opacity-50');
        draggedInfografiaId = null;
      }

      async function moveInfografiaListItem(id, direction) {
        const list = document.getElementById('admin-infografias-sortable');
        if (!list) return;
        const item = list.querySelector('.infografia-sort-item[data-id="' + id + '"]');
        if (!item) return;
      ...
      async function saveInfografiaOrder() {
        const list = document.getElementById('admin-infografias-sortable');
        if (!list) return;
        const orderedIds = Array.from(list.querySelectorAll('.infografia-sort-item'))
          .map(item => item.getAttribute('data-id'))
          .filter(Boolean);
        if (!orderedIds.length) return;

        try {
          const response = await fetch('/api/admin/infografias/reorder', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
      ...
      function cargarEditarBlog(btn) {
        ...
        document.getElementById('blog_seo_titulo').value = data.seoTitle || data.titulo || '';
        document.getElementById('blog_seo_descripcion').value = data.descripcion || '';
        document.getElementById('blog_seo_keywords').value = data.keywords || '';
      ...
      function resetBlogForm() {
        ...
        document.getElementById('blog_seo_titulo').value = '';
        document.getElementById('blog_seo_descripcion').value = '';
        document.getElementById('blog_seo_keywords').value = '';
      }

      function prepareCatequesisForm(category) {
        switchTab('blog');
        window.location.hash = 'blog';
        resetBlogForm();
        const select = document.getElementById('blog_categoria');
        if (select) select.value = category;
        const title = document.getElementById('blog_form_title');
        if (title) title.innerText = category === 'catequesis-ninos'
          ? 'Crear Catequesis IA para Niños'
          : 'Crear Catequesis IA para Jóvenes';
        const textarea = document.getElementById('blog_content_editor');
        if (textarea && !textarea.value) {
          textarea.value = category === 'catequesis-ninos'
            ? '## Objetivo de la catequesis\\n\\nExplica aquí qué aprenderán los niños.\\n\\n## Desarrollo\\n\\n### Pregunta inicial\\n\\n### Explicación sencilla\\n\\n## Actividad\\n\\n## Oración final\\n'
            : '## Objetivo de la catequesis\\n\\nExplica aquí qué aprenderán los jóvenes.\\n\\n## Desarrollo\\n\\n### Pregunta para abrir diálogo\\n\\n### Fundamento de fe\\n\\n## Aplicación a la vida\\n\\n## Oración final\\n';
        }
        document.getElementById('blogForm')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }

      async function runDailyContentNow(button) {
        if (!confirm('Esto generará contenidos nuevos con OpenAI si no se han generado hoy. ¿Continuar?')) return;
        const original = button.innerText;
        button.disabled = true;
        button.innerText = 'Generando...';
        try {
          const res = await fetch('/api/admin/daily-content/run', { method: 'POST', headers: { 'Content-Type': 'application/json' } });
          const data = await res.json();
          if (!res.ok || data.error) throw new Error(data.error || 'No se pudo generar.');
          alert(data.skipped ? ('No se generó: ' + data.reason) : 'Generación terminada. Revisa Fe Católica y Catequesis IA.');
          loadAdminBlogs();
        } catch (e) {
      ...
      window.addEventListener('DOMContentLoaded', () => {
        ...
        if (typeof addOracionImageRow === 'function') {
          const oracionContainer = document.getElementById('oracion-images-container');
          if (oracionContainer && oracionContainer.querySelectorAll('.oracion-image-row').length === 0) {
            addOracionImageRow({ cover: true });
          }
        }
      });
      ...
      function updateHtmlPreview() {
        const code = normalizeEditorText(document.getElementById('html-editor-code').value || '');
        ...
        if (lowerCode.includes('<p') || lowerCode.includes('<div') || lowerCode.includes('<table') || lowerCode.includes('</ul>') || lowerCode.includes('</ol>') || lowerCode.includes('<article') || lowerCode.includes('<section') || lowerCode.includes('<figure') || lowerCode.includes('<img') || lowerCode.includes('<strong') || lowerCode.includes('<em') || lowerCode.includes('</h1>') || lowerCode.includes('</h2>') || lowerCode.includes('</h3>')) {
      ...
      function formatInlineEditorText(text) {
        return String(text || '')
          .replace(/\\*\\*\\*([^*]+)\\*\\*\\*/g, '<strong><em>$1</em></strong>')
          .replace(/\\*\\*([^*]+)\\*\\*/g, '<strong>$1</strong>')
          .replace(/__([^_]+)__/g, '<strong>$1</strong>')
          .replace(/\\*([^*]+)\\*/g, '<em>$1</em>')
          .replace(/!\\[([^\\]]*)\\]\\(([^)]+)\\)/g, '<img src="$2" alt="$1" loading="lazy" style="max-width:100%;border-radius:10px;margin:14px 0">')
      ...
      function formatMixedHtmlPreview(raw) {
        ...
        return normalizeEditorText(raw)
          ...
          .map(block => {
            ...
            return '<p>' + formatInlineEditorText(b).replace(/\\n/g, '<br>') + '</p>';
          })
          .join('\\n\\n');
      }
      ...
      function insertHtmlSnippet(prefix, suffix = '') {
        ...
        prefix = normalizeEditorText(prefix);
        suffix = normalizeEditorText(suffix);
      ...
      function insertClassicHtmlTemplate() {
        const template = \`<style>
.cgpt-classic {
  --classic-navy: #081A33;
  --classic-wine: #6B1F2A;
  --classic-gold: #BC8A36;
  --classic-paper: #F3E2B8;
  --classic-line: #C59A4B;
  color: #221A14;
  font-family: Georgia, "Times New Roman", serif;
  line-height: 1.7;
}
.cgpt-classic * { box-sizing: border-box; }
.cgpt-classic-section {
  background: radial-gradient(circle at top left, rgba(255,255,255,.34), transparent 38%), linear-gradient(135deg, #F7EAC7 0%, #E4C989 100%);
...
.cgpt-classic p { margin: 0 0 1rem; }
.cgpt-classic .drop-cap::first-letter {
  float: left;
  color: #9A3A12;
  font-size: 4rem;
  line-height: .82;
...
.cgpt-classic td {
  border: 1px solid rgba(157, 119, 45, .45);
  padding: 10px 12px;
  vertical-align: top;
}
.cgpt-classic .century {
  display: inline-block;
  background: var(--classic-navy);
  color: #F4CE65;
  border-radius: 2px;
...
@media (max-width: 640px) {
  .cgpt-classic-section { padding: 16px; }
  .cgpt-classic .drop-cap::first-letter { float: none; font-size: inherit; padding-right: 0; }
  .cgpt-classic table { min-width: 620px; font-size: .86rem; }
}
</style>
<article class="cgpt-classic">
  <section class="cgpt-classic-section">
    <div class="cgpt-classic-kicker">
      <span>CatólicosGPT</span>
      <span>Classic version</span>
    </div>
    <div style="text-align:center"><span class="cgpt-classic-title">Siglos I — V</span></div>
    <h2>Título del contenido clásico</h2>
    <p class="drop-cap">Escribe aquí la introducción del artículo. Esta versión está pensada para contenidos largos, cronologías, historia de la Iglesia, tablas formativas y material con estética editorial clásica.</p>
    <div class="cgpt-classic-table-wrap">
      <table>
        <thead>
          <tr>
            <th>Siglo</th>
        ...
        <tbody>
          <tr>
            <td><span class="century">I</span></td>
            <td>c. 33</td>
            <td>Nacimiento de la Iglesia en Pentecostés.</td>
            <td>San Pedro</td>
          </tr>
          <tr>
            <td><span class="century">IV</span></td>
            <td>325</td>
            <td>Concilio de Nicea y defensa de la divinidad de Cristo.</td>
            <td>San Atanasio</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="cgpt-classic-caption">Tabla — Sustituye estos datos por el contenido final</div>
  </section>
</article>\`;
        const textarea = document.getElementById('html-editor-code');
        if (!textarea) return;
        if (textarea.value.trim() && !confirm('Esto agregará la plantilla Classic al contenido actual. ¿Continuar?')) return;
      ...
      function applyHtmlChanges() {
        ...
        if (targetElement) {
          targetElement.value = normalizeEditorText(document.getElementById('html-editor-code').value);
      ...
      function drawCloudinaryResources() {
        ...
        grid.innerHTML = currentCloudinaryResources.map((r, idx) => {
          ...
          if (isAudio) {
            ...
            thumbHtml = \`
              <div class="w-full aspect-square bg-cream flex flex-col items-center justify-center text-maroon gap-2 p-3 rounded-t-xl border-b border-gold/20">
                <span class="inline-flex w-16 h-16 items-center justify-center rounded-xl border border-gold/40 bg-white shadow-sm overflow-hidden">
                  <svg viewBox="0 0 64 64" class="w-full h-full block" xmlns="http://www.w3.org/2000/svg" role="img">
                    <path d="M17 6h22l10 10v42H17z" fill="#FAF6F0" stroke="#6B1F2A" stroke-width="3" stroke-linejoin="round"/>
                    <path d="M39 6v12h10" fill="none" stroke="#BC8A36" stroke-width="3" stroke-linejoin="round"/>
                    <rect x="11" y="30" width="42" height="17" rx="4" fill="#6B1F2A"/>
                ...
                <span class="text-[10px] font-mono font-bold uppercase tracking-wider text-gold">Recurso PDF</span>
              </div>
            \`;
          } else {
            thumbHtml = \`
      ...
      function toggleSelectResource(url, idx) {
        ...
        if (selectedResourcesMap.has(url)) {
          ...
          const singleSelectContexts = ['blog_cover', 'santo_photo', 'videos', 'podcasts', 'recursos-pdf', 'recursos-pdf-cover'];
      ...
      function selectCloudinaryPdfResource(resource) {
        if (!resource || !resource.url) return;
        if ((resource.format || '').toLowerCase() === 'pdf' && resource.resource_type !== 'raw') {
          alert('Este PDF está guardado en Cloudinary como imagen y Cloudinary puede bloquearlo con error 401. Súbelo o selecciónalo como RAW en la carpeta catolicosgpt/recursos-pdf para que CatólicosGPT pueda descargarlo con URL propia.');
          return;
        }
        const name = resource.name || (resource.public_id ? resource.public_id.split('/').pop() : 'documento.pdf');
        const cleanName = readableCloudinaryName(resource);
        const detectedTitle = titleCaseSpanish(cleanName || name);

        document.getElementById('pdf_url').value = resource.url || '';
        document.getElementById('pdf_cloudinary_public_id').value = resource.public_id || '';
        document.getElementById('pdf_cloudinary_resource_type').value = resource.resource_type || 'raw';
        document.getElementById('pdf_cloudinary_version').value = resource.version || '';
        document.getElementById('pdf_cloudinary_asset_id').value = resource.asset_id || '';
        document.getElementById('pdf_cloudinary_secure_url').value = resource.secure_url || resource.url || '';
        document.getElementById('pdf_format').value = resource.format || 'pdf';
        document.getElementById('pdf_bytes').value = resource.bytes || '';
        const selectedName = document.getElementById('pdf_selected_name');
        ...
        const categoria = document.getElementById('pdf_categoria');
        if (categoria && (!categoria.value || categoria.value === 'catequesis')) {
          const lower = detectedTitle.toLowerCase();
          if (lower.includes('niñ') || lower.includes('colorear')) document.getElementById('pdf_audiencia').value = 'ninos';
          if (lower.includes('sacramento') || lower.includes('confes') || lower.includes('eucarist')) categoria.value = 'sacramentos';
          else if (lower.includes('virgen') || lower.includes('maria') || lower.includes('maría')) categoria.value = 'virgen-maria';
          else if (lower.includes('santo') || lower.includes('santa')) categoria.value = 'santos';
          else if (lower.includes('oracion') || lower.includes('oración')) categoria.value = 'oracion';
        }
        const status = document.getElementById('pdf_upload_status');
        if (status) status.textContent = 'PDF seleccionado desde Cloudinary. Generando descripción y keywords con OpenAI si están vacías...';
        autoGeneratePdfSeoFromSelection();
      }

      async function autoGeneratePdfSeoFromSelection() {
        const title = document.getElementById('pdf_titulo')?.value || '';
        if (!title) return;
        const descEl = document.getElementById('pdf_descripcion');
        const kwEl = document.getElementById('pdf_keywords');
        if ((descEl?.value || '').trim() && (kwEl?.value || '').trim()) return;
        try {
          const response = await fetch('/api/admin/seo/openai', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ entityType: 'recurso-pdf', requestedField: 'all', context: getSeoContext('recurso-pdf') })
          });
          const data = await response.json();
          if (!response.ok || data.error) throw new Error(data.error || 'No se pudo generar SEO.');
          if (descEl && !descEl.value.trim() && data.metaDescription) descEl.value = data.metaDescription;
          if (kwEl && !kwEl.value.trim() && data.keywords) kwEl.value = data.keywords;
          const status = document.getElementById('pdf_upload_status');
          if (status) status.textContent = 'PDF seleccionado y campos SEO sugeridos por OpenAI.';
        } catch (err) {
          const status = document.getElementById('pdf_upload_status');
          if (status) status.textContent = 'PDF seleccionado. OpenAI no pudo completar SEO automáticamente: ' + err.message;
        }
      }
      ...
      function confirmCloudinarySelection() {
        ...
        } else if (activeExplorerContext === 'recursos-pdf') {
          selectCloudinaryPdfResource(list[0]);
        } else if (activeExplorerContext === 'recursos-pdf-cover') {
...
function buildLocalSeoByEntity({ entityType, context } = {}) {
  const ctx = context || {};
  ...
  const keywords = [
    ...
    entityType === 'oracion' ? 'oracion catolica' : '',
    entityType === 'oracion' ? 'oraciones catolicas' : '',
    entityType === 'oracion' ? 'novenas catolicas' : '',
    'CatolicosGPT',
    'CatólicosGPT',
    'catolicos gpt',
    'ia catolica',
    ...
    'formacion catolica',
    'fe catolica'
  ].map(k => k.toLowerCase()).filter((k, idx, arr) => k && arr.indexOf(k) === idx).join(', ');
...
app.post('/admin/crear-oracion', (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).send('No autorizado');

  try {
    const urls = normalizeArrayField(req.body.oracionImageUrls).map(v => String(v || '').trim()).filter(Boolean);
    if (!String(req.body.titulo || '').trim()) {
      return res.status(400).send('Debes escribir el título de la oración.');
    }
    if (urls.length === 0) {
      return res.status(400).send('Debes agregar al menos una imagen de Cloudinary.');
    }

    const alts = normalizeArrayField(req.body.oracionImageAlts);
    const names = normalizeArrayField(req.body.oracionImageNames);
    const widths = normalizeArrayField(req.body.oracionImageWidths);
    const heights = normalizeArrayField(req.body.oracionImageHeights);
    const covers = normalizeArrayField(req.body.oracionImageCovers);

    const imagenes = urls.map((url, index) => ({
      url,
      alt: String(alts[index] || `Oración católica CatólicosGPT: ${req.body.titulo}`).trim(),
      name: String(names[index] || '').trim(),
      width: Number(widths[index]) || 1080,
      height: Number(heights[index]) || 1920,
      esPortada: String(covers[index] || '') === '1'
    }));
    if (!imagenes.some(img => img.esPortada)) imagenes[0].esPortada = true;
    ...
    oraciones.upsertOracion({
      originalSlug: req.body.oracion_original_slug,
      titulo: req.body.titulo,
      tipo: req.body.tipo || 'oracion',
      categoria: req.body.categoria || req.body.tipo || 'oracion',
      descripcion: req.body.descripcion || localSeo.metaDescription,
      seoTitle: req.body.seoTitle || localSeo.seoTitle,
      metaDescription: req.body.metaDescription || req.body.descripcion || localSeo.metaDescription,
      keywords: req.body.keywords || localSeo.keywords,
      tipoVisualizacion: req.body.tipoVisualizacion || (imagenes.length > 1 ? 'carrusel' : 'continua'),
      imagenes,
      publicado: req.body.publicado === 'true' || req.body.publicado === 'on'
    });

    res.redirect('/admin#oraciones');
  } catch (e) {
    res.status(400).send('No se pudo guardar la oración: ' + escapeHtml(e.message));
  }
});

app.get('/admin/eliminar-oracion', (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).send('No autorizado');
  const slug = String(req.query.slug || '').trim();
  if (slug) oraciones.deleteOracion(slug);
  res.redirect('/admin#oraciones');
});

app.post('/api/admin/recursos-pdf/upload', async (req, res) => {
  const user = getAuthedUser(req);
...
app.post('/admin/recursos-pdf/guardar', async (req, res) => {
  ...
  try {
    ...
    await recursosPdf.upsertResourceAsync({
      ...
      pdfUrl: safePdfUrl,
      driveFileId,
      driveViewUrl: req.body.driveViewUrl || getGoogleDriveViewUrl(driveFileId),
      driveDownloadUrl: req.body.driveDownloadUrl || getGoogleDriveDownloadUrl(driveFileId),
      cloudinaryPublicId: req.body.cloudinaryPublicId,
      cloudinaryResourceType: req.body.cloudinaryResourceType || 'raw',
      cloudinaryVersion: req.body.cloudinaryVersion,
      cloudinaryAssetId: req.body.cloudinaryAssetId,
      cloudinarySecureUrl: safeSecureUrl,
    ...
    res.status(400).send('No se pudo guardar el recurso PDF: ' + escapeHtml(e.message));
  }
});

app.get('/admin/recursos-pdf/eliminar', (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).send('No autorizado');
  if (req.query.slug) {
    recursosPdf.deleteBySlug(String(req.query.slug));
  }
  res.redirect('/admin#recursos-pdf');
});
...
app.get('/api/admin/google-drive/pdfs', async (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).json({ error: 'No autorizado' });

  try {
    const folderId = String(req.query.folderId || process.env.GOOGLE_DRIVE_FOLDER_ID || DEFAULT_GOOGLE_DRIVE_PDF_FOLDER_ID).trim();
    const resources = await listGoogleDrivePdfResources({
      folderId,
    ...
    console.error('[Google Drive PDF API]', err.message);
    res.status(502).json({
      error: err.message || 'No se pudo leer Google Drive.',
      resources: [],
      source: 'google-drive'
    });
  }
...
// API: GENERAR SEO CON OPENAI PARA BLOG, SANTORAL E INFOGRAFÍAS (AJAX)
app.post('/api/admin/seo/openai', async (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).json({ error: 'No autorizado' });
  ...
  if (requestedField && !allowedFields.includes(requestedField)) {
    return res.status(400).json({ error: 'Campo SEO no soportado.' });
  }

  try {
    const seo = await openaiChat.generateSeoJson({ entityType, requestedField, context });
    return res.json({ ...seo, provider: 'openai' });
  } catch(e) {
    console.error('[SEO OpenAI Error]', e.message);
    return res.json({
      ...buildLocalSeoByEntity({ entityType, context }),
      provider: 'local',
      warning: 'OpenAI no estuvo disponible. Se completó una versión local editable.'
    });
  }
});

app.post('/api/admin/daily-content/run', async (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).json({ error: 'No autorizado' });

  try {
    const result = await getDailyContentModule().runDailyContentJob({ force: req.body && req.body.force === true });
    return res.json(result);
  } catch (e) {
    return res.status(500).json({ error: 'Error generando contenidos diarios: ' + e.message });
  }
});

// Compatibilidad con el botón antiguo de infografías.
app.post('/api/seo/generar-seo-infografia', async (req, res) => {
  ...
  try {
    const seo = await openaiChat.generateSeoJson({
      entityType: 'infografia',
      requestedField: 'all',
      context: { titulo, tema, categoria }
    });
    return res.json({ ...seo, provider: 'openai' });
  } catch(e) {
    console.error('[SEO OpenAI Infografía Error]', e.message);
    return res.json({
      ...buildLocalInfografiaSeo({ titulo, tema, categoria }),
      provider: 'local',
      warning: 'OpenAI no estuvo disponible. Se completó el SEO con una versión local editable.'
    });
  }
});
...
app.post('/admin/crear-infografia-manual', async (req, res) => {
  ...
  const {
    ...
    esDibujoNinos,
    esImprimible,
    infografia_original_id,
    infografia_original_slug
  } = req.body;
  ...
  const validImageRows = imageUrls
    .map((url, index) => ({
      url: String(url || '').trim(),
      ...
      cover: imageCovers[index] || ''
    }))
    .filter(row => row.url);
  imageUrls = validImageRows.map(row => row.url);
  imageNames = validImageRows.map(row => row.name);
  imageAlts = validImageRows.map(row => row.alt);
  imageWidths = validImageRows.map(row => row.width);
  imageHeights = validImageRows.map(row => row.height);
  imageCovers = validImageRows.map(row => row.cover);
  ...
  try {
    ...
    const isEditing = !!infografia_original_id;
    const existingRecord = isEditing ? catalog.infografias.find(i => String(i.id) === String(infografia_original_id)) : null;
    const requestedSlug = infografia_original_slug || (existingRecord && existingRecord.slug) || slug;
    const uniqueSlug = isEditing
      ? requestedSlug
      : (catalog.infografias.some(i => i.slug === slug) ? `${slug}-${Date.now().toString().slice(-4)}` : slug);
    ...
    const isChildrenResource = mostrarEnNinos === 'true'
      || esDibujoNinos === 'true'
      || esImprimible === 'true'
      || audienciaRecurso === 'ninos'
      || ['catequesis-ninos', 'dibujo-para-colorear', 'infografia-ninos'].includes(categoria);
    ...
    if (isChildrenResource) {
      const childKeywords = 'catequesis para niños, dibujos católicos para colorear, recursos católicos imprimibles, cartillas católicas para niños, CatólicosGPT, IA Católica';
      finalKeywords = finalKeywords ? `${finalKeywords}, ${childKeywords}` : childKeywords;
      if (!/niñ|infantil|imprim/i.test(finalDesc || '')) {
        finalDesc = `${finalDesc} Recurso infantil de CatólicosGPT listo para descargar, imprimir y usar en catequesis.`;
      }
    }

    const newInf = {
      id: isEditing ? infografia_original_id : `inf-${Date.now()}`,
      slug: uniqueSlug,
      tema,
      ...
      userPlan: 'admin',
      userId: user.id,
      fechaCreacion: (existingRecord && existingRecord.fechaCreacion) || new Date().toISOString(),
      fechaISO: (existingRecord && existingRecord.fechaISO) || new Date().toISOString().slice(0, 10),
      publicado: true,
      keywords: finalKeywords
    };

    newInf.audienciaRecurso = audienciaRecurso || (isChildrenResource ? 'ninos' : 'general');
    newInf.mostrarEnNinos = isChildrenResource || mostrarEnNinos === 'true';
    newInf.esDibujoNinos = esDibujoNinos === 'true' || categoria === 'dibujo-para-colorear';
    newInf.esImprimible = esImprimible === 'true' || categoria === 'infografia-ninos' || categoria === 'dibujo-para-colorear';

    if (isEditing) {
      const updated = infografias.updateInfografia(infografia_original_id, newInf);
      if (!updated) return res.status(404).send('Infografía no encontrada para editar.');
    } else {
...
// ACCIÓN: CREAR O EDITAR POST DE BLOG CON SEO MANUAL/OPENAI
app.post('/admin/crear-blog', async (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).send('No autorizado');

  const { titulo, categoria, contenidoMd, imagenPortada, blog_original_slug, seo_titulo, seo_descripcion, seo_keywords } = req.body;
  ...
  try {
    const localSeo = buildLocalSeoByEntity({
      entityType: 'blog',
      context: { titulo, categoria, contenido: contenidoMd, seoTitle: seo_titulo, metaDescription: seo_descripcion, keywords: seo_keywords }
    });
    let seoFields = {
      titulo: seo_titulo || titulo,
      descripcion: seo_descripcion || localSeo.metaDescription,
      keywords: seo_keywords || localSeo.keywords,
      altText: titulo,
    ...
    const finalPost = {
      slug,
      titulo,
      seoTitle: seoFields.titulo || titulo,
...
app.post('/api/admin/infografias/reorder', (req, res) => {
  const user = getAuthedUser(req);
  if (!isStrictAdminUser(user)) return res.status(403).json({ error: 'No autorizado' });
  ...
  try {
    ...
    const catalog = infografias.reorderInfografias(orderedIds);
...
app.post('/admin/crear-santo', express.urlencoded({ extended: true }), async (req, res) => {
  ...
  try {
    ...
    const localSeo = buildLocalSeoByEntity({
      entityType: 'santoral',
      context: { nombre, dia, mes: mes_select, tipo, lema, biografia, seoTitle: seo_titulo, metaDescription: seo_descripcion, keywords: seo_keywords }
    });
    ...
    const santoData = {
      ...
      seo_titulo: finalSeoTitle || localSeo.seoTitle || `${nombre} — Santo del Día | CatólicosGPT`,
      seo_descripcion: finalSeoDesc || localSeo.metaDescription || `Conoce la vida y obra de ${nombre}. Biografía completa en el Santoral de CatólicosGPT.`,
      seo_keywords: finalSeoKeywords || localSeo.keywords || `santo del dia, ${nombre}, santoral, vida de santos`
    };
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  try {
    getDailyContentModule().scheduleDailyContentJob();
  } catch (e) {
    console.error('[Daily Content] No se pudo programar la generación diaria:', e.message);
  }
});
