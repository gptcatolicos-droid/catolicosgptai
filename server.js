...
function getDailyContentModule() {
  if (!dailyContentModule) {
    dailyContentModule = require('./daily-content-module');
  }
            ...
            ${visiblePdfs.map(pdf => `
              <article class="bg-white border border-border rounded-2xl p-5 shadow-sm flex flex-col gap-3 hover:border-gold transition">
                ...
                <div class="flex flex-wrap gap-2 mt-auto">
                  ...
                  ${pdfResourceHasFile(pdf) ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-[10px] text-white bg-gold rounded-lg px-3 py-2 font-bold hover:bg-maroon transition">${pdfIconHtml('w-5 h-5')} Descargar</a>` : ''}
                </div>
                <div class="pt-2 border-t border-border/40">
            ...
            ${visiblePdfs.map(pdf => {
              ...
              <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
                ...
                <div class="flex gap-2 mt-auto pt-2">
                  ...
                  ${pdfResourceHasFile(pdf) ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-[10px] text-white bg-maroon rounded-lg px-3 py-2 font-bold hover:bg-gold transition">${pdfIconHtml('w-5 h-5')} Descargar</a>` : ''}
                </div>
                <div class="pt-2 border-t border-border/40">
  ...
  const html = `
    <article class="max-w-4xl mx-auto w-full px-4 py-10 flex flex-col gap-6">
      ...
      <header class="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-4">
        ...
        ${pdfResourceHasFile(pdf) ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="self-start inline-flex items-center gap-2 bg-maroon hover:bg-gold text-white font-bold px-5 py-3 rounded-xl transition text-sm">${pdfIconHtml('w-6 h-6')} Descargar PDF</a>` : `<p class="text-sm text-red-800">Este recurso aún no tiene PDF enlazado.</p>`}
        <div class="pt-3 border-t border-border/40">
            ...
            ${(pdfCatalog.recursos || []).length ? (pdfCatalog.recursos || []).map(pdf => `
              <div class="pdf-admin-card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3" data-search="${escapeHtml([pdf.titulo, pdf.descripcion, pdf.categoria, pdf.audiencia, pdf.keywords].join(' ').toLowerCase())}">
                ...
                <div class="flex flex-wrap gap-2 shrink-0">
                  ...
                  ${pdfResourceHasFile(pdf) ? `<a href="/catequesis-ia/recursos/${escapeHtml(pdf.slug)}/descargar" target="_blank" class="inline-flex items-center gap-1 text-gold border border-gold px-3 py-1 rounded-lg font-bold hover:bg-gold hover:text-white transition">${pdfIconHtml('w-4 h-4')} PDF</a>` : ''}
