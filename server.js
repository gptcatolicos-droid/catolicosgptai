...
function getPdfSourceCandidates(pdf = {}) {
  ...
  const rawPublicId = String(pdf.cloudinaryPublicId || '').trim();
  const format = String(pdf.format || 'pdf').replace(/^\./, '') || 'pdf';
  const publicIdNoExt = rawPublicId.replace(new RegExp(`\\.${format}$`, 'i'), '');

  if (cleanUrl) candidates.push(cleanUrl);
  if (originalUrl && originalUrl !== cleanUrl) candidates.push(originalUrl);
  if (publicIdNoExt) {
    candidates.push(`https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/raw/upload/${publicIdNoExt}.${format}`);
...
app.get('/catequesis-ia/recursos/:slug/descargar', async (req, res) => {
  ...
  const candidates = getPdfSourceCandidates(pdf);
  let lastError = '';
  ...
  const html = `
    ...
    <script>
      ...
      function resetPdfForm() {
        const ids = ['pdf_original_slug', 'pdf_cloudinary_public_id', 'pdf_cloudinary_resource_type', 'pdf_format', 'pdf_bytes', 'pdf_titulo', 'pdf_categoria', 'pdf_url', 'pdf_descripcion', 'pdf_keywords'];
        ids.forEach(id => {
