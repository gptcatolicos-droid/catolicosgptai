...
function cloudinaryPdfDeliveryUrl(pdf = {}, { attachment = false } = {}) {
  const cleanFilename = recursosPdf.slugify(pdf.titulo || pdf.slug || 'catolicosgpt-recurso');
  const publicId = String(pdf.cloudinaryPublicId || '').replace(/\.pdf$/i, '');
  const format = String(pdf.format || 'pdf').replace(/^\./, '') || 'pdf';
  if (publicId) {
    const transformation = attachment ? `fl_attachment:${cleanFilename}/` : '';
    return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/raw/upload/${transformation}${publicId}.${format}`;
  }
  const url = String(pdf.pdfUrl || '').trim();
  if (!url) return '';
  if (!attachment) return url;
  return cloudinaryAttachmentUrl(url, cleanFilename);
}
            ...
            ${visiblePdfs.map(pdf => `
              <article class="bg-white border border-border rounded-2xl p-5 shadow-sm flex flex-col gap-3 hover:border-gold transition">
                ...
                <div class="flex flex-wrap gap-2 mt-auto">
                  ...
                  ${pdf.pdfUrl ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-[10px] text-white bg-gold rounded-lg px-3 py-2 font-bold hover:bg-maroon transition">${pdfIconHtml('w-5 h-5')} Descargar</a>` : ''}
                </div>
              </article>
            `).join('')}
          </div>
            ...
            ${visiblePdfs.map(pdf => `
              <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
                ...
                <div class="flex gap-2 mt-auto pt-2">
                  ...
                  ${pdf.pdfUrl ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-[10px] text-white bg-maroon rounded-lg px-3 py-2 font-bold hover:bg-gold transition">${pdfIconHtml('w-5 h-5')} Descargar</a>` : ''}
                </div>
              </article>
            `).join('')}
          </div>
        ...
        ${visiblePosts.length ? `
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ${visiblePosts.slice(0, 24).map(p => `
              <a href="/blog/${p.slug}" class="seo-card flex flex-col gap-3 hover:border-gold transition">
                ...
                <span class="text-[10px] text-maroon font-bold mt-auto">Leer guía →</span>
              </a>
            `).join('')}
          </div>
        ...
        ${visibleInfografias.length ? `
          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ${visibleInfografias.slice(0, 18).map(i => {
              const cover = (i.imagenes || []).find(img => img.esPortada) || (i.imagenes || [])[0] || {};
              return `
                <a href="/infografias/${i.slug}" class="seo-card flex flex-col gap-3 hover:border-gold transition">
                  ...
                  <span class="text-[10px] text-maroon font-bold mt-auto">Ver recurso →</span>
                </a>
              `;
            }).join('')}
          </div>
...
app.get('/catequesis-ia/recursos/:slug/descargar', async (req, res) => {
  const pdf = recursosPdf.getBySlug(req.params.slug);
  if (!pdf || pdf.publicado === false || !pdf.pdfUrl) {
  ...
  const deliveryUrl = cloudinaryPdfDeliveryUrl(pdf, { attachment: true }) || pdf.pdfUrl;
            ...
            ${(pdfCatalog.recursos || []).length ? (pdfCatalog.recursos || []).map(pdf => `
              <div class="pdf-admin-card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3" data-search="${escapeHtml([pdf.titulo, pdf.descripcion, pdf.categoria, pdf.audiencia, pdf.keywords].join(' ').toLowerCase())}">
                ...
                <div class="flex flex-wrap gap-2 shrink-0">
                  ...
                  ${pdf.pdfUrl ? `<a href="/catequesis-ia/recursos/${escapeHtml(pdf.slug)}/descargar" target="_blank" class="inline-flex items-center gap-1 text-gold border border-gold px-3 py-1 rounded-lg font-bold hover:bg-gold hover:text-white transition">${pdfIconHtml('w-4 h-4')} PDF</a>` : ''}
