...
try {
  ...
  console.warn('[Startup] dotenv no disponible; Cloud Run usará variables de entorno nativas.');
}
...
function safeModule(name, defaults = {}) {
  return new Proxy(defaults, {
    get(target, prop) {
      if (prop in target) return target[prop];
      if (prop === 'then') return undefined;
      return (...args) => {
        console.warn(`[Startup] ${name}.${String(prop)} no disponible; respuesta vacía para mantener el servidor activo.`);
        return null;
      };
    }
  });
...
function fallbackSlug(value = 'catolicosgpt') {
  return String(value || 'catolicosgpt')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 90) || 'catolicosgpt';
}

const fallbackAuth = safeModule('auth', {
  getUserById: () => null,
  login: async () => ({ error: 'Modulo de autenticacion no disponible.' }),
  register: async () => ({ error: 'Modulo de autenticacion no disponible.' }),
  updateUser: () => null,
  upgradePlan: () => null
});

const fallbackInfografias = safeModule('infografias', {
  loadCatalog: () => ({ infografias: [], total: 0 }),
  saveCatalog: () => ({ infografias: [], total: 0 }),
  getInfografias: () => ({ items: [], total: 0, totalPages: 0 }),
  getInfografiaDelDia: () => null,
  getInfografiaBySlug: () => null,
  generateSlug: fallbackSlug,
  updateInfografia: () => null,
  deleteInfografia: () => false,
  setInfografiaDelDia: () => false,
  reorderInfografias: () => ({ infografias: [], total: 0 })
});

const fallbackLiturgia = safeModule('liturgia', {
  init: async () => ({}),
  refreshLiturgia: async () => ({}),
  get: () => null,
  todayBogota: () => new Date().toISOString().slice(0, 10)
});

const fallbackBlog = safeModule('blog', {
  loadBlog: () => ({ posts: [], total: 0 }),
  getPosts: () => ({ items: [], total: 0, totalPages: 0 }),
  getPostBySlug: () => null,
  ...
  upsertPost: () => null,
  deletePost: () => false,
  evaluarYCrearArticuloSEO: async () => null
});

const fallbackVideos = safeModule('videos', {
  loadVideos: () => ({ videos: [], total: 0 }),
  saveVideos: () => ({ videos: [], total: 0 }),
  getVideos: () => [],
  getVideoBySlug: () => null,
  deleteVideo: () => false,
  reorderVideos: () => ({ videos: [], total: 0 })
});

const fallbackPodcast = safeModule('podcast', {
  loadPodcasts: () => ({ podcasts: [], total: 0 }),
  savePodcasts: () => ({ podcasts: [], total: 0 }),
  getPodcasts: () => [],
  getPodcastBySlug: () => null,
  deletePodcast: () => false,
  reorderPodcasts: () => ({ podcasts: [], total: 0 })
});

const fallbackSeo = safeModule('seo', {
  generateSitemapXML: () => '<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"></urlset>',
  generateRSSFeed: () => '<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>CatolicosGPT</title></channel></rss>'
});

// Importar todos los subsistemas creados
const auth          = optionalRequire('./auth-module', fallbackAuth);
const infografias  = optionalRequire('./infografias-module', fallbackInfografias);
const liturgia      = optionalRequire('./liturgia-cache', fallbackLiturgia);
const misas         = optionalRequire('./misas-module', safeModule('misas', { getMisasCanales: () => ({}) }));
const blog          = optionalRequire('./blog-module', fallbackBlog);
const podcast       = optionalRequire('./podcast-module', fallbackPodcast);
const videos        = optionalRequire('./videos-module', fallbackVideos);
const recursos      = optionalRequire('./recursos-module', safeModule('recursos', { consultarRecursosLocales: () => [] }));
const recursosPdf   = optionalRequire('./recursos-pdf-module', safeModule('recursosPdf', { getRecursos: () => ({ items: [], total: 0 }), getBySlug: () => null }));
const seo           = optionalRequire('./seo-module', fallbackSeo);
const seoTopics     = optionalRequire('./seo-topics', safeModule('seoTopics', { getTemasSEO: () => [], getTemaSEBySlug: () => null }));
const biblia        = optionalRequire('./biblia-module', safeModule('biblia', { detectarSolicitudBiblica: () => null, renderizarCitaAsync: async () => null, obtenerCitaAsync: async () => null }));
const advancedEngine = optionalRequire('./advanced-query-engine', safeModule('advancedEngine', {
  buscarEnCacheDoctrinal: () => null,
  esConsultaCombinada: () => false,
  esConsultaBiblica: () => false,
  esConsultaCatecismo: () => false,
  ejecutarModoCombinado: async () => false,
  ejecutarModoBiblicoAvanzado: async () => false,
  ejecutarModoCatecismo: async () => false
}));
...
try {
  ...
  console.warn('[Startup] @google/genai no disponible; Gemini queda desactivado y el servidor sigue activo.');
}
...
function shareButtonsHtml({ title = 'CatólicosGPT', path = '/', description = '', compact = false, label = 'Compartir' } = {}) {
  ...
  const buttonBase = compact
    ? 'inline-flex items-center justify-center gap-1.5 rounded-full px-2.5 py-1.5 text-[10px] font-bold transition border'
    : 'inline-flex items-center justify-center gap-2 rounded-xl px-3.5 py-2 text-xs font-bold transition border';
  ...
  return `
    <div class="catolicos-share flex ${compact ? 'items-center gap-1.5 flex-wrap' : 'flex-col gap-2'}" aria-label="${escapeHtml(label)}">
      ...
      <div class="flex flex-wrap gap-2">
        <a href="https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}" target="_blank" rel="noopener noreferrer" onclick="window.open(this.href,'cgptShareFb','width=640,height=520,noopener,noreferrer'); return false;" class="${buttonBase} bg-[#1877F2] border-[#1877F2] text-white hover:brightness-95" title="Compartir en Facebook">
        ...
        <a href="https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedText}" target="_blank" rel="noopener noreferrer" onclick="window.open(this.href,'cgptShareX','width=640,height=520,noopener,noreferrer'); return false;" class="${buttonBase} bg-espresso border-espresso text-white hover:bg-maroon" title="Compartir en X">
          <span class="${iconClass} inline-flex items-center justify-center font-sans font-black text-[13px] leading-none">X</span>
          X
        </a>
        <a href="https://api.whatsapp.com/send?text=${encodedText}%20${encodedUrl}" target="_blank" rel="noopener noreferrer" class="${buttonBase} bg-[#25D366] border-[#25D366] text-white hover:brightness-95" title="Compartir en WhatsApp">
...
app.get('/catequesis-ia', (req, res) => {
  ...
  const pdfCatalog = recursosPdf.getRecursos({ audiencia: audience, limit: 60 });
  ...
  const visiblePosts = posts.filter(filterAudience);
  const visibleInfografias = catequesisInfografias.filter(filterAudience);
  const visiblePdfs = pdfCatalog.items || [];
  ...
  const html = `
    <div class="max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-7">
      ...
      <nav class="bg-white border border-border rounded-xl p-4 flex flex-wrap gap-2">
        <a href="/catequesis-ia" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'todo' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">Todo</a>
        <a href="/catequesis-ia?audiencia=general" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'general' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">General</a>
        <a href="/catequesis-ia?audiencia=ninos" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'ninos' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">Niños</a>
        <a href="/catequesis-ia?audiencia=jovenes" class="px-4 py-2 rounded-full text-xs font-bold ${audience === 'jovenes' ? 'bg-maroon text-white' : 'bg-cream border text-ink hover:bg-cream2'}">Jóvenes</a>
      </nav>
      ...
      <section class="flex flex-col gap-4">
        ...
        ${visibleInfografias.length ? `
          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ${visibleInfografias.slice(0, 18).map(i => {
              const cover = (i.imagenes || []).find(img => img.esPortada) || (i.imagenes || [])[0] || {};
              return `
                <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
    ...
    <div class="max-w-4xl mx-auto w-full px-4 py-8 flex flex-col gap-6 animate-fade-in">
      ...
      <div class="bg-white border rounded-2xl p-6 shadow-sm flex flex-col gap-4 sacred-border">
        ...
        <div class="flex gap-2.5 mt-1 flex-wrap">
          ...
          <a id="share-x-btn" href="#" target="_blank" class="bg-espresso hover:bg-maroon text-white px-4 py-2 rounded-full text-xs font-bold transition flex items-center gap-1.5">
