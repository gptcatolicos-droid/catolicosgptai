        ...
        <div class="bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          ...
          <form method="POST" action="/admin/recursos-pdf/guardar" id="pdfResourceForm" class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div class="flex flex-col gap-2 md:col-span-3 bg-cream/40 border border-dashed rounded-xl p-4">
              <label class="font-semibold text-espresso text-xs">Biblioteca Cloudinary de PDFs</label>
              <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                <div class="flex items-center gap-3 flex-1 min-w-0 bg-white border border-border rounded-xl px-3 py-2">
                  ...
                  <div class="min-w-0">
                    ...
                    <div class="text-[10px] text-ink2">Carpeta sugerida: catolicosgpt/recursos-pdf</div>
                  </div>
                </div>
                <button type="button" onclick="openCloudinaryExplorer('recursos-pdf')" class="bg-maroon hover:bg-gold text-white px-4 py-2 rounded-lg font-bold text-xs transition cursor-pointer border-0">Seleccionar PDF</button>
              </div>
              <p id="pdf_upload_status" class="text-[11px] text-ink2 italic">El título se detecta desde el nombre del archivo y OpenAI puede completar descripción y keywords.</p>
            </div>
      ...
      async function uploadPdfToCloudinary(btn) {
        ...
        try {
          ...
          if (!document.getElementById('pdf_titulo').value) {
            document.getElementById('pdf_titulo').value = file.name.replace(/\\.pdf$/i, '').replace(/[-_]+/g, ' ');
          }
          if (status) status.textContent = 'PDF subido correctamente a catolicosgpt/recursos-pdf.';
        } catch (err) {
