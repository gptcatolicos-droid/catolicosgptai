...
app.get('/', (req, res) => {
  ...
  const html = `
    <div class="max-w-[98%] mx-auto w-[99%] sm:w-full px-1 py-1 sm:px-4 sm:py-6 flex flex-col h-[58svh] min-h-[470px] sm:h-[calc(100vh-80px)] overflow-hidden">
      ...
      <div class="flex-1 flex flex-col bg-white border border-[#E6DFD4] rounded-2xl shadow-sm overflow-hidden h-full">
        ...
        <!-- CHAT BOX MESSAGES -->
        <div id="chat-box" class="flex-1 overflow-y-auto p-2 sm:p-5 flex flex-col gap-3 bg-[#FAF9F5]">
          ...
          <div id="welcome-screen" class="flex-1 flex flex-col items-center justify-start sm:justify-center text-center pt-4 pb-2 sm:py-6 max-w-2xl mx-auto gap-3 sm:gap-6 my-0 sm:my-auto">
            ...
            <div class="grid grid-cols-2 gap-2 sm:gap-5 w-full max-w-xl px-2 sm:px-4 mt-1 sm:mt-4">
              <!-- Card Santo del Día -->
              <a href="${todaySaintTarget.path}" data-full-page="1" class="group flex flex-col items-center sm:items-start text-center sm:text-left p-3 sm:p-6 bg-white border border-[#E6DFD4] hover:border-gold/50 rounded-xl sm:rounded-2xl shadow-xs hover:shadow-md transition duration-300 gap-2 sm:gap-3.5 min-h-[118px] sm:min-h-0">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-amber-50 text-gold flex items-center justify-center border border-amber-200 group-hover:scale-105 transition shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-cross"><path d="M11 2h2v7h6v2h-6v11h-2v-11H5V9h6V2z"/></svg>
                </div>
                <div class="flex flex-col gap-1 w-full">
                  <h3 class="font-display font-semibold text-[9px] sm:text-[10px] text-gold uppercase tracking-wider font-mono">Santo del Día</h3>
                  <p class="font-serif font-bold text-xs sm:text-sm text-espresso line-clamp-2 sm:line-clamp-1 group-hover:text-maroon transition leading-tight">${todaySaintTarget.nombre || (dSanto ? dSanto.nombre : 'Vida de los Santos')}</p>
              ...
              <a href="/infografia-del-dia" class="group flex flex-col items-center sm:items-start text-center sm:text-left p-3 sm:p-6 bg-white border border-[#E6DFD4] hover:border-gold/50 rounded-xl sm:rounded-2xl shadow-xs hover:shadow-md transition duration-300 gap-2 sm:gap-3.5 min-h-[118px] sm:min-h-0">
                <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-rose-50 text-maroon flex items-center justify-center border border-rose-150 group-hover:scale-105 transition shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-image"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>
                </div>
                <div class="flex flex-col gap-1 w-full">
                  <h3 class="font-display font-semibold text-[9px] sm:text-[10px] text-maroon uppercase tracking-wider font-mono">Infografía del Día</h3>
                  <p class="font-serif font-bold text-xs sm:text-sm text-espresso line-clamp-2 sm:line-clamp-1 group-hover:text-gold transition leading-tight">${infografiaDelDia ? (infografiaDelDia.titulo || infografiaDelDia.tema) : 'Formación en Imágenes'}</p>
        ...
        <!-- CHAT INPUT WRAP -->
        <div class="p-2.5 sm:p-4 border-t border-border bg-white shadow-inner">
...
function obtenerRecursosRelacionados(query, opts = {}) {
  if (!query) return { infografias: [], blogs: [], videos: [], intent: null };
  ...
  if (finalInfs.length === 0 && !opts.directOnly && !explicitVideo) {
    finalInfs = allInfs.slice(0, 2);
  }

  let finalBlogs = scoredBlogs.slice(0, 2);
  if (finalBlogs.length === 0 && !opts.directOnly && !explicitInfografia && !explicitVideo) {
    finalBlogs = allBlogs.slice(0, 2);
  }

  let finalVideos = scoredVideos.slice(0, explicitVideo ? 4 : 1);
  if (finalVideos.length === 0 && !opts.directOnly && !explicitInfografia) {
    finalVideos = allVideos.slice(0, 1);
  }

  return {
    intent: explicitInfografia ? 'infografia' : explicitVideo ? 'video' : null,
...
function renderRelacionadosHtml(recursosObj) {
  const { infografias: infs, blogs, videos: vids } = recursosObj;
  const total = (infs ? infs.length : 0) + (blogs ? blogs.length : 0) + (vids ? vids.length : 0);
  if (total === 0) return '';
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // ── INTERCEPTOR DIRECTO: "INFOGRAFÍA DE..." / "VIDEO DE..." ──
...
app.get('/catequesis-ia', (req, res) => {
  ...
  const labelFor = cat => cat === 'catequesis-ninos' ? 'Niños' : cat === 'catequesis-jovenes' ? 'Jóvenes' : 'Catequesis';
  ...
  const html = `
    <div class="max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-7">
      <header class="flex flex-col gap-2">
        ...
        <h1 class="font-display font-bold text-3xl text-maroon tracking-wide">Catequesis para Niños y Jóvenes</h1>
        <p class="font-serif text-ink2 text-sm italic max-w-3xl">Guías prácticas, recursos visuales e itinerarios de formación católica para familias, colegios, parroquias y grupos juveniles.</p>
      </header>
