...
function pdfResourceHasFile(pdf = {}) {
  return Boolean(String(pdf.pdfUrl || '').trim() || String(pdf.cloudinaryPublicId || '').trim());
}
  ...
  const html = `
          ...
          <form method="POST" action="/admin/recursos-pdf/guardar" id="pdfResourceForm" class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div class="flex flex-col gap-1.5 md:col-span-2">
              <div class="flex items-center justify-between gap-2">
                ...
                <span class="text-[10px] text-ink2">Se llena al seleccionar el PDF desde Cloudinary.</span>
              </div>
              <input type="url" name="pdfUrl" id="pdf_url" placeholder="https://res.cloudinary.com/.../archivo.pdf" class="border border-border rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-gold text-xs bg-white">
            </div>
            ...
            <div class="flex flex-col gap-2 md:col-span-3 bg-cream/40 border border-dashed rounded-xl p-4">
              <label class="font-semibold text-espresso text-xs">Subir o seleccionar PDF en Cloudinary RAW</label>
              ...
              <p id="pdf_upload_status" class="text-[11px] text-ink2 italic">Recomendado: usa “Subir PDF RAW”. Al finalizar, CatólicosGPT completa título, descripción SEO y keywords con OpenAI.</p>
            </div>
    ...
    <script>
      ...
      function resetPdfForm() {
        const ids = ['pdf_original_slug', 'pdf_cloudinary_public_id', 'pdf_cloudinary_resource_type', 'pdf_cloudinary_version', 'pdf_cloudinary_asset_id', 'pdf_cloudinary_secure_url', 'pdf_format', 'pdf_bytes', 'pdf_titulo', 'pdf_categoria', 'pdf_url', 'pdf_cover_url', 'pdf_descripcion', 'pdf_keywords'];
        ids.forEach(id => {
        ...
        const status = document.getElementById('pdf_upload_status');
        if (status) status.textContent = 'Selecciona desde tu carpeta catolicosgpt/recursos-pdf de Cloudinary.';
      ...
      function editPdfResource(pdf) {
        ...
        const selectedName = document.getElementById('pdf_selected_name');
        if (selectedName) selectedName.textContent = pdf.cloudinaryPublicId ? pdf.cloudinaryPublicId.split('/').pop() + '.pdf' : (pdf.pdfUrl ? 'PDF enlazado' : 'Ningún PDF seleccionado');
        const status = document.getElementById('pdf_upload_status');
        if (status) status.textContent = 'Recurso cargado para edición. Puedes cambiar el PDF seleccionando otro desde Cloudinary.';
