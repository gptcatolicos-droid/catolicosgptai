...
<div class="shrink-0">${pdfIconHtml('w-9 h-9')}</div>
...
function cloudinaryPdfDeliveryUrl(pdf = {}, { attachment = false } = {}) {
  ...
  if (publicId) {
    const baseUrl = `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/${resourceType}/upload/${publicId}.${format}`;
...
function getPdfSourceCandidates(pdf = {}) {
  ...
  if (secureUrl) candidates.push(cleanCloudinaryPdfUrl(secureUrl));
  if (originalUrl && originalUrl !== cleanUrl) candidates.push(originalUrl);
  if (publicIdNoExt) {
    ...
    const signedUrl = getSignedCloudinaryPdfUrl(publicIdNoExt, format);
    if (signedUrl) candidates.push(signedUrl);
    const privateUrl = getPrivateCloudinaryPdfUrl(publicIdNoExt, format);
...
function getSignedCloudinaryPdfUrl(publicId = '', format = 'pdf') {
...
function getPrivateCloudinaryPdfUrl(publicId = '', format = 'pdf') {
  if (!publicId || !process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET) return '';
  try {
    const cloudinaryMod = require('cloudinary').v2;
    cloudinaryMod.config({
...
async function getCloudinaryVerifiedPdfCandidates(pdf = {}) {
  ...
  try {
    ...
    for (const lookupId of lookupIds) {
      try {
        resource = await cloudinaryMod.api.resource(lookupId, { resource_type: 'raw' });
        if (resource) break;
      } catch (err) {
        lookupError = err;
      }
    }
    if (!resource) throw lookupError || new Error('No se encontró el recurso raw en Cloudinary.');
    ...
    return [
      ...
      getSignedCloudinaryPdfUrl(resource.public_id || publicId, format),
      getPrivateCloudinaryPdfUrl(resource.public_id || publicId, format)
    ].filter(Boolean);
  } catch (err) {
...
app.get('/catequesis-ia/recursos/:slug/descargar', (req, res) => {
  const pdf = recursosPdf.getBySlug(req.params.slug);
  ...
  const directUrl = [
    pdf.cloudinarySecureUrl,
    pdf.pdfUrl,
    cleanCloudinaryPdfUrl(pdf.pdfUrl),
    cloudinaryPdfDeliveryUrl(pdf),
    cloudinaryPdfDeliveryUrl(pdf, { attachment: true })
  ...
  const sourceUrl = cloudinaryAttachmentUrl(directUrl, getPdfDownloadFilename(pdf).replace(/\.pdf$/i, ''));
  const filename = getPdfDownloadFilename(pdf);
  fetch(sourceUrl, { headers: { Accept: 'application/pdf,*/*' } })
    .then(upstream => {
    ...
    .catch(err => {
      console.error('[PDF Download] No se pudo servir el PDF desde Cloudinary', err.message, { slug: pdf.slug, sourceUrl });
      ...
      return res.status(502).send(renderPage('PDF no disponible', `
        <div class="max-w-xl mx-auto my-12 bg-white border border-border rounded-2xl p-8 text-center shadow-sm flex flex-col gap-4">
          ...
          <p class="text-sm text-ink2 leading-relaxed">El registro existe, pero la URL pública de Cloudinary no devolvió un PDF descargable. En el admin selecciona el PDF desde la biblioteca, no pegues la URL interna de la consola.</p>
          <a href="/catequesis-ia/recursos/${escapeHtml(pdf.slug)}" class="inline-flex justify-center bg-maroon text-white font-bold px-5 py-3 rounded-xl">Volver al recurso</a>
        </div>
      `, req));
    });
  ...
  const html = `
    ...
    <script>
      ...
      function resetPdfForm() {
        const ids = ['pdf_original_slug', 'pdf_cloudinary_public_id', 'pdf_cloudinary_resource_type', 'pdf_cloudinary_version', 'pdf_cloudinary_asset_id', 'pdf_cloudinary_secure_url', 'pdf_format', 'pdf_bytes', 'pdf_titulo', 'pdf_categoria', 'pdf_url', 'pdf_descripcion', 'pdf_keywords'];
        ids.forEach(id => {
      ...
      function toggleSelectResource(url, idx) {
        ...
        if (selectedResourcesMap.has(url)) {
          ...
          const singleSelectContexts = ['blog_cover', 'santo_photo', 'videos', 'podcasts', 'recursos-pdf'];
...
app.post('/api/admin/recursos-pdf/upload', async (req, res) => {
  ...
  try {
    ...
    const result = await cloudinaryMod.uploader.upload(dataUrl, {
      ...
      public_id: `${safeBase}-${Date.now()}.pdf`,
      use_filename: true,
...
app.get('/api/admin/cloudinary/resources', async (req, res) => {
  ...
  try {
    ...
    const resourcesFormatted = (searchResult.resources || []).map(r => {
      ...
      const isPdf = format === 'pdf' && ['raw', 'image'].includes(r.resource_type);
