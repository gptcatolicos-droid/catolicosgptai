...
function cloudinaryPdfDeliveryUrl(pdf = {}, { attachment = false } = {}) {
  ...
  if (publicId) {
    return `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/raw/upload/${publicId}.${format}`;
  }
  const url = String(pdf.pdfUrl || '').trim();
  return url || '';
}
...
function cleanCloudinaryPdfUrl(url = '') {
  ...
  if (!raw) return '';
  if (!raw.includes('res.cloudinary.com') || !raw.includes('/raw/upload/')) return raw;
  return raw
    .replace(/\/raw\/upload\/fl_attachment:[^/]+\//, '/raw/upload/')
    .replace(/\/raw\/upload\/f_[^/]+,fl_attachment:[^/]+\//, '/raw/upload/');
}
...
app.get('/catequesis-ia/recursos/:slug/descargar', async (req, res) => {
  ...
  for (const sourceUrl of candidates) {
    try {
      ...
      if (!upstream.ok || !upstream.body) {
        lastError = `Cloudinary respondió ${upstream.status}`;
        continue;
      }
  ...
  console.error('[PDF Download]', lastError || 'No se pudo leer el PDF desde Cloudinary', { slug: pdf.slug, publicId: pdf.cloudinaryPublicId, pdfUrl: pdf.pdfUrl });
  return res.status(502).send(renderPage('PDF no disponible', `
    <div class="max-w-xl mx-auto my-12 bg-white border border-border rounded-2xl p-8 text-center shadow-sm flex flex-col gap-4">
      ...
      <h1 class="font-display text-maroon text-2xl font-bold">No se pudo entregar el PDF desde CatólicosGPT</h1>
      <p class="text-sm text-ink2 leading-relaxed">El registro existe, pero Cloudinary no entregó un archivo PDF válido. Revisa que el PDF seleccionado sea público y que el archivo no haya sido renombrado o movido en Cloudinary.</p>
...
app.post('/admin/recursos-pdf/guardar', (req, res) => {
  ...
  try {
    recursosPdf.upsertResource({
      ...
      cloudinarySecureUrl: req.body.cloudinarySecureUrl,
...
app.get('/api/admin/cloudinary/resources', async (req, res) => {
  ...
  try {
    ...
    const resourcesFormatted = (searchResult.resources || []).map(r => {
      ...
      const isPdf = r.resource_type === 'raw' && format === 'pdf';
      const publicId = String(r.public_id || '').replace(/\.pdf$/i, '');
      const rawPdfUrl = isPdf
        ? `https://res.cloudinary.com/${CLOUDINARY_CLOUD_NAME}/raw/upload/${publicId}.pdf`
        : '';
      return {
