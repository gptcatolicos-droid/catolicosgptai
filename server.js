            ...
            ${visiblePdfs.map(pdf => `
              <article class="bg-white border border-border rounded-2xl p-5 shadow-sm flex flex-col gap-3 hover:border-gold transition">
                ...
                <div class="flex flex-wrap gap-2 mt-auto">
                  ...
                  ${pdf.pdfUrl ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="text-[10px] text-white bg-gold rounded-lg px-3 py-2 font-bold hover:bg-maroon transition">Descargar PDF</a>` : ''}
                </div>
              </article>
            `).join('')}
          </div>
            ...
            ${visiblePdfs.map(pdf => `
              <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
                ...
                <div class="flex gap-2 mt-auto pt-2">
                  ...
                  ${pdf.pdfUrl ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="text-[10px] text-white bg-maroon rounded-lg px-3 py-2 font-bold hover:bg-gold transition">Descargar PDF</a>` : ''}
                </div>
              </article>
            `).join('')}
          </div>
  ...
  const html = `
    <article class="max-w-4xl mx-auto w-full px-4 py-10 flex flex-col gap-6">
      ...
      <header class="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-4">
        ...
        ${pdf.pdfUrl ? `<a href="/catequesis-ia/recursos/${pdf.slug}/descargar" target="_blank" rel="noopener" class="self-start bg-maroon hover:bg-gold text-white font-bold px-5 py-3 rounded-xl transition text-sm">Descargar PDF</a>` : `<p class="text-sm text-red-800">Este recurso aún no tiene PDF enlazado.</p>`}
      </header>
      <section class="bg-white border border-border rounded-2xl p-6 shadow-sm">
...
app.get('/catequesis-ia/recursos/:slug/descargar', async (req, res) => {
  ...
  const filename = `${recursosPdf.slugify(pdf.titulo || pdf.slug)}.pdf`;
  const candidates = [pdf.pdfUrl];

  if (pdf.cloudinaryPublicId && process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) {
    try {
      ...
      candidates.push(cloudinaryMod.utils.private_download_url(String(pdf.cloudinaryPublicId).replace(/\.pdf$/i, ''), 'pdf', {
        resource_type: pdf.cloudinaryResourceType || 'raw',
        type: 'upload',
        attachment: true,
        expires_at: Math.floor(Date.now() / 1000) + 300
      }));
    } catch (e) {
      console.warn('[PDF Download] No se pudo generar URL firmada:', e.message);
    }
  }
  for (const url of [...new Set(candidates.filter(Boolean))]) {
    try {
      ...
      console.warn('[PDF Download] Falló candidato Cloudinary:', e.message);
    }
  }

  return res.status(502).send(renderPage('No se pudo cargar el PDF', `
    <div class="max-w-xl mx-auto p-8 my-10 bg-white border border-border rounded-2xl text-center flex flex-col gap-4">
      <p class="text-gold font-mono text-xs uppercase tracking-[0.18em] font-bold">Recurso PDF</p>
      <h1 class="font-display text-maroon text-2xl font-bold">No se pudo cargar el PDF desde CatólicosGPT</h1>
      <p class="text-ink2 text-sm leading-relaxed">El registro existe y está publicado, pero el servidor recibió desde Cloudinary una respuesta que no es un PDF válido. Puedes abrir el archivo original mientras ajustamos la entrega pública del documento.</p>
      <div class="flex flex-col sm:flex-row gap-2 justify-center">
        <a href="${escapeHtml(pdf.pdfUrl)}" target="_blank" rel="noopener" class="bg-maroon hover:bg-gold text-white px-5 py-3 rounded-xl font-bold text-sm transition">Abrir archivo original</a>
        <a href="/catequesis-ia/recursos/${pdf.slug}" class="border border-maroon text-maroon hover:bg-maroon hover:text-white px-5 py-3 rounded-xl font-bold text-sm transition">Volver al recurso</a>
      </div>
    </div>
  `, req));
});
  ...
  const html = `
      ...
      <div id="tab-content-recursos-pdf" class="tab-pane hidden flex flex-col gap-6">
        <div class="bg-white border border-border rounded-2xl p-6 shadow-sm flex flex-col gap-4">
          <div class="border-b pb-3">
            ...
            <p class="text-ink-2 text-xs leading-relaxed mt-1">Sube PDFs públicos a Cloudinary en la carpeta <code class="font-mono bg-cream-2 px-1 text-maroon">catolicosgpt/recursos-pdf</code>. Luego aparecen en Catequesis IA y pueden ser recomendados por el chat.</p>
          </div>
          ...
          <form method="POST" action="/admin/recursos-pdf/guardar" id="pdfResourceForm" class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs sm:text-sm">
            ...
            <div class="flex flex-col gap-1.5 md:col-span-2">
              <div class="flex items-center justify-between gap-2">
                ...
                <span class="text-[10px] text-ink2">Se llena automáticamente al subir el archivo.</span>
              </div>
            ...
            <div class="flex flex-col gap-2 md:col-span-3 bg-cream/40 border border-dashed rounded-xl p-4">
              <label class="font-semibold text-espresso text-xs">Subir PDF a Cloudinary</label>
              <div class="flex flex-col sm:flex-row gap-2">
                <input type="file" id="pdf_file_input" accept="application/pdf,.pdf" class="border border-border rounded-lg px-3 py-2 text-xs bg-white flex-1">
                <button type="button" onclick="uploadPdfToCloudinary(this)" class="bg-maroon hover:bg-gold text-white px-4 py-2 rounded-lg font-bold text-xs transition cursor-pointer border-0">☁️ Subir PDF</button>
              </div>
              <p id="pdf_upload_status" class="text-[11px] text-ink2 italic">Carpeta destino: catolicosgpt/recursos-pdf</p>
            </div>
            ...
            ${(pdfCatalog.recursos || []).length ? (pdfCatalog.recursos || []).map(pdf => `
              <div class="pdf-admin-card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3" data-search="${escapeHtml([pdf.titulo, pdf.descripcion, pdf.categoria, pdf.audiencia, pdf.keywords].join(' ').toLowerCase())}">
                ...
                <div class="flex flex-wrap gap-2 shrink-0">
                  ...
                  ${pdf.pdfUrl ? `<a href="${pdf.pdfUrl}" target="_blank" class="text-gold border border-gold px-3 py-1 rounded-lg font-bold hover:bg-gold hover:text-white transition">PDF</a>` : ''}
    ...
    <script>
      ...
      function resetPdfForm() {
        const ids = ['pdf_original_slug', 'pdf_cloudinary_public_id', 'pdf_bytes', 'pdf_titulo', 'pdf_categoria', 'pdf_url', 'pdf_descripcion', 'pdf_keywords'];
        ids.forEach(id => {
          const el = document.getElementById(id);
          if (el) el.value = id === 'pdf_categoria' ? 'catequesis' : '';
        });
  ...
  try {
    ...
    if (type && type !== 'all') {
      if (type === 'audio') {
        ...
        expressions.push('resource_type:raw');
      } else {
