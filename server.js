...
app.get('/catequesis-ia/recursos/:slug/descargar', async (req, res) => {
  ...
  if (pdf.cloudinaryPublicId && process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) {
    try {
      ...
      candidates.push(cloudinaryMod.utils.private_download_url(pdf.cloudinaryPublicId, 'pdf', {
  ...
  return res.status(502).send(renderPage('PDF bloqueado por Cloudinary', `
    <div class="max-w-xl mx-auto p-8 my-10 bg-white border border-border rounded-2xl text-center flex flex-col gap-4">
      <h1 class="font-display text-maroon text-2xl font-bold">Cloudinary bloqueó la entrega del PDF</h1>
      <p class="text-ink2 text-sm leading-relaxed">El recurso existe en la biblioteca, pero Cloudinary respondió con una restricción de seguridad para archivos PDF. El registro queda guardado en CatólicosGPT; solo falta habilitar la entrega pública de PDFs o usar otra fuente de almacenamiento para documentos.</p>
      <a href="/catequesis-ia/recursos/${pdf.slug}" class="text-maroon font-bold underline">Volver al recurso</a>
    </div>
  `, req));
});
