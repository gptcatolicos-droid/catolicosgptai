...
AUTHORITY_SEO_PAGES
  .filter(page => page.path !== '/ia-catolica')
  .forEach(page => {
...
<span class="inline-block text-[9px] font-bold text-maroon bg-cream/80 border border-maroon/10 px-2 py-0.5 rounded font-mono uppercase tracking-wider mb-1.5">&#x1F4C4; Guía PDF</span>
            ...
            ${visiblePdfs.map(pdf => `
              <article class="seo-card flex flex-col gap-3 hover:border-gold transition">
                <div class="flex items-center justify-between gap-2">
                  <span class="text-[10px] font-bold text-gold font-mono uppercase tracking-widest">${labelFor(pdf.audiencia)}</span>
                ...
                <div class="flex gap-2 mt-auto pt-2">
                  ...
                  ${pdf.pdfUrl ? `<a href="${pdf.pdfUrl}" target="_blank" rel="noopener" class="text-[10px] text-white bg-maroon rounded-lg px-3 py-2 font-bold hover:bg-gold transition">Descargar PDF</a>` : ''}
                </div>
              </article>
            `).join('')}
          </div>
  ...
  const html = `
    <article class="max-w-4xl mx-auto w-full px-4 py-10 flex flex-col gap-6">
      ...
      <header class="bg-white border border-border rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-4">
        ...
        ${pdf.pdfUrl ? `<a href="${pdf.pdfUrl}" target="_blank" rel="noopener" class="self-start bg-maroon hover:bg-gold text-white font-bold px-5 py-3 rounded-xl transition text-sm">Descargar PDF</a>` : `<p class="text-sm text-red-800">Este recurso aún no tiene PDF enlazado.</p>`}
      </header>
      <section class="bg-white border border-border rounded-2xl p-6 shadow-sm">
    ...
    <div class="admin-shell max-w-6xl mx-auto w-full px-4 py-8 flex flex-col gap-6">
      ...
      <div class="admin-tabs flex border-b border-[#E6DFD4] overflow-x-auto whitespace-nowrap gap-1">
        ...
        <button onclick="switchTab('recursos-pdf')" id="tab-btn-recursos-pdf" class="tab-btn px-5 py-3 font-semibold text-sm border-b-2 border-transparent text-ink-2 hover:text-maroon transition flex items-center gap-2">
          📄 PDFs (${(pdfCatalog.recursos || []).length})
        </button>
