...
function createBudget(directory=process.env.AGENT_BUDGET_DIR||process.env.DATA_DIR||path.join(__dirname,'data')){
 ...
 function admit(client,{unlimited=false,limit,unmetered=false}={}){const day=new Date().toISOString().slice(0,10),month=day.slice(0,7);return transaction(s=>{
  ...
  if(!unmetered && (m.usd>=number('OPENAI_AGENT_MONTHLY_BUDGET_USD',30)||d.usd>=number('OPENAI_AGENT_DAILY_BUDGET_USD',2)))throw Error('budget_exhausted');
 ...
 function reserve(body,{unmetered=false}={}){const day=new Date().toISOString().slice(0,10),month=day.slice(0,7),rate=prices(body.model);
  ...
  transaction(s=>{const[m,d]=buckets(s,day,month);if(!unmetered && (m.usd+usd>number('OPENAI_AGENT_MONTHLY_BUDGET_USD',30)||d.usd+usd>number('OPENAI_AGENT_DAILY_BUDGET_USD',2)))throw Error('budget_exhausted');m.usd+=usd;d.usd+=usd;m.calls++;});
 ...
 return{admit,reserve,usage,unmetered:unmeteredView};
}
