...
function createBudget(directory=process.env.AGENT_BUDGET_DIR||process.env.DATA_DIR||path.join(__dirname,'data')){
 ...
 function admit(client){const day=new Date().toISOString().slice(0,10),month=day.slice(0,7);return transaction(s=>{
  ...
  if((s.clients[key]?.count||0)>=number('AGENT_DAILY_CLIENT_REQUESTS',10))throw Error('daily_quota');
