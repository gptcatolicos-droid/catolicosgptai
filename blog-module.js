...
// Aquí vivían 300 títulos fijos y la función que los sembraba como artículos
// publicados: los 300 compartían un único cuerpo, palabra por palabra, y se
// les inventaba una fecha retrasada para simular un historial. Se retiran con
// el resto del contenido de plantilla. El blog se llena por el generador
// diario y por el admin, que es de donde sale contenido que alguien escribió.
...
function loadBlog() {
  ...
  // Aquí había un relleno automático: si el catálogo bajaba de mil artículos,
  // se regeneraba el lote de plantilla entero, y si eso fallaba se sembraban
  // otros 300. Era la razón de que la limpieza no durase. Un catálogo sano
  // nunca va a tener mil artículos escritos de verdad, así que la condición se
  // cumplía siempre: cada arranque deshacía lo que la retirada había hecho.
  //
  // El catálogo se llena ahora sólo por donde debe: el generador diario, que
  // escribe diez artículos con fuentes, y lo que se publique desde el admin.
  // Si está vacío, se queda vacío; eso es un dato, no un problema que tapar.
  if (!catalog.posts) catalog.posts = [];
  return catalog;
}
...
function parseMarkdown(md) {
  ...
  if (lower.includes('<table') || lower.includes('<div') || lower.includes('<p') || lower.includes('</ul>') || lower.includes('</ol>') || lower.includes('<article') || lower.includes('<section') || lower.includes('<h1') || lower.includes('<h2') || lower.includes('<h3') || lower.includes('<h4') || lower.includes('<span') || lower.includes('<strong') || lower.includes('<em') || lower.includes('<img') || lower.includes('<figure') || lower.includes('<iframe') || lower.includes('<style')) {
